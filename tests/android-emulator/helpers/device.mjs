import { adbExec, adbShell, adbInstall, adbForceStop, adbClearApp, adbStartActivity, adbKeyEvent } from './adb.mjs';
...
export const getCurrentFocusWindow = async (deviceId) => {
  const { stdout } = await adbShell(deviceId, 'dumpsys window windows | grep -E "mCurrentFocus|mFocusedApp"');
  return stdout.trim();
};

export const isKeyguardShowing = async (deviceId) => {
  const { stdout } = await adbShell(deviceId, 'dumpsys window policy | grep -E "isStatusBarKeyguard|mShowingLockscreen|mDreamingLockscreen"');
...
export const ensureDeviceReadyForAutomation = async ({
  ...
  throw new Error(`Device preflight failed: app not focused or keyguard still active (focus=${focus})`);
};
