...
import {
  ...
  setMockConfigLoader,
} from "../../src/lib/mock/mockConfig.js";
...
export async function createMockC64Server(
  initial: Record<
    string,
    Record<string, string | number | ConfigItemState>
  > = {},
  ...
  const yamlState =
    Object.keys(initial).length === 0 ? await buildStateFromYaml() : {};
  const defaults =
    Object.keys(initial).length === 0
      ? yamlState
      : normalizeInitialState(initial);
  ...
  const toDriveStateKey = (value: string): keyof typeof driveState | null => {
    ...
    if (normalized === "iec drive" || normalized === "soft iec drive")
      return "softiec";
  ...
  const syncDriveStateFromConfig = (
    category: string,
    item: string,
    value: unknown,
  ) => {
    ...
    if (category === "SoftIEC Drive Settings") {
      if (item === "IEC Drive")
        driveState.softiec.enabled = parseEnabledValue(value);
    ...
    if (category === "Printer Settings") {
      if (item === "IEC printer")
        driveState.printer.enabled = parseEnabledValue(value);
      if (item === "Bus ID") {
        const numeric = parseNumericValue(value);
  ...
  const server = http.createServer(
    (req: IncomingMessage, res: ServerResponse) => {
      ...
      const drivePowerOrResetMatch = parsed.pathname.match(
      ...
      if (
        parsed.pathname === "/v1/runners:sidplay" &&
      ...
      if (
        parsed.pathname === "/v1/machine:writemem" &&
        (method === "POST" || method === "PUT")
      ) {
        return sendJson(200, { errors: [] });
      }
