...
export type MockTimingMode = "fast" | "realistic";
...
const FAST_TIMING_SCALE = 0.1;
const FAST_MINIMUM_DELAY_MS = 5;
...
export const resolveMockTimingDelayMs = (params: {
  ...
  const { profile, method, pathname, requestSequence, faultMode, timingMode, latencyOverrideMs } = params;
  ...
  const resolvedDelayMs =
    baseDelayMs + resolveDeterministicJitterMs(profile, { ...timingClass, jitterRangeMs }, requestSequence);
  if (timingMode === "realistic") {
    return resolvedDelayMs;
  }
  return Math.max(FAST_MINIMUM_DELAY_MS, Math.round(resolvedDelayMs * FAST_TIMING_SCALE));
};
