...
describe("detectSenderMismatch", () => {
  it("names the address the packets came from and the address the app expected", () => {
    const mismatch = detectSenderMismatch(
      { rejectedPackets: 4210, lastRejectedSource: "192.0.2.148", expectedSource: "192.0.2.9" },
      "192.0.2.9",
    );
    expect(mismatch).toEqual({ source: "192.0.2.148", expected: "192.0.2.9", rejectedPackets: 4210 });
  });

  it("falls back to the resolved filter address when the saved host is not known", () => {
    const mismatch = detectSenderMismatch(
      { rejectedPackets: 12, lastRejectedSource: "192.0.2.148", expectedSource: "192.0.2.9" },
      null,
    );
    expect(mismatch?.expected).toBe("192.0.2.9");
  });

  it("reports nothing when the filter refused nothing", () => {
    expect(detectSenderMismatch({ rejectedPackets: 0, expectedSource: "192.0.2.9" }, "c64u")).toBeNull();
  });
  ...
  it("reports nothing when the refused address is the one the app is already accepting", () => {
    ...
    expect(
      detectSenderMismatch(
        { rejectedPackets: 30, lastRejectedSource: "192.0.2.148", expectedSource: "192.0.2.148" },
        "192.0.2.148",
      ),
    ).toBeNull();
  });
...
describe("describeSenderMismatch", () => {
  it("says where the packets are coming from rather than that the stream stopped", () => {
    const mismatch = { source: "192.0.2.148", expected: "c64u", rejectedPackets: 900 };
    expect(describeSenderMismatch(mismatch, "video")).toBe(
      "Video packets are arriving from 192.0.2.148 and being dropped — the app is only accepting packets from c64u.",
    );
    expect(describeSenderMismatch(mismatch, "audio")).toMatch(/^Audio packets are arriving from 192\.0\.2\.148/);
  });

  it("still names the sender when the expected address is unknown", () => {
    expect(describeSenderMismatch({ source: "198.51.100.4", expected: null, rejectedPackets: 5 }, "video")).toBe(
      "Video packets are arriving from 198.51.100.4 and being dropped.",
    );
  });
});
...
describe("describeSenderAdoption", () => {
  it("labels the recovery with the address it will accept", () => {
    expect(describeSenderAdoption({ source: "192.0.2.148", expected: "c64u", rejectedPackets: 1 })).toBe(
      "Use 192.0.2.148",
    );
  });
});
