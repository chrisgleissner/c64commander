...
describe("reduceVolumeState", () => {
  ...
  it("handles sync", () => {
    const result = reduceVolumeState(baseState, {
      type: "sync",
      index: 8,
      muted: true,
    });
  ...
  it("handles unmute with reason logic", () => {
    ...
    // Unmute manual
    const r1 = reduceVolumeState(mutedState, {
      type: "unmute",
      reason: "manual",
    });
    ...
    // Unmute sync
    const r2 = reduceVolumeState(mutedState, {
      type: "unmute",
      reason: "sync",
    });
    ...
    const r3 = reduceVolumeState(mutedState, {
      type: "unmute",
      reason: "manual",
      index: 9,
    });
