...
describe("playbackPersistenceBudget", () => {
  it("allows persistence when payload is within item and size budgets", () => {
    ...
    expect(shouldPersistLegacyPlaylistBlob(playlist as any, LEGACY_PLAYLIST_MAX_BYTES)).toBe(true);
  });
  ...
  it("blocks persistence when payload bytes exceed budget", () => {
    ...
    expect(shouldPersistLegacyPlaylistBlob(playlist as any, LEGACY_PLAYLIST_MAX_BYTES + 1)).toBe(false);
  });
});
