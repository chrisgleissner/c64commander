...
vi.mock("@/components/ui/slider", () => ({
  Slider: ({
    ...
    <div
      data-testid="slider"
      data-value={value[0]}
      data-midpoint={midpoint ? "true" : "false"}
    >
      <input
        type="range"
        value={value[0]}
...
vi.mock("@/components/ui/select", () => ({
  Select: ({ children, value, onValueChange, disabled }: any) => (
    <div data-testid="select" data-value={value} data-disabled={disabled}>
      <button onClick={() => onValueChange && onValueChange("opt1")}>
        Change
      </button>
      {children}
    </div>
...
describe("SidCard", () => {
  ...
  it("renders shaping controls (editable)", () => {
    ...
    render(
      <SidCard
        {...defaultProps}
        shapingControls={[
          { label: "Shape", value: "V", options: ["V"], onChange },
        ]}
      />,
    );
  ...
  it("renders shaping controls (readonly)", () => {
    render(
      <SidCard
        {...defaultProps}
        shapingControls={[{ label: "Shape", value: "V" }]}
      />,
    ); // No options/onChange
  ...
  it("checks midpoint props passed", () => {
    render(
      <SidCard {...defaultProps} volumeMidpoint={null} panMidpoint={50} />,
    );
    const sliders = screen.getAllByTestId("slider");
