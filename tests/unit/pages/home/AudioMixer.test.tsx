...
vi.mock('@/pages/home/SidCard', () => ({
    SidCard: (props: any) => (
        <div data-testid={`sid-card-${props.testIdSuffix}`}>
            ...
            <button data-testid="power-toggle" onClick={props.onPowerToggle}>Toggle</button>
            <button data-testid="volume-change" onClick={() => props.onVolumeChange?.(5)}>VolChange</button>
            <button data-testid="volume-commit" onClick={() => props.onVolumeCommit?.(5)}>VolCommit</button>
            <button data-testid="volume-async-change" onClick={() => props.onVolumeChangeAsync?.(5)}>VolAsyncChange</button>
            <button data-testid="volume-async-commit" onClick={() => props.onVolumeCommitAsync?.(5)}>VolAsyncCommit</button>
            <button data-testid="pan-change" onClick={() => props.onPanChange?.(3)}>PanChange</button>
            <button data-testid="pan-commit" onClick={() => props.onPanCommit?.(3)}>PanCommit</button>
            <button data-testid="pan-async-change" onClick={() => props.onPanChangeAsync?.(3)}>PanAsyncChange</button>
            <button data-testid="pan-async-commit" onClick={() => props.onPanCommitAsync?.(3)}>PanAsyncCommit</button>
            <button data-testid="identity-change" onClick={() => props.onIdentityChange?.('NewProfile')}>IdentityChange</button>
            <button data-testid="address-change" onClick={() => props.onAddressChange?.('$DE00')}>AddressChange</button>
        </div>
    ),
}));
...
vi.mock('@/components/SectionHeader', () => ({
    SectionHeader: (props: any) => (
        <div data-testid={props.resetTestId}>
            ...
            <button onClick={props.resetAction} disabled={props.resetDisabled} data-testid="reset-btn">Reset</button>
        </div>
    ),
}));
...
describe('AudioMixer', () => {
    ...
    const defaultProps = {
        ...
        runMachineTask: vi.fn().mockImplementation(async (_id: string, action: () => Promise<void>) => { await action(); }),
    };
