...
describe('useHomeActions', () => {
    beforeEach(() => {
        ...
        selectRamDumpFolderMock.mockResolvedValue({ treeUri: 'content://tree/music', rootName: 'Music' });
    ...
    it('reports pause/resume failures and clears pending state', async () => {
        ...
        expect(addErrorLogMock).toHaveBeenCalledWith('Machine pause/resume failed', expect.objectContaining({
            targetState: 'paused',
            error: 'pause failed',
        }));
        expect(reportUserErrorMock).toHaveBeenCalledWith(expect.objectContaining({
