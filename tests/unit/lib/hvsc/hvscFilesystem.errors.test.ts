...
describe("hvscFilesystem error handling", () => {
  ...
  describe("ensureDir", () => {
    ...
    it("extracts nested error messages correctly", async () => {
      ...
      await expect(hvscFS.ensureHvscDirs()).rejects.toEqual({
        error: { message: "Deep failure" },
      });
  ...
  describe("helper util coverage", () => {
    ...
    it("listHvscFolder handles string entries (legacy/fallback)", async () => {
      ...
      expect(Filesystem.stat).toHaveBeenCalledWith(
        expect.objectContaining({
          path: expect.stringContaining("string-file.sid"),
        }),
      );
    });

    it("decodeBase64Text returns raw string on failure", async () => {
      vi.mocked(sidUtils.base64ToUint8).mockImplementationOnce(() => {
    ...
    it("getHvscSongByVirtualPath handles durations logic", async () => {
      vi.mocked(resolveHvscSonglengthDuration).mockResolvedValueOnce({
        durations: [10, 20],
        durationSeconds: 10,
      } as any);
