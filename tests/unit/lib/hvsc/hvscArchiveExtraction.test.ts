...
describe('hvscArchiveExtraction', () => {
  ...
  describe('7z extraction', () => {
    ...
    it('falls back to zip if 7z fails', async () => {
      // Mock 7z failure
      mockModule.callMain.mockImplementation(() => {
        throw new Error('7z broken');
      });

      // Mock Zip success
      // @ts-expect-error - mock return shape
      vi.mocked(unzipSync).mockReturnValue({
        'fallback.txt': new Uint8Array([1]),
      });
    ...
    it('throws if 7z fails and fallback fails', async () => {
      ...
      vi.mocked(unzipSync).mockImplementation(() => {
        throw new Error('Zip also broken');
      });
    ...
    it('handles cleanup errors gracefully', async () => {
      ...
      expect(addErrorLog).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ step: 'rmdir-output' }),
      );
      expect(addErrorLog).toHaveBeenCalledWith(
        'SevenZip cleanup failed',
        expect.objectContaining({ step: 'unlink-archive' }),
      );
      expect(addErrorLog).toHaveBeenCalledWith(
        'SevenZip cleanup failed',
        expect.objectContaining({ step: 'rmdir-workdir' }),
      );
    });
  });
