...
describe('getCheckboxMapping', () => {
    ...
    it('returns checkbox mapping for Enabled/Disabled', () => {
        ...
        expect(result).toEqual({ checkedValue: 'Enabled', uncheckedValue: 'Disabled' });
    });
...
describe('inferControlKind', () => {
    ...
    it('returns checkbox for Enabled/Disabled values', () => {
        expect(inferControlKind({ name: 'Feature', currentValue: 'Enabled', possibleValues: ['Enabled', 'Disabled'] }))
            .toBe('checkbox');
    });

    it('returns checkbox for On/Off values', () => {
        expect(inferControlKind({ name: 'Feature', currentValue: 'On', possibleValues: ['On', 'Off'] }))
            .toBe('checkbox');
    });

    it('returns slider for Off/Low/Medium/High values', () => {
        expect(inferControlKind({ name: 'Volume', currentValue: 'Low', possibleValues: ['Off', 'Low', 'Medium', 'High'] }))
            .toBe('slider');
    });

    it('returns slider for audio mixer volume', () => {
        expect(inferControlKind({ name: 'Vol SID', category: 'Audio Mixer', currentValue: '100', possibleValues: ['0', '50', '100'] }))
            .toBe('slider');
    });

    it('returns slider for numeric option list', () => {
        expect(inferControlKind({ name: 'Frequency', currentValue: '985', possibleValues: ['985', '1050', '1250'] }))
            .toBe('slider');
    });

    it('returns slider for left/center/right values', () => {
        expect(inferControlKind({
            name: 'Position',
            currentValue: 'Center',
    ...
    it('returns select for 2+ non-checkbox non-slider values', () => {
        expect(inferControlKind({ name: 'Mode', currentValue: 'A', possibleValues: ['A', 'B', 'C'] }))
            .toBe('select');
    });

    it('returns text when no possibleValues provided', () => {
        expect(inferControlKind({ name: 'CustomValue', currentValue: 'hello' }))
            .toBe('text');
    });

    it('returns text when possibleValues is empty', () => {
        expect(inferControlKind({ name: 'CustomValue', currentValue: 'hello', possibleValues: [] }))
            .toBe('text');
    });

    it('returns text when possibleValues has only one option', () => {
        expect(inferControlKind({ name: 'CustomValue', currentValue: 'hello', possibleValues: ['onlyone'] }))
            .toBe('text');
    });

    it('returns select when possibleValues has exactly 2 unrecognized values', () => {
        expect(inferControlKind({ name: 'Mode', currentValue: 'X', possibleValues: ['X', 'Y'] }))
            .toBe('select');
    });

    it('returns slider for too many numeric options at limit', () => {
        const values = Array.from({ length: 40 }, (_, i) => String(i));
        expect(inferControlKind({ name: 'Count', currentValue: '0', possibleValues: values }))
            .toBe('slider');
    });
    ...
    it('returns select when some values have MHz but not all numeric (line 75 FALSE branch)', () => {
        ...
        expect(inferControlKind({ name: 'CPU Speed', currentValue: '1.2 MHz', possibleValues: ['1.2 MHz', 'Turbo'] }))
            .toBe('select');
    });

    it('returns slider for volume name containing volume keyword', () => {
        expect(inferControlKind({ name: 'System Volume', currentValue: '80', possibleValues: ['0', '50', '80', '100'] }))
            .toBe('slider');
    });
