...
describe("fileValidation", () => {
  ...
  it("accepts valid samples for all supported file types", () => {
    ...
    expect(validateFileBytes(new Uint8Array(351062), "d71")).toMatchObject({ ok: true, detectedType: "d71" });
  ...
  it("accepts RSID headers and strong-type mismatches for non-CRT formats", () => {
    ...
    const headerOnlySid = new Uint8Array(0x76);
    headerOnlySid.set(ascii("PSID"), 0);
    setBE16(headerOnlySid, 4, 2);
    setBE16(headerOnlySid, 6, 0x76);
    setBE16(headerOnlySid, 14, 1);
    setBE16(headerOnlySid, 16, 1);
    expect(validateFileBytes(headerOnlySid, "sid")).toMatchObject({ ok: false, code: "INVALID_DATA_OFFSET" });
  ...
  it("uses the preferred weak type and reports unknown bytes when no strong type exists", () => {
    expect(FileTypeDetector.detect(Uint8Array.from([0x01, 0x08, 0x60]), "prg")).toBe("prg");
    expect(FileTypeDetector.detect(Uint8Array.from([0xde, 0xad, 0xbe, 0xef]))).toBe("unknown");
  });
