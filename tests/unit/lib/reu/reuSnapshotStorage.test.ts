...
import { persistReuSnapshotFile, readReuSnapshotBytes, deleteReuSnapshotFile } from "@/lib/reu/reuSnapshotStorage";
...
describe("reuSnapshotStorage", () => {
  ...
  describe("persistReuSnapshotFile", () => {
    ...
    it("saves to native-data when native but not android", async () => {
      ...
      expect(mkdir).toHaveBeenCalledWith(expect.objectContaining({ path: "reu-snapshots", recursive: true }));
      expect(writeFile).toHaveBeenCalledWith(expect.objectContaining({ path: "reu-snapshots/my-snapshot.reu" }));
