...
describe('actionSummaries', () => {
    it('builds complete summary with REST/FTP/error effects', () => {
        const events: TraceEvent[] = [
            makeEvent('1', 'action-start', 'abc', 10, { name: 'Refresh', trigger: { kind: 'user', name: 'tap', intervalMs: null, details: null } }),
            makeEvent('2', 'rest-request', 'abc', 11, { method: 'GET', normalizedUrl: '/v1/version', target: 'real-device' }),
            makeEvent('3', 'rest-response', 'abc', 13, { status: 200, durationMs: 123, body: { product: 'C64U' } }),
            makeEvent('4', 'ftp-operation', 'abc', 14, { operation: 'LIST', path: '/music', target: 'real-device', result: 'ok' }),
        ...
        expect(summary.effects).toEqual(
            expect.arrayContaining([
                expect.objectContaining({ type: 'REST', method: 'GET', path: '/v1/version', product: 'C64U', status: 200 }),
                expect.objectContaining({ type: 'FTP', operation: 'LIST', path: '/music' }),
    ...
    it('handles incomplete actions with fallback naming and unknown origin', () => {
        const events: TraceEvent[] = [
            makeEvent('1', 'rest-request', 'missing-start', 30, { method: 'POST', url: 'http://127.0.0.1:8080/v1/config' }),
            makeEvent('2', 'rest-response', 'missing-start', 31, { status: 503, error: 'service unavailable' }),
        ];
        ...
        expect(summary.effects).toEqual(
            expect.arrayContaining([
                expect.objectContaining({ type: 'REST', status: 503, error: 'service unavailable' }),
            ])
        );
    });
    ...
    it('adds pending REST request effect and action-end fallback error', () => {
        const events: TraceEvent[] = [
            ...
            makeEvent('2', 'rest-request', 'pending', 2, { method: 'PUT', url: '/v1/config', target: 'internal-mock' }),
        ...
        expect(summary.effects).toEqual(
            expect.arrayContaining([
                expect.objectContaining({ type: 'REST', method: 'PUT', status: 'error', error: 'write failed' }),
                expect.objectContaining({ type: 'ERROR', label: 'action-end error', message: 'write failed' }),
            ])
        );
    });
});
