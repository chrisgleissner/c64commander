...
import { loadLocalSources, getLocalSourceListingMode, getLocalSourceRuntimeFile, requireLocalSourceEntries } from '@/lib/sourceNavigation/localSourcesStore';
import {
    buildDiskMountType,
...
describe('diskMount', () => {
    ...
    describe('resolveLocalDiskBlob', () => {
        ...
        it('reads from localUri when available', async () => {
            vi.mocked(FolderPicker.readFile).mockResolvedValue({ data: btoa('test data') });
            const blob = await resolveLocalDiskBlob({ path: '/test.d64', localUri: 'content://uri', location: 'local' } as any);
            expect(blob).toBeInstanceOf(Blob);
            expect(FolderPicker.readFile).toHaveBeenCalledWith({ uri: 'content://uri' });
        });

        it('reads from localTreeUri when available', async () => {
            vi.mocked(FolderPicker.readFileFromTree).mockResolvedValue({ data: btoa('tree data') });
            const blob = await resolveLocalDiskBlob({ path: '/test.d64', localTreeUri: 'tree://uri', location: 'local' } as any);
            expect(blob).toBeInstanceOf(Blob);
            expect(FolderPicker.readFileFromTree).toHaveBeenCalledWith({ treeUri: 'tree://uri', path: '/test.d64' });
        });
        ...
        it('falls back to local sources when no direct uri', async () => {
            ...
            vi.mocked(FolderPicker.readFileFromTree).mockResolvedValue({ data: btoa('source data') });

            const blob = await resolveLocalDiskBlob({ path: '/test.d64', location: 'local' } as any);
            expect(blob).toBeInstanceOf(Blob);
        });
        ...
        it('resolves from source entries in entries mode', async () => {
            ...
            vi.mocked(FolderPicker.readFile).mockResolvedValue({ data: btoa('entry data') });
        ...
        it('throws when no source can resolve the file', async () => {
            vi.mocked(loadLocalSources).mockReturnValue([]);
            await expect(resolveLocalDiskBlob({ path: '/test.d64', location: 'local' } as any)).rejects.toThrow('Local disk access is missing');
        });
        ...
        it('tries matching source by sourceId first', async () => {
            ...
            const blob = await resolveLocalDiskBlob({ path: '/test.d64', location: 'local', sourceId: 'src2' } as any);
            expect(blob).toBe(runtimeFile);
    ...
    describe('mountDiskToDrive', () => {
        ...
        it('mounts ultimate disk via API', async () => {
            await mountDiskToDrive(mockApi as any, 'a', { path: '/disk.d64', location: 'ultimate' } as any);
            expect(mockApi.mountDrive).toHaveBeenCalledWith('a', '/disk.d64', 'd64', 'readwrite');
        });

        it('throws for unsupported disk type', async () => {
            await expect(mountDiskToDrive(mockApi as any, 'a', { path: '/disk', location: 'ultimate' } as any)).rejects.toThrow('Unsupported');
        });
        ...
        it('logs and rethrows on mount failure', async () => {
            ...
            await expect(mountDiskToDrive(mockApi as any, 'a', { path: '/disk.d64', location: 'ultimate' } as any)).rejects.toThrow('mount error');
        });
    });
});
