...
const usePlaybackHarness = ({
    ...
    const defaultBuildPlaylistItem = (entry: PlayableEntry, songNrOverride?: number, addedAtOverride?: string | null): PlaylistItem | null => ({
...
describe('usePlaybackPersistence – edge cases', () => {
    ...
    it('skips playlist overwrite when current playlist exists and restored is empty', async () => {
        ...
        const existingItem: PlaylistItem = {
            id: 'hvsc::existing',
            request: { source: 'hvsc', path: '/existing.sid', file: undefined, songNr: undefined },
            category: 'sid',
            label: 'existing.sid',
            path: '/existing.sid',
            addedAt: new Date().toISOString(),
    ...
    it('persist playlist with items having null addedAt and undefined status (uses fallbacks)', async () => {
        ...
        const buildPlaylistItemOverride = (entry: PlayableEntry, songNrOverride?: number, addedAtOverride?: string | null): PlaylistItem | null => ({
            id: `${entry.source}:${entry.sourceId ?? ''}:${entry.path}`,
            request: { source: entry.source, path: entry.path, file: entry.file, songNr: songNrOverride },
            category: 'sid',
            label: entry.name,
            path: entry.path,
            durationMs: entry.durationMs,
        ...
        await waitFor(() => {
            ...
            if (stored) {
                const parsed = JSON.parse(stored) as { items: Array<{ addedAt?: string | null; status?: string }> };
