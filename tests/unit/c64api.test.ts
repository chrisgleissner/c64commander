...
describe("c64api", () => {
  ...
  // HARD18-013 (M2): interactive multi-item Audio Mixer/lighting writes ride
  // the same tempfile-buffering POST /v1/configs handler documented above -
  // hardware-verified to wedge the network stack (u64 fw 3.15 alpha wedged on
  // the FIRST such POST during the mandated HIL soak, 2026-07-07). Until the
  // firmware-gated flag is HIL-validated, a small multi-item batch to these
  // hot categories must decompose to sequential single-item PUTs.
  it("HARD18-013: decomposes a two-item Audio Mixer batch into sequential PUTs, never one POST", async () => {
    const fetchMock = getFetchMock();
    fetchMock
      .mockResolvedValueOnce(
        categoryConfigResponse("Audio Mixer", {
          "Vol Master": { selected: "0 dB", options: ["0 dB", "-10 dB"] },
          "Vol UltiSid 1": { selected: "0 dB", options: ["0 dB", "-10 dB"] },
        }),
      )
      .mockResolvedValueOnce(okJsonResponse())
      .mockResolvedValueOnce(okJsonResponse());
    ...
    const api = new C64API("http://c64u");
    await api.updateConfigBatch({
      "Audio Mixer": { "Vol Master": "-10 dB", "Vol UltiSid 1": "-10 dB" },
    });

    const postWrites = fetchMock.mock.calls.filter(([, init]) => (init as RequestInit | undefined)?.method === "POST");
    expect(postWrites).toHaveLength(0);
    const putWrites = fetchMock.mock.calls
      .filter(([, init]) => (init as RequestInit | undefined)?.method === "PUT")
      .map(([input]) => String(input));
    expect(putWrites).toEqual([
      expect.stringContaining("/v1/configs/Audio%20Mixer/Vol%20Master?value=-10%20dB"),
      expect.stringContaining("/v1/configs/Audio%20Mixer/Vol%20UltiSid%201?value=-10%20dB"),
    ]);
  });

  it("HARD18-013: decomposes a two-item lighting batch (LED Strip Settings) into sequential PUTs", async () => {
    const fetchMock = getFetchMock();
    fetchMock
      .mockResolvedValueOnce(
        categoryConfigResponse("LED Strip Settings", {
          "LedStrip Mode": { selected: "Off", options: ["Off", "Static"] },
    ...
    const api = new C64API("http://c64u");
    await api.updateConfigBatch({
      "LED Strip Settings": { "LedStrip Mode": "Static", "Strip Intensity": 20 },
    });
    ...
    const putWrites = fetchMock.mock.calls
      ...
      .map(([input]) => String(input));
    expect(putWrites).toEqual([
      expect.stringContaining("/v1/configs/LED%20Strip%20Settings/LedStrip%20Mode?value=Static"),
      expect.stringContaining("/v1/configs/LED%20Strip%20Settings/Strip%20Intensity?value=20"),
    ]);
  });

  it("HARD18-013: a genuine bulk multi-category batch still POSTs, unaffected by hot-category decomposition", async () => {
    const fetchMock = getFetchMock();
    const audioItems = Object.fromEntries(
      Array.from({ length: 10 }, (_, i) => [`Vol Item ${i}`, { selected: "0 dB", options: ["0 dB", "-10 dB"] }]),
    );
    const otherItems = Object.fromEntries(
      Array.from({ length: 10 }, (_, i) => [`Other Item ${i}`, { selected: "0 dB", options: ["0 dB", "-10 dB"] }]),
    );
    fetchMock
      .mockResolvedValueOnce(categoryConfigResponse("Audio Mixer", audioItems))
      .mockResolvedValueOnce(categoryConfigResponse("Other Category", otherItems))
      .mockResolvedValueOnce(okJsonResponse());

    const api = new C64API("http://c64u");
    await api.updateConfigBatch({
      "Audio Mixer": Object.fromEntries(Object.keys(audioItems).map((item) => [item, "-10 dB"])),
      "Other Category": Object.fromEntries(Object.keys(otherItems).map((item) => [item, "-10 dB"])),
    });

    const postWrites = fetchMock.mock.calls.filter(([, init]) => (init as RequestInit | undefined)?.method === "POST");
    expect(postWrites).toHaveLength(1);
    const putWrites = fetchMock.mock.calls.filter(([, init]) => (init as RequestInit | undefined)?.method === "PUT");
    expect(putWrites).toHaveLength(0);
  });
