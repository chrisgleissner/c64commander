...
describe('mockConfig', () => {
          ...
          Audio: {
            items: {
              Volume: {
                selected: '+6 dB',
                options: ['+6 dB', '0 dB'],
                details: { min: '0', max: 10, format: 'db', presets: ['0', '6'] },
              },
            },
          },
        },
  ...
  it('builds and caches payload from yaml', () => {
    ...
    const cached = getMockConfigPayload();
    expect(cached).toBe(payload);
  });

  it('resets cache when loader changes', () => {
    const first = getMockConfigPayload();
    setMockConfigLoader(() => ({ config: { general: { base_url: 'http://other' } } }));
    const second = getMockConfigPayload();
