...
const readWorkflow = (name: string) =>
  readFileSync(path.resolve(process.cwd(), '.github/workflows', name), 'utf8');
...
describe('telemetry release gate workflow rules', () => {
  it('hard-fails Android telemetry on monitor exit code 3 for release/tag flows', () => {
    ...
    expect(workflow).toContain(
      'telemetry gate failed: main process disappearance/restart detected on release flow',
    );
    expect(workflow).toContain(
  ...
  it('hard-fails iOS telemetry on monitor exit code 3 for stable tag and release branch flows', () => {
    ...
    expect(workflow).toContain(
      'telemetry gate failed: app process disappearance/restart detected on stable tag flow',
    );
    expect(workflow).toContain(
      'telemetry gate failed: app process disappearance/restart detected on release flow',
    );
    expect(workflow).toContain(
  ...
  it('uploads iOS telemetry and diagnostics artifacts on failure', () => {
    const workflow = readWorkflow('ios.yaml');
    expect(workflow).toContain(
      '- name: Upload iOS failure diagnostics (${{ matrix.group.name }})',
    );
  ...
  it('hardens fuzz monitor lifecycle to always persist exit codes', () => {
    ...
    const trapMatches =
      workflow.match(/trap 'write_code_file "\$status"' EXIT/g) ?? [];
    const fallbackMatches =
      workflow.match(
        /synthesized monitor\.exitcode=1 because wrapper exited before writing status/g,
      ) ?? [];
