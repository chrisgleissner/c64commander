...
describe("renderIssueEntry", () => {
  const baseGroup = {
    ...
    signature: {
      ...
      topFrames: ["src/lib/c64api.ts:42"],
    },
  ...
  const baseCls = {
    domain: "NETWORK",
    confidence: "HIGH",
    explanation: "Network disruption.",
  };
  ...
  it("Explanation is omitted when null", () => {
    const lines = renderIssueEntry(baseGroup, {
      domain: "UNKNOWN",
      confidence: "LOW",
      explanation: null,
    });
  ...
  it("classification counts in README header match actual group counts", async () => {
    ...
    const groups = [
      ...
      // Should become UNCERTAIN (backend)
      {
        ...
        signature: {
          message: "JSON parse error in response",
          exception: "app.log.error",
          topFrames: [],
        },
        severityCounts: { errorLog: 1 },
