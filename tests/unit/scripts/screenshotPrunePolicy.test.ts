...
describe("shouldSkipFuzzyScreenshotPrune", () => {
  it("skips fuzzy prune for import screenshots and profiles", () => {
    ...
    expect(shouldSkipFuzzyScreenshotPrune("docs/img/app/play/import/03-local-file-picker.png")).toBe(true);
    expect(shouldSkipFuzzyScreenshotPrune("docs/img/app/play/import/profiles/compact/02-c64u-file-picker.png")).toBe(
      true,
    );
  });
  ...
  it("does not skip fuzzy prune for unrelated screenshots", () => {
    ...
    expect(shouldSkipFuzzyScreenshotPrune("docs/img/app/settings/01-overview.png")).toBe(false);
  });
});
