...
describe("DecisionStateView", () => {
  ...
  it("renders the fallback metrics when no recent transport effects exist", () => {
    render(<DecisionStateView onBack={vi.fn()} onRepair={vi.fn()} repairRunning={false} actionSummaries={[]} />);
  ...
  it("renders populated decision, health-check, and transport metrics", () => {
    ...
    expect(screen.getByTestId("decision-state-reconcilers")).toHaveTextContent(
      "Remote config differed from local cache",
    );
  ...
  it("invokes back and repair actions and disables repair while running", () => {
    ...
    render(<DecisionStateView onBack={onBack} onRepair={onRepair} repairRunning={true} actionSummaries={[]} />);
