...
describe('buildActionSummaries', () => {
  ...
  it('uses unknown error label when error event has no message', () => {
    const traces: TraceEvent[] = [
      buildTrace({ id: 'E1', type: 'action-start', correlationId: 'C1', relativeMs: 0, data: { name: 'test' } }),
      buildTrace({ id: 'E2', type: 'error', correlationId: 'C1', relativeMs: 50, data: {} }),
      buildTrace({ id: 'E3', type: 'action-end', correlationId: 'C1', relativeMs: 100, data: { status: 'error' } }),
    ];
    const [summary] = buildActionSummaries(traces);
  ...
  it('deduplicates error effects when same message appears in error event and action-end', () => {
    const traces: TraceEvent[] = [
      buildTrace({ id: 'E1', type: 'action-start', correlationId: 'C1', relativeMs: 0, data: { name: 'test' } }),
      buildTrace({ id: 'E2', type: 'error', correlationId: 'C1', relativeMs: 50, data: { message: 'disk full' } }),
      buildTrace({ id: 'E3', type: 'action-end', correlationId: 'C1', relativeMs: 100, data: { status: 'error', error: 'disk full' } }),
    ];
    const [summary] = buildActionSummaries(traces);
    const errorEffects = (summary.effects ?? []).filter((e) => e.type === 'ERROR');
  ...
  it('adds default error effect when no error events and no error field but status is error', () => {
    const traces: TraceEvent[] = [
      buildTrace({ id: 'E1', type: 'action-start', correlationId: 'C1', relativeMs: 0, data: { name: 'test' } }),
      buildTrace({ id: 'E2', type: 'action-end', correlationId: 'C1', relativeMs: 100, data: { status: 'error' } }),
    ];
    const [summary] = buildActionSummaries(traces);
    const errorEffects = (summary.effects ?? []).filter((e) => e.type === 'ERROR');
    expect(errorEffects).toHaveLength(1);
    expect(errorEffects[0] && 'message' in errorEffects[0] ? errorEffects[0].message : '').toBe('action ended with error');
  });
  ...
  it('includes product from REST response body', () => {
    const traces: TraceEvent[] = [
      buildTrace({ id: 'E1', type: 'action-start', correlationId: 'C1', relativeMs: 0, data: { name: 'test' } }),
      buildTrace({ id: 'E2', type: 'rest-request', correlationId: 'C1', relativeMs: 10, data: { method: 'GET', normalizedUrl: '/v1/info', target: 'real-device' } }),
      buildTrace({ id: 'E3', type: 'rest-response', correlationId: 'C1', relativeMs: 50, data: { status: 200, body: { product: '1541ultimate2+' }, durationMs: 40, error: null } }),
      buildTrace({ id: 'E4', type: 'action-end', correlationId: 'C1', relativeMs: 100, data: { status: 'success' } }),
    ];
    const [summary] = buildActionSummaries(traces);
    const restEffect = summary.effects?.find((e) => e.type === 'REST');
  ...
  it('treats array body as null responseBody so product is absent', () => {
    const traces: TraceEvent[] = [
      buildTrace({ id: 'E1', type: 'action-start', correlationId: 'C1', relativeMs: 0, data: { name: 'test' } }),
      buildTrace({ id: 'E2', type: 'rest-request', correlationId: 'C1', relativeMs: 10, data: { method: 'GET', normalizedUrl: '/v1/items', target: 'real-device' } }),
      buildTrace({ id: 'E3', type: 'rest-response', correlationId: 'C1', relativeMs: 50, data: { status: 200, body: [{ id: 1 }], durationMs: 20, error: null } }),
      buildTrace({ id: 'E4', type: 'action-end', correlationId: 'C1', relativeMs: 100, data: { status: 'success' } }),
    ];
    const [summary] = buildActionSummaries(traces);
    const restEffect = summary.effects?.find((e) => e.type === 'REST');
  ...
  it('falls back to endStatus when response has no status key', () => {
    const traces: TraceEvent[] = [
      buildTrace({ id: 'E1', type: 'action-start', correlationId: 'C1', relativeMs: 0, data: { name: 'test' } }),
      buildTrace({ id: 'E2', type: 'rest-request', correlationId: 'C1', relativeMs: 10, data: { method: 'GET', normalizedUrl: '/v1/info', target: 'real-device' } }),
      buildTrace({ id: 'E3', type: 'rest-response', correlationId: 'C1', relativeMs: 50, data: { durationMs: 20, error: null } }),
      buildTrace({ id: 'E4', type: 'action-end', correlationId: 'C1', relativeMs: 100, data: { status: 'success' } }),
    ];
    const [summary] = buildActionSummaries(traces);
    const restEffect = summary.effects?.find((e) => e.type === 'REST');
  ...
  it('falls back to url when normalizedUrl absent in REST request', () => {
    const traces: TraceEvent[] = [
      buildTrace({ id: 'E1', type: 'action-start', correlationId: 'C1', relativeMs: 0, data: { name: 'test' } }),
      buildTrace({ id: 'E2', type: 'rest-request', correlationId: 'C1', relativeMs: 10, data: { method: 'POST', url: 'http://device/v1/configs', target: 'real-device' } }),
      buildTrace({ id: 'E3', type: 'rest-response', correlationId: 'C1', relativeMs: 50, data: { status: 201, durationMs: 30, error: null } }),
      buildTrace({ id: 'E4', type: 'action-end', correlationId: 'C1', relativeMs: 100, data: { status: 'success' } }),
    ];
    const [summary] = buildActionSummaries(traces);
    const restEffect = summary.effects?.find((e) => e.type === 'REST');
  ...
  it('leaves no error field on pending REST request when endError is null', () => {
    const traces: TraceEvent[] = [
      buildTrace({ id: 'E1', type: 'action-start', correlationId: 'C1', relativeMs: 0, data: { name: 'test' } }),
      buildTrace({ id: 'E2', type: 'rest-request', correlationId: 'C1', relativeMs: 10, data: { method: 'GET', normalizedUrl: '/v1/info', target: 'real-device' } }),
      buildTrace({ id: 'E3', type: 'action-end', correlationId: 'C1', relativeMs: 100, data: { status: 'incomplete' } }),
    ];
    const [summary] = buildActionSummaries(traces);
    const restEffect = summary.effects?.find((e) => e.type === 'REST');
    expect(restEffect).toBeDefined();
  ...
  it('resolves origin as unknown for unrecognized origin value', () => {
    const traces: TraceEvent[] = [
      buildTrace({ id: 'E1', type: 'action-start', correlationId: 'C1', relativeMs: 0, origin: 'legacy' as 'user', data: { name: 'test' } }),
      buildTrace({ id: 'E2', type: 'action-end', correlationId: 'C1', relativeMs: 100, data: { status: 'success' } }),
    ];
    const [summary] = buildActionSummaries(traces);
  ...
  it('uses relative duration when event timestamps are null', () => {
    ...
    const traces: TraceEvent[] = [
      { id: 'E1', timestamp: null as unknown as string, relativeMs: 0, type: 'action-start', origin: 'user', correlationId: 'C1', data: { name: 'test' } },
      { id: 'E2', timestamp: null as unknown as string, relativeMs: 50, type: 'rest-response', origin: 'user', correlationId: 'C1', data: { status: 200, durationMs: 50 } },
      { id: 'E3', timestamp: null as unknown as string, relativeMs: 100, type: 'action-end', origin: 'user', correlationId: 'C1', data: { status: 'success' } },
    ];
    const [summary] = buildActionSummaries(traces);
  ...
  it('falls back to UNKNOWN method and unknown path when rest-response has no method or url in request data', () => {
    ...
    const traces: TraceEvent[] = [
      buildTrace({ id: 'E1', type: 'action-start', correlationId: 'C1', relativeMs: 0, data: { name: 'test' } }),
      buildTrace({ id: 'E2', type: 'rest-request', correlationId: 'C1', relativeMs: 10, data: {} }),
      buildTrace({ id: 'E3', type: 'rest-response', correlationId: 'C1', relativeMs: 50, data: { status: 200, durationMs: 30 } }),
      buildTrace({ id: 'E4', type: 'action-end', correlationId: 'C1', relativeMs: 100, data: { status: 'success' } }),
    ];
    const [summary] = buildActionSummaries(traces);
    const restEffect = summary.effects?.find((e) => e.type === 'REST');
  ...
  it('returns null status when readNumber returns null for non-numeric string status', () => {
    ...
    const traces: TraceEvent[] = [
      buildTrace({ id: 'E1', type: 'action-start', correlationId: 'C1', relativeMs: 0, data: { name: 'test' } }),
      buildTrace({ id: 'E2', type: 'rest-request', correlationId: 'C1', relativeMs: 10, data: { method: 'GET', normalizedUrl: '/v1/info', target: 'real-device' } }),
      buildTrace({ id: 'E3', type: 'rest-response', correlationId: 'C1', relativeMs: 50, data: { status: 'ok', durationMs: 30 } }),
      buildTrace({ id: 'E4', type: 'action-end', correlationId: 'C1', relativeMs: 100, data: { status: 'success' } }),
    ];
    const [summary] = buildActionSummaries(traces);
    const restEffect = summary.effects?.find((e) => e.type === 'REST');
  ...
  it('uses null endStatus fallback when rest-response has no status key and actionEnd has no status', () => {
    ...
    const traces: TraceEvent[] = [
      buildTrace({ id: 'E1', type: 'action-start', correlationId: 'C1', relativeMs: 0, data: { name: 'test' } }),
      buildTrace({ id: 'E2', type: 'rest-request', correlationId: 'C1', relativeMs: 10, data: { method: 'GET', normalizedUrl: '/v1/info', target: 'real-device' } }),
      buildTrace({ id: 'E3', type: 'rest-response', correlationId: 'C1', relativeMs: 50, data: { durationMs: 30 } }),
      buildTrace({ id: 'E4', type: 'action-end', correlationId: 'C1', relativeMs: 100, data: {} }),
    ];
    const [summary] = buildActionSummaries(traces);
    const restEffect = summary.effects?.find((e) => e.type === 'REST');
    expect(restEffect && 'status' in restEffect ? restEffect.status : 'missing').toBeNull();
  ...
  it('falls back to unknown operation, path and null target for FTP event missing fields', () => {
    ...
    const traces: TraceEvent[] = [
      buildTrace({ id: 'E1', type: 'action-start', correlationId: 'C1', relativeMs: 0, data: { name: 'test' } }),
      buildTrace({ id: 'E2', type: 'ftp-operation', correlationId: 'C1', relativeMs: 20, data: {} }),
      buildTrace({ id: 'E3', type: 'action-end', correlationId: 'C1', relativeMs: 100, data: { status: 'success' } }),
    ];
    const [summary] = buildActionSummaries(traces);
    const ftpEffect = summary.effects?.find((e) => e.type === 'FTP');
  ...
  it('resolves origin from actionEnd when no action-start event exists', () => {
    ...
    const traces: TraceEvent[] = [
      buildTrace({ id: 'E1', type: 'rest-request', correlationId: 'C1', relativeMs: 10, origin: 'automatic', data: { method: 'GET', normalizedUrl: '/v1/info', target: 'real-device' } }),
      buildTrace({ id: 'E2', type: 'action-end', correlationId: 'C1', relativeMs: 100, origin: 'automatic', data: { status: 'success' } }),
    ];
    const [summary] = buildActionSummaries(traces);
