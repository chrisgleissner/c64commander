...
const getActiveAction = vi.fn(() => null as {
...
const getPlaybackTraceSnapshot = vi.fn(() => null as {
    sourceKind: string;
...
describe('logger', () => {
    ...
    it('normalizes Error objects in details', () => {
        ...
        expect(logged.error).toEqual(expect.objectContaining({
            name: 'Error',
            message: 'oops',
        }));
        spy.mockRestore();
    ...
    it('normalizes string errors in details', () => {
        ...
        const logged = addLog.mock.calls[0][2];
        expect(logged.error).toEqual({ name: 'Error', message: 'string err', stack: null });
        spy.mockRestore();
    });
    ...
    describe('installConsoleDiagnosticsBridge', () => {
        it('returns noop uninstall when explicitly disabled', () => {
            const uninstall = logger.installConsoleDiagnosticsBridge({ enabled: false });
