        ...
        > = [
          [
            ...
            async () => {
              ...
              return { status: r.status, latencyMs: r.latencyMs };
            },
            async () => {
              const r = await request({ method: "GET", url: "/v1/version" });
              return { status: r.status, latencyMs: r.latencyMs };
        ...
        for (const [nameA, nameB, fnA, fnB] of pairs) {
          ...
          log({
            ...
            details: {
              ...
              aOk,
              bOk,
              aLatencyMs: a.latencyMs,
              bLatencyMs: b.latencyMs,
              conflict,
            },
          });
      ...
      run: async ({ config, log }) => {
        ...
        try {
          ...
          try {
            ...
            console.warn("FTP cleanup delete failed", {
          ...
          await Promise.all([
            clientA.close().catch((closeError) => {
              console.warn("FTP client A close failed", {
                error: String(closeError),
              });
            }),
            clientB.close().catch((closeError) => {
              console.warn("FTP client B close failed", {
                error: String(closeError),
              });
            }),
          ]);
      ...
      run: async ({ request, config, log }) => {
        ...
        try {
          ...
          const [restVersion, ftpList] = await Promise.all([
            request({ method: "GET", url: "/v1/version" }).catch((e) => ({
              status: 500,
              latencyMs: 0,
              error: String(e),
            })),
          ...
          const [restConfigs, ftpList2] = await Promise.all([
            request({ method: "GET", url: "/v1/configs" }).catch((e) => ({
              status: 500,
              latencyMs: 0,
              error: String(e),
            })),
          ...
          const [restDrives, ftpMlsd] = await Promise.all([
            request({ method: "GET", url: "/v1/drives" }).catch((e) => ({
              status: 500,
              latencyMs: 0,
              error: String(e),
            })),
