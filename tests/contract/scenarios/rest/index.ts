  ...
  return [
    ...
    scenario("rest.version", true, async ({ request, log }) => {
      ...
      log({
        ...
        details: { correlationId: r.correlationId },
      });
    }),
    ...
    scenario("rest.configs.category", true, async ({ request, log }) => {
      ...
      for (const category of cats.slice(0, 3)) {
        const r = await request({
          method: "GET",
          url: `/v1/configs/${encodeURIComponent(category)}`,
        });
        log({
          kind: "rest",
          ...
          status: r.status,
          latencyMs: r.latencyMs,
          details: { category, correlationId: r.correlationId },
        });
      }
    }),
    ...
    scenario("rest.machine.readmem", true, async ({ request, log }) => {
      ...
      log({
        ...
        status: r.status,
        latencyMs: r.latencyMs,
        details: { address: "D020", length: 2, correlationId: r.correlationId },
      });
    }),
    ...
    scenario("rest.files.info", true, async ({ request, log }) => {
      const r = await request({
        method: "GET",
        url: `/v1/files/${encodeFilePath("/Flash/roms/1541.rom")}:info`,
      });
      log({
        kind: "rest",
        op: "GET /v1/files/{path}:info",
        status: r.status,
        latencyMs: r.latencyMs,
        details: {
    ...
    scenario("rest.configs.safe-write", true, async ({ request, log }) => {
      ...
      if (!targetCategory) {
        log({
          kind: "rest",
          op: "configs.safe-write",
          status: "skipped",
          reason: "no safe category",
        });
        return;
      }
      ...
      if (nextValue === undefined) {
        log({
          ...
          op: "configs.safe-write",
          status: "skipped",
          reason: "no reversible value",
        });
        return;
      }

      const setR = await request({
        method: "PUT",
        url: `/v1/configs/${encodeURIComponent(targetCategory)}/${encodeURIComponent(itemName)}`,
      ...
      if (restoreValue !== undefined) {
        ...
        log({
          ...
          status: restR.status,
          latencyMs: restR.latencyMs,
          details: { correlationId: restR.correlationId },
        });
      }
    }),
    ...
    scenario("rest.configs.batch", true, async ({ request, log }) => {
      ...
      const batchPayload = {
        [target.category]: { [target.item]: target.value },
      };
      const r = await request({
        method: "POST",
        url: "/v1/configs",
    ...
    scenario("rest.configs.load-from-flash", true, async ({ request, log }) => {
      ...
      log({
        kind: "rest",
        op: "PUT /v1/configs:load_from_flash",
        status: r.status,
        latencyMs: r.latencyMs,
        details: { correlationId: r.correlationId },
      });
    ...
    scenario("rest.machine.pause-resume", true, async ({ request, log }) => {
      ...
      log({
        kind: "rest",
        op: "PUT /v1/machine:pause",
        status: pause.status,
        latencyMs: pause.latencyMs,
        details: { correlationId: pause.correlationId },
      });
      await delay(500);
      ...
      log({
        kind: "rest",
        op: "PUT /v1/machine:resume",
        status: resume.status,
        latencyMs: resume.latencyMs,
        details: { correlationId: resume.correlationId },
      });
      await delay(500);
    }),
    ...
    scenario("rest.drives.on-off", true, async ({ request, log }) => {
      ...
      if (!wasEnabled) {
        ...
        log({
          kind: "rest",
          op: "PUT /v1/drives/{drive}:on",
          status: on.status,
          latencyMs: on.latencyMs,
          details: { drive: "b", correlationId: on.correlationId },
        });
        await delay(300);
        const off = await request({ method: "PUT", url: "/v1/drives/b:off" });
        log({
          kind: "rest",
          op: "PUT /v1/drives/{drive}:off",
          status: off.status,
          latencyMs: off.latencyMs,
          details: { drive: "b", correlationId: off.correlationId },
        });
      } else {
        const off = await request({ method: "PUT", url: "/v1/drives/b:off" });
        log({
    ...
    scenario("rest.drives.set-mode", true, async ({ request, log }) => {
      const drivesResp = await request({ method: "GET", url: "/v1/drives" });
      if (drivesResp.status !== 200) {
        log({ kind: "rest", op: "drives.set-mode", status: "skipped" });
        return;
      }
      const drives =
        ((drivesResp.data as Record<string, unknown>).drives as Array<
      ...
      const r = await request({
        method: "PUT",
        url: "/v1/drives/a:set_mode",
        params: { mode: nextMode },
      });
      log({
        kind: "rest",
        op: "PUT /v1/drives/{drive}:set_mode",
        status: r.status,
        latencyMs: r.latencyMs,
        details: { drive: "a", mode: nextMode, correlationId: r.correlationId },
      });
      await delay(300);
      ...
      log({
        kind: "rest",
        op: "PUT /v1/drives/{drive}:set_mode restore",
    ...
    scenario("rest.drives.reset", true, async ({ request, log }) => {
      ...
      log({
        kind: "rest",
        op: "PUT /v1/drives/{drive}:reset",
        status: r.status,
        latencyMs: r.latencyMs,
        details: { drive: "a", correlationId: r.correlationId },
      });
    }),
    ...
    scenario("rest.drives.load-rom", true, async ({ request, log }) => {
      const r = await request({
        method: "PUT",
        url: "/v1/drives/a:load_rom",
        params: { file: "/Flash/roms/1541.rom" },
      });
      log({
        kind: "rest",
        op: "PUT /v1/drives/{drive}:load_rom",
        status: r.status,
        latencyMs: r.latencyMs,
        details: {
          drive: "a",
          file: "/Flash/roms/1541.rom",
          correlationId: r.correlationId,
        },
      });
    }),
    ...
    scenario("rest.machine.reset", false, async ({ request, log, config }) => {
      if (!config.allowMachineReset) {
        log({
          ...
          op: "machine.reset",
          status: "skipped",
          reason: "allowMachineReset=false",
        });
        return;
      }
    ...
    scenario("rest.machine.reboot", false, async ({ request, log, config }) => {
      ...
      for (let i = 0; i < 10; i++) {
        try {
          const p = await request({ method: "GET", url: "/v1/version" });
          if (p.status === 200) {
            log({
              kind: "rest",
              op: "GET /v1/version (post-reboot)",
              status: p.status,
    ...
    scenario("rest.configs.save-to-flash", false, async ({ request, log }) => {
      ...
      log({
        kind: "rest",
        op: "PUT /v1/configs:save_to_flash",
        status: r.status,
        latencyMs: r.latencyMs,
        details: { correlationId: r.correlationId },
      });
    ...
    scenario("rest.drives.mount", false, async ({ request, log, config }) => {
      ...
      const mount = await request({
        ...
        params: {
          image: imagePath,
          type: config.media?.diskType ?? "d64",
          mode: config.media?.diskMode ?? "readonly",
        },
      });
      log({
        kind: "rest",
        ...
        details: {
          drive,
          image: imagePath,
          correlationId: mount.correlationId,
        },
      });
      await delay(500);
      ...
      log({
        ...
        details: { drive, correlationId: rm.correlationId },
      });
    }),
    ...
    scenario("rest.drives.load-rom-upload", false, async ({ request, log }) => {
      ...
      log({
        kind: "rest",
        op: "PUT /v1/drives/{drive}:load_rom (baseline)",
        status: putR.status,
        latencyMs: putR.latencyMs,
        details: { correlationId: putR.correlationId },
      });
      // POST variant requires ROM binary; log coverage
      log({
        kind: "rest",
        op: "POST /v1/drives/{drive}:load_rom",
        status: "documented",
        reason: "exercised via PUT; POST requires binary ROM data",
      });
    }),
...
async function resolveOrScratch(
  config: HarnessConfig,
  log: (event: LogEventInput) => void,
  exts: string[],
  label: string,
...
async function resolveMediaFilePath({
  ...
  try {
    ...
    log({
      kind: "ftp",
      op: "discover.search",
      status: resolved ? "found" : "not-found",
      details: { label, basePath, resolved },
    });
    ...
    Array.from({ length: totalRequests }, async (_, i) => {
      ...
      try {
        ...
        log({
          ...
          status: r.status,
          latencyMs: r.latencyMs,
          details: { correlationId: r.correlationId, concurrent: true },
        });
