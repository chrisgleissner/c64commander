...
export const normalizeSelectOptions = (
  ...
  const includesEmpty =
    options.some((option) => String(option).trim().length === 0) ||
...
const hasNormalizedMatch = (option: string, candidates: string[]) => {
  ...
  return candidates.some(
    (candidate) => normalizedOption === normalizeOptionToken(candidate),
  );
};

export const resolveToggleOption = (
  options: string[],
  enabled: boolean,
  hints?: ToggleOptionHints,
) => {
  ...
  const match = options.find((option) =>
    hasNormalizedMatch(option, [...preferred, ...defaults]),
  );
...
export const resolveOptionIndex = (options: string[], currentValue: string) => {
  ...
  if (numericValue !== null) {
    index = options.findIndex(
      (option) => parseNumericOption(option) === numericValue,
    );
  }
...
export const resolveVolumeCenterIndex = (options: string[]) => {
  const numericIndex = options.findIndex(
    (option) => parseNumericOption(option) === 0,
  );
...
export const clampSliderValue = (value: number, max: number) =>
  Math.min(Math.max(value, 0), max);

export const clampToRange = (value: number, min: number, max: number) =>
  Math.min(Math.max(value, min), max);

export const resolveSliderIndex = (value: number, max: number) =>
  clampSliderValue(Math.round(value), max);
...
export const resolveSidSocketToggleValue = (
  ...
  const match = options.find((option) => {
    ...
    return enable
      ? enabledTokens.includes(normalized)
      : disabledTokens.includes(normalized);
  });
  if (match) return match;
