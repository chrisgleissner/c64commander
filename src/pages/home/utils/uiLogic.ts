...
export const normalizeSelectOptions = (options: string[], currentValue: string) => {
  const cleaned = options.map((option) => String(option)).filter((option) => option.trim().length > 0);
  const unique = Array.from(new Set(cleaned));
  if (currentValue.trim().length > 0 && !unique.includes(currentValue)) {
    unique.push(currentValue);
  }
  const includesEmpty =
    options.some((option) => String(option).trim().length === 0) || currentValue.trim().length === 0;
