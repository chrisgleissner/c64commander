...
export function useVolumeOverride({
  isPlaying,
  isPaused,
}: UseVolumeOverrideProps) {
  ...
  const volumeSteps = useMemo(() => {
    const baseOptions =
      sidVolumeItems.find(
        (item) => Array.isArray(item.options) && item.options.length,
      )?.options ?? [];
  ...
  const volumeSessionSnapshotRef = useRef<Record<
    string,
    string | number
  > | null>(null);
  ...
  const volumeUiTargetRef = useRef<{ index: number; setAtMs: number } | null>(
    null,
  );
  ...
  const snapshotToUpdates = useCallback(
    (
      ...
      if (!snapshot) return {};
      const updates = buildEnabledSidUnmuteUpdates(
        snapshot.volumes,
        sidEnablement,
      );
  ...
  const ensureVolumeSessionSnapshot = useCallback(async () => {
    ...
    if (volumeSessionSnapshotRef.current)
      return volumeSessionSnapshotRef.current;
  ...
  const scheduleVolumeUpdate = useCallback(
    (nextIndex: number, immediate = false) => {
      ...
      if (!target) return;
      const updates = buildEnabledSidVolumeUpdates(
        sidVolumeItems,
        sidEnablement,
        target,
      );
  ...
  const handleToggleMute = useCallback(async () => {
    ...
    if (!volumeMuted) {
      ...
      manualMuteSnapshotRef.current = captureSidMuteSnapshot(
        items,
        sidEnablement,
      );
  ...
  useEffect(() => {
    ...
    const muteValues = enabledSidVolumeItems.map((item) =>
    ...
    if (!activeIndices.length) {
      ...
      const snapshotIndices = snapshot
        ? Object.values(snapshot.volumes).map((value) =>
            resolveVolumeIndex(value),
          )
        : [];
      const muteIndices = muteValues.map((value) => resolveVolumeIndex(value));
      const muteCounts = new Map<number, number>();
      muteIndices.forEach((index) =>
        muteCounts.set(index, (muteCounts.get(index) ?? 0) + 1),
      );
      const muteIndex =
        Array.from(muteCounts.entries()).sort((a, b) => b[1] - a[1])[0]?.[0] ??
        defaultVolumeIndex;
    ...
    const nextIndex =
      Array.from(counts.entries()).sort((a, b) => b[1] - a[1])[0]?.[0] ??
      defaultVolumeIndex;
    const syncDecision = resolveVolumeSyncDecision(
      volumeUiTargetRef.current,
      nextIndex,
      Date.now(),
    );
