...
export class TelnetMock implements TelnetTransport {
    ...
    async connect(_host: string, _port: number): Promise<void> {
        ...
        if (this.password !== null) {
            ...
            this.pendingOutput.push(
                new Uint8Array([...initBytes, ...ris, ...prompt]),
            );
        } else {
            this.authenticated = true;
            const fileBrowser = this.renderScreen();
            this.pendingOutput.push(
                new Uint8Array([...initBytes, ...ris, ...fileBrowser]),
            );
        }
    }
    ...
    async send(data: Uint8Array): Promise<void> {
        ...
        this.sendCount++;
        if (
            this.options.disconnectAfterSends &&
            this.sendCount >= this.options.disconnectAfterSends
        ) {
            this.connected = false;
    ...
    private processInput(text: string): void {
        ...
        if (text === '\x1b[15~' || text === '\x1b[11~') {
            ...
            if (this.menu.submenuOpen) {
                ...
                this.menu.actionIndex = Math.min(
                    actions.length - 1,
                    this.menu.actionIndex + 1,
                );
            } else if (this.menu.open) {
                this.menu.categoryIndex = Math.min(
                    this.fixture.categories.length - 1,
                    this.menu.categoryIndex + 1,
                );
            }
        } else if (text === '\x1b[C') {
            // RIGHT — enter submenu
    ...
    private getVisibleActions(): Array<{ label: string; enabled: boolean }> {
        ...
        return category.actions.filter(
            (a) => !this.options.missingItems?.includes(a.label),
        );
    }
