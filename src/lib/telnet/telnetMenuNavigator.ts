...
export function createMenuNavigator(session: TelnetSessionApi): MenuNavigator {
  ...
  async function navigate(
    ...
    const checkTimeout = () => {
      if (Date.now() - startTime > ACTION_TIMEOUT_MS) {
        throw new TelnetError(
          `Navigation timed out after ${ACTION_TIMEOUT_MS}ms`,
          'TIMEOUT',
          { path, elapsed: Date.now() - startTime },
        );
      }
    };
    ...
    try {
      ...
      state = 'SCANNING_MENU';
      let topMenu = findTopMenu(screen);
      if (!topMenu) {
        throw new TelnetError(
          'Action menu not detected on screen after opening',
          'MENU_NOT_FOUND',
        );
      }
      ...
      if (!subMenu) {
        throw new TelnetError(
          `Submenu not visible after entering category "${categoryLabel}"`,
          'MENU_NOT_FOUND',
          { categoryLabel },
        );
      }
  ...
  async function openActionMenu(
    ...
    if (!findTopMenu(screen)) {
      ...
      if (!findTopMenu(screen)) {
        throw new TelnetError(
          `Action menu not visible after ${menuKey} (tried twice)`,
          'MENU_NOT_FOUND',
          { menuKey },
        );
      }
  ...
  async function navigateToItem(
    ...
    // Find target index
    const targetIndex = menu.items.findIndex((item) =>
      matchLabel(item.label, targetLabel),
    );
    if (targetIndex < 0) {
      const available = menu.items.map((i) => i.label).join(', ');
      throw new TelnetError(
        `Item "${targetLabel}" not found. Available: [${available}]`,
        'ITEM_NOT_FOUND',
        { targetLabel, available: menu.items.map((i) => i.label) },
      );
    }
    ...
    for (let step = 0; step < distance; step++) {
      ...
      if (!refreshedMenu) {
        throw new TelnetError(
          'Menu disappeared during navigation',
          'DESYNC',
          { targetLabel, step },
        );
      }
