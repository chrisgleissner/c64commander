...
export const FEATURE_FLAG_DEFINITIONS: readonly FeatureFlagDefinition[] = [
  ...
  },
  {
    ...
    group: "experimental",
    title: "Audio Mirror",
    description: "Hear the running machine: receive the device audio stream in-app and play it, with an optional audio recording. Off by default: the phone app still needs its stream receiver (a UDP transport) before the audio arrives.",
  },
  {
    ...
    group: "experimental",
    title: "Video Mirror",
    description: "See the running machine: decode and render the device VIC stream to a canvas, with zoom/pan immersive control in Remote Input. Off by default: the phone app still needs its stream receiver (a UDP transport) before the video arrives.",
  },
  {
    id: "new_disk_enabled",
    enabled: true,
