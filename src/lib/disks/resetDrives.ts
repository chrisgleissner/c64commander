...
const DISK_RESET_CLASSES: DriveDeviceClass[] = ["PHYSICAL_DRIVE_A", "PHYSICAL_DRIVE_B", "SOFT_IEC_DRIVE"];
...
const listDiskResetTargets = (payload?: Pick<DrivesResponse, "drives"> | null) => {
  ...
  return DISK_RESET_CLASSES.map(
    (deviceClass) => normalized.devices.find((entry) => entry.class === deviceClass) ?? null,
...
const resetTargets = async (api: ResetApi, targets: KnownDriveDevice[], unsupportedMessage: string) => {
  if (!targets.length) {
...
export const resetDiskDevices = async (api: ResetApi, payload?: Pick<DrivesResponse, "drives"> | null) => {
...
export const resetPrinterDevice = async (api: ResetApi, payload?: Pick<DrivesResponse, "drives"> | null) => {
...
export const resetConnectedDrives = async (api: ResetApi, payload?: Pick<DrivesResponse, "drives"> | null) => {
