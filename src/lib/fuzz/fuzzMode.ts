...
export const isFuzzModeEnabled = () => {
  try {
    if (
      ...
      import.meta.env?.VITE_FUZZ_MODE === '1'
    )
      return true;
  } catch (error) {
  ...
  if ((window as Window & { __c64uFuzzMode?: boolean }).__c64uFuzzMode)
    return true;
  return readStorageValue(FUZZ_MODE_KEY) === '1';
};

export const getFuzzMockBaseUrl = () =>
  readStorageValue(FUZZ_MOCK_BASE_URL_KEY);
...
export const isFuzzSafeBaseUrl = (baseUrl: string) => {
  ...
  if (fuzzMock && baseUrl === fuzzMock && isSafeMockBaseUrl(fuzzMock))
    return true;
  try {
