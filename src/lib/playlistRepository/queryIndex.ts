import type { PlaylistItemRecord, PlaylistQueryOptions, PlaylistQueryResult, PlaylistQueryRow, TrackRecord } from './types';
...
export const buildPlaylistQueryIndex = (
    ...
    rows.forEach((row) => {
        ...
        if (category) {
            idsByCategory[category] = [...(idsByCategory[category] ?? []), rowId];
        }
        buildSearchGrams(row.searchText).forEach((gram) => {
            idsBySearchGram[gram] = [...(idsBySearchGram[gram] ?? []), rowId];
        });
    });
...
export const queryPlaylistIndex = (index: PersistedPlaylistQueryIndex, options: PlaylistQueryOptions): PlaylistQueryResult => {
