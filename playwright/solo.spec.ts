...
test.describe('Config page SID solo routing', () => {
  ...
  test('default state has no solo enabled', async ({ page }: { page: Page }) => {
  ...
  test('solo enable mutes other SIDs without moving sliders', async ({ page }: { page: Page }) => {
    await openAudioMixer(page);
  ...
  test('solo switch toggles active SID instantly', async ({ page }: { page: Page }) => {
    await openAudioMixer(page);
    await page.getByLabel('Solo Vol UltiSid 2').click();
    await expect.poll(() => server.getState()['Audio Mixer']['Vol Socket 1'].value).toBe('OFF');
  ...
  test('solo disable restores configured mix', async ({ page }: { page: Page }) => {
    await openAudioMixer(page);
  ...
  test('navigation reset clears solo and restores mix', async ({ page }: { page: Page }) => {
    await openAudioMixer(page);
    await page.getByLabel('Solo Vol Socket 1').click();
    await expect.poll(() => server.getState()['Audio Mixer']['Vol Socket 2'].value).toBe('OFF');
