...
const seedActivityDots = async (page: Page) => {
  await page.evaluate(() => {
    ...
    const tracing = (
      window as Window & {
        __c64uTracing?: { seedTraces?: (items: TraceEvent[]) => void };
      }
    ).__c64uTracing;
...
test.describe('Home header and diagnostics overlay', () => {
    ...
    await page.addInitScript(() => {
      ...
      (
      ...
      ).__c64uSeedTraces = [
        {
          ...
          type: 'rest-response',
          origin: 'user',
          correlationId: 'COR-dot-1',
          data: { status: 200, durationMs: 5, error: null },
        },
        {
          id: 'EVT-dot-2',
  ...
  test('status indicators open diagnostics as modal without navigation or scroll jump', async ({
    page,
  }: { page: Page }, testInfo: TestInfo) => {
  ...
  test('clear all diagnostics empties every tab', async ({
    ...
    await page
      .getByRole('alertdialog', { name: /Clear diagnostics/i })
        ...
        .poll(async () =>
          page.evaluate(
            () =>
              (
                window as Window & {
                  __c64uDiagnosticsSharePayloads?: unknown[];
                }
              ).__c64uDiagnosticsSharePayloads?.length ?? 0,
          ),
        )
        .toBeGreaterThan(0);
  ...
  test('diagnostics overlay suppresses traces until share/error and restores after close', async ({
    page,
  }: { page: Page }, testInfo: TestInfo) => {
    await page.addInitScript(() => {
      (
