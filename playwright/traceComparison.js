...
const extractUserActionGroups = (events) => {
  ...
  for (let i = 0; i < effectiveStarts.length; i += 1) {
    ...
    const endIndex =
      i + 1 < effectiveStarts.length ? effectiveStarts[i + 1].index - 1 : sorted[sorted.length - 1].index;
    ...
    groupEvents.forEach(({ event, index }) => {
      ...
      if (event.type === "rest-request") {
        ...
        const url =
          typeof data?.normalizedUrl === "string" ? data.normalizedUrl : typeof data?.url === "string" ? data.url : "";
        const request = {
          method,
          url,
...
const isNoisyRestCall = (call) =>
  call.method === "GET" &&
  typeof call.url === "string" &&
  (call.url.startsWith("/v1/info") || call.url.startsWith("/v1/drives") || call.url.startsWith("/v1/configs"));

const isNoisyOnlyAction = (action) =>
  action.restCalls.length > 0 && action.ftpOps.length === 0 && action.restCalls.every((call) => isNoisyRestCall(call));

const filterEssentialActions = (actions) =>
  actions.filter((action) => (action.restCalls.length > 0 || action.ftpOps.length > 0) && !isNoisyOnlyAction(action));
...
const getActionSignature = (action, options = {}) => {
  const restCallsBase = options.ignoreTarget ? action.restCalls.map(({ target, ...call }) => call) : action.restCalls;
  ...
  expectedActions.forEach((expectedAction, expectedIndex) => {
    ...
    if (index === -1) {
      const reusedIndex = actualActions.findIndex(
        (candidate, idx) => used[idx] && isActionEquivalent(expectedAction, candidate),
      );
      if (reusedIndex !== -1) {
        return;
      }
...
export const compareTracesEssential = (expectedEvents, actualEvents) => {
  ...
  const expectedActionsFinal = dropSystemDuplicatesForUserCalls(useUserActions ? expectedUserActions : expectedActions);
  const actualActionsFinal = dropSystemDuplicatesForUserCalls(useUserActions ? actualUserActions : actualActions);
