...
test.describe('FTP performance', () => {
  ...
  test('FTP navigation shows delayed loading indicator on slow requests', async ({
    page,
  }: { page: Page }, testInfo: TestInfo) => {
    await page.route('**/v1/ftp/list', async (route) => {
    ...
    await expect(page.getByTestId('ftp-loading')).toBeVisible({
      timeout: 1200,
    });
  ...
  test('adding large FTP folder avoids ftp.read and completes quickly', async ({
    page,
  }: { page: Page }, testInfo: TestInfo) => {
    let readCount = 0;
    ...
    const megaCollectionRow = dialog
      .locator('[data-testid="source-entry-row"]', {
        hasText: 'Mega Collection',
      })
      .first();
