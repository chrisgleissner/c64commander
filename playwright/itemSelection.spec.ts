...
test.describe('Item Selection Dialog UX', () => {
  ...
  test('add items modal has single close button', async ({ page }: { page: Page }, testInfo) => {
  ...
  test('add items modal does not occupy full viewport height', async ({ page }: { page: Page }, testInfo) => {
    await page.goto('/play');
    await snap(page, testInfo, 'play-page-loaded');
  ...
  test('add items modal content is scrollable', async ({ page }: { page: Page }, testInfo) => {
    await page.goto('/play');
    await snap(page, testInfo, 'play-page-loaded');
  ...
  test('C64 Ultimate folder selection shows confirm button', async ({ page }: { page: Page }, testInfo) => {
    await page.goto('/play');
    await snap(page, testInfo, 'play-page-loaded');
    ...
    // Find and select a folder checkbox
    const firstFolderRow = page.locator('[data-testid="source-entry-row"]').first();
    const checkbox = firstFolderRow.locator('input[type="checkbox"]').first();
  ...
  test('Play page: C64 Ultimate full flow adds items', async ({ page }: { page: Page }, testInfo) => {
    await page.goto('/play');
    ...
    // Navigate to USB2 folder
    const usb2Row = page.getByText('USB2', { exact: false }).locator('..').locator('..').locator('..');
    await usb2Row.getByRole('button', { name: 'Open' }).click();
    await page.waitForTimeout(500);
    await snap(page, testInfo, 'usb2-opened');

    // Select a file or folder
    const firstCheckbox = page.locator('[data-testid="source-entry-row"]').first().locator('input[type="checkbox"]');
    ...
    await expect(page.locator('text=/Playlist|Play queue|Track/i')).toBeVisible();
    await snap(page, testInfo, 'back-to-play-page');
  });

  test('Disks page: C64 Ultimate full flow adds disks', async ({ page }: { page: Page }, testInfo) => {
    ...
    await expect(page.locator('text=/Disk|Collection/i')).toBeVisible();
