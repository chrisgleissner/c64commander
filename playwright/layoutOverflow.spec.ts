...
const expectNoHorizontalOverflow = async (page: Page) => {
  ...
  expect(
    ...
    "Document width overflow detected",
  ).toBeLessThanOrEqual(1);
  expect(
    overflow.bodyOverflow,
    "Body width overflow detected",
  ).toBeLessThanOrEqual(1);
};
...
test.describe("Layout overflow safeguards", () => {
  ...
  test.beforeEach(async ({ page }, testInfo) => {
    disableTraceAssertions(
      testInfo,
      "Layout-only coverage; trace assertions disabled.",
    );
  ...
  layoutTest(
    "disks page handles long names without overflow @layout",
    async ({ page }, testInfo) => {
  ...
  layoutTest(
    "playlist list handles long names without overflow @layout",
    async ({ page }, testInfo) => {
      await page.addInitScript(() => {
        const payload = {
          items: [
  ...
  layoutTest(
    "playlist filter header does not cause overflow @layout",
    async ({ page }, testInfo) => {
      await page.addInitScript(() => {
  ...
  layoutTest(
    "FTP browser handles long names without overflow @layout",
    async ({ page }, testInfo) => {
      ...
      await page
        ...
        .click();
      await clickSourceSelectionButton(
      ...
      await dialog
        .locator('[data-testid="source-entry-row"]', {
          hasText: "Long-Names-For-Overflow-Testing",
        })
        .first()
        .click();
      await dialog
        .locator('[data-testid="source-entry-row"]', {
          hasText:
            "Super-Long-Folder-Name-That-Is-Definitely-Too-Wide-For-Mobile-Viewports-And-Should-Wrap",
        })
        .first()
        .click();
  ...
  layoutTest(
    "diagnostics dialog stays within viewport @layout",
    async ({ page }, testInfo) => {
  ...
  layoutTest(
    "settings page handles long hostnames without overflow @layout",
    async ({ page }, testInfo) => {
      await page.goto("/settings", { waitUntil: "domcontentloaded" });
      ...
      await passwordInput.fill(
        "ultra-long-password-value-that-should-not-force-horizontal-scrolling-on-small-devices",
      );
  ...
  layoutTest(
    "settings logs handle long error messages without overflow @layout",
    async ({ page }, testInfo) => {
        ...
        const payload = [
          {
            ...
            level: "error",
            message:
              "Extremely-long-error-message-with-a-very-long-url-https://example.c64u.local/some/really/long/path/that/should-not-overflow",
            timestamp: new Date().toISOString(),
            details: {
              requestUrl:
                "https://example.c64u.local/some/really/long/path/that/should-not-overflow?with=query&and=parameters",
            },
          },
        ];
  ...
  layoutTest(
    "play dialogs stay within viewport @layout",
    async ({ page }, testInfo) => {
      await page.addInitScript(() => {
        const payload = {
          items: [
  ...
  layoutTest(
    "primary pages avoid horizontal overflow @layout",
    async ({ page }, testInfo) => {
      const pages = [
        { path: "/", label: "home" },
  ...
  layoutTest(
    "disk dialogs stay within viewport @layout",
    async ({ page }, testInfo) => {
      await seedDiskLibrary(page, [
        {
          id: "local:/Extremely/Long/Path/With/Deep/Structure/And-A-Super-Long-Disk-Name-That-Should-Not-Overflow-Device-Width.d64",
  ...
  layoutTest(
    "viewport matrix preserves layout and scrolling @layout",
    async ({ page }, testInfo) => {
      ...
      for (const viewport of viewports) {
        ...
        const diskAddItems = page.getByRole("button", {
          name: /Add disks|Add more disks/i,
        });
        if (
          await diskAddItems.isVisible({ timeout: 2000 }).catch(() => false)
        ) {
          ...
          await snap(
            page,
            testInfo,
            `matrix-disks-add-items-${viewport.label}`,
          );
          await page.keyboard.press("Escape");
        }
  ...
  test("AppBar header has safe-area top padding on all pages @layout", async ({
    ...
    for (const route of routes) {
      ...
      const hasSafeClass = await header.evaluate((el: HTMLElement) =>
        el.classList.contains("pt-safe"),
      );
      expect(hasSafeClass, `header on ${route} should have pt-safe class`).toBe(
        true,
      );
      await snap(
        page,
        testInfo,
        `safe-area-${route.replace("/", "root").replace(/\//g, "-")}`,
      );
    }
  });
});
