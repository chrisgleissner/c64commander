...
test.describe('Disk management', () => {
  ...
  test('DOS status line renders message above raw line without labels @layout', async ({ page }: { page: Page }, testInfo: TestInfo) => {
    ...
    await expect(page.getByTestId('drive-status-message-soft-iec')).toHaveText('DOS MISMATCH');
    ...
    await expect(page.getByTestId('drive-status-message-soft-iec')).toHaveClass(/text-amber-600/);
