...
test.describe("Coverage probes", () => {
  ...
  test.beforeEach(async ({ page }: { page: Page }, testInfo: TestInfo) => {
    disableTraceAssertions(
      testInfo,
      "Coverage-only probe routes; trace assertions disabled.",
    );
  ...
  test("exercises internal helpers for coverage", async ({
    page,
  }: {
    page: Page;
  }) => {
    ...
    await expect(
      page.locator('[data-testid="coverage-probe-page"]'),
    ).toBeVisible();
    await expect(page.getByText("Errors")).toHaveCount(0);
  });

  test("covers primary routes for coverage", async ({
    page,
  }: {
    page: Page;
  }) => {
