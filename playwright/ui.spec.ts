...
const resolveExpectedVersion = () => {
  ...
  try {
    ...
    if (!m) {
      console.warn(`[test] ${versionTsPath} exists but APP_VERSION pattern was not found; falling back to git logic.`);
    } else if (m[1]) {
      return m[1];
    }
  } catch (err) {
    ...
    if (!error || error.code !== "ENOENT") {
      console.warn(`[test] Failed to read ${versionTsPath}: ${(error && error.message) || String(error)}`);
    }
  ...
  if (!tag) {
    // In shallow CI checkouts no tag may be reachable; fall back to package.json
    // version to match the Vite build fallback.
    try {
      const pkgContent = fs.readFileSync(path.resolve("package.json"), "utf-8");
      const pkgVersion = (JSON.parse(pkgContent) as { version?: string }).version;
      return pkgVersion || "";
    } catch {
      return "";
    }
