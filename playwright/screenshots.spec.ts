...
const hasPixelDiffAgainstExisting = async (
  filePath: string,
...
const waitForOverlaysToClear = async (page: Page) => {
  ...
  const openToasts = notificationRegion.locator(
...
const scrollHeadingIntoView = async (
  ...
  const targetY = await locator.evaluate(
    (node, payload) => {
      ...
      const desired =
        rect.top + window.scrollY - payload.offset - payload.extraOffset;
...
const capturePageSections = async (
  page: Page,
  testInfo: TestInfo,
  pageId: string,
) => {
...
const captureLabeledSections = async (
  page: Page,
  testInfo: TestInfo,
  pageId: string,
) => {
    ...
    async ({ page }: { page: Page }, testInfo: TestInfo) => {
      ...
      await page.addInitScript(
        ({ baseUrl }) => {
          ...
          ).__c64uMockServerBaseUrl = baseUrl;
          (
