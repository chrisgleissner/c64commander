...
## [2026-05-05T11:20:00Z] PR247-CONVERGE-001: restored append-only history and fixed RAM snapshot convergence drift

### Classification

`DOC_PLUS_CODE`, `UI_CHANGE`

### Action performed

- Restored the existing `PLANS.md` and `WORKLOG.md` history instead of replacing it, then appended this PR `#247` follow-up record.
- Patched `src/lib/c64api.ts` so machine control endpoints (`reset`, `reboot`, `pause`, `resume`, `poweroff`, `menu_button`) again use the bounded `CONTROL_REQUEST_TIMEOUT_MS`.
- Renamed the scheduled retry log message to match the actual trigger (`scheduled timeout` / abort-driven retry), and kept the structured retry details explicit.
- Cached the response payload preview result once in `src/lib/diagnostics/actionSummaries.ts` to avoid duplicate parsing work inside the conditional spread.
- Updated Playwright fixture seeding in `playwright/uiMocks.ts` and `playwright/visualSeeds.ts` so `ram_snapshots_enabled` is on in environments that intentionally exercise RAM snapshot dialogs and screenshots.
- Added/updated unit assertions in `tests/unit/c64api.test.ts` and `tests/unit/fuzz/fuzzBackendIgnore.test.ts` to lock the timeout and log-message behavior.

### Why it was changed

- Copilot review threads on this branch correctly identified two remaining code issues and two documentation-history regressions.
- The failing Playwright shard and screenshot jobs were both waiting for `home-save-ram` / `home-load-ram`, which are now intentionally hidden unless `ram_snapshots_enabled` is enabled.

### Validation target for this convergence pass

- Focused unit coverage for `c64api` and the touched diagnostics/log assertions.
- Targeted Playwright coverage for RAM snapshot, compact snapshot-manager layout, and screenshot capture of the RAM dialogs.
- Repository validation and CI re-run after the fixes are committed and pushed.
