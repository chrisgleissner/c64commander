...
| Run machine actions          | Quick action cards: Reset, Reboot, Pause/Resume, Menu, Save RAM, Load RAM, Power Cycle, Power Off | Tap                                                            | Sends machine control request or device-aware Telnet action, serializes busy state, shows toast                                                                          | `MachineControls` -> `useHomeActions` / `useTelnetActions` / `useC64MachineControl` -> `c64api` + Telnet transport           |
| Run overflow machine actions | Quick Actions overflow menu                                                                       | Tap                                                            | Exposes secondary Telnet operations such as reboot-without-clearing-RAM and save-REU                                                                                     | `HomePage` -> `MachineControls` overflow -> `useTelnetActions`                                                               |
| Save/load RAM image          | `Save RAM`, `Load RAM`, `Reboot`                                                                  | Tap                                                            | Uses folder/file picker, dumps or restores full RAM image, while the visible reboot action clears RAM before rebooting via Telnet                                        | `MachineControls` -> `useHomeActions` / `useTelnetActions` -> `ramOperations` / `ramDumpStorage` -> memory REST endpoints    |
...
- Machine actions are serialized in `useHomeActions` and `useTelnetActions`; concurrent actions are blocked by local task state and the Telnet in-flight guard, not by a device-side lock.
