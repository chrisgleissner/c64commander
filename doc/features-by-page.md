...
Run machine actions | Quick action cards: Reset, Reboot, Pause/Resume, Menu, Power Off | Tap | Sends machine control request, serializes busy state, shows toast | `MachineControls` -> `useHomeActions` / `useC64MachineControl` -> `c64api` -> `/v1/machine:*`
Save/load RAM image | `Save RAM`, `Load RAM`, `Reboot (Clear RAM)` | Tap | Uses folder/file picker, dumps or restores full RAM image, or clears RAM before reboot | `MachineControls` -> `useHomeActions` -> `ramOperations` / `ramDumpStorage` -> memory REST endpoints
...
Manage home drives | Drive cards | Toggle, select bus/type, tap path, reset | Updates drive config; C64U physical drives can mount remote images; reset refreshes device data | `DriveManager` -> shared config actions / `getC64API().mountDrive` -> config + drive endpoints
Manage printer | Printer card | Toggle, change bus/config, reset | Updates printer emulation settings | `PrinterManager` -> shared config actions -> `/v1/configs/...`
...
- Machine actions are serialized in `useHomeActions`; concurrent actions are blocked by local task state, not by a device-side lock.
