...
- **C64** — the tune plays on the Commodore 64 Ultimate and you listen there.
...
- **This device** — your phone renders the SID itself with a built-in libsidplayfp engine. No C64 needed, and nothing is streamed.
...
Crossfading only works for tunes playing on your phone. Two tunes have to sound at the same moment for one to fade into the other, and the Commodore 64 Ultimate plays a single tune live on its one sound chip — so unless **Listen on** is set to **This device** the setting is greyed out and tunes change cleanly instead.
