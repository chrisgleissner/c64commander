...
If you would rather not power up a C64 at all, turn on the **on-device playback engine** — a single "Play on: **C64** / **This device**" choice that appears on the Play screen for SID tunes. Pick **This device** and the app renders the SID on your phone or tablet itself, with a built-in libsidplayfp engine — no C64, and no network stream, required. A small number of tunes rely on the C64's own ROM routines (RSID files); those always play on the C64, and the app switches back to it automatically with a brief note.
