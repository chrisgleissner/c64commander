...
C64 Commander puts your C64 at your fingertips: its music, its games, its disks and its settings, over your own home network.
...
Three things work together: the device running the app, the connected Ultimate-family device, and the network between them.
...
Now jot down the IP address shown under **Wired Network Setup** or **WI-FI Network Setup**. You will need it if the app cannot find the connected Ultimate-family device by itself.

## First Connection

Next, start C64 Commander. If no saved device is reachable, it scans the local network for supported devices.
...
If it finds nothing, C64 Commander opens **No C64 found** so you can set it up by hand.
...
Settings transfer saves your app settings, feature switches and device-safety tuning to a file you can import on another phone or tablet. Saved devices and passwords are left out, so the file is safe to pass around.
...
A setting you change from the app reaches the connected Ultimate-family device at once: the colors, the video mode, the LED lights and the rest all change as you go.
...
If that happens, hold **RESTORE** while you switch the connected Ultimate-family device on. It starts with its default settings instead of the saved ones, and you have a working machine again. Nothing is erased: your saved values are still there, so you can put the setting right and save again.
...
Preferred path: choose C64U for files already on your C64, and Local for files on the device running the app.
...
1. Try Home's own cards first: CPU & RAM, Ports, Video, Audio, User Interface, Lighting.
...
Your C64 plays all of it, and your phone or tablet plays most of it. This section is about choosing the music yourself. The next one, **SID Radio**, lets the app choose for you.
...
To play music by itself, your phone or tablet needs copies of two programs built into every C64: the **KERNAL** and **BASIC** ROMs. Many tunes call into them; without them, those tunes start and then play nothing.

The ROMs are under copyright and cannot be shipped with an app, so C64 Commander reads them from your own machine. It does this by itself, the first time you play a tune here while the C64 is connected. There is nothing to set up.

The copies stay on your phone or tablet. They are never uploaded, never shared, and never put in a diagnostics report. Read them only from a machine that is yours, or that you have permission to use.

#### Moving around inside a tune

These work for tunes playing on your phone or tablet. On the C64, the buttons only step from one tune to the next.
...
Your phone or tablet plays either its own tune or the sound sent from your C64, never both at once. Whichever you start last wins.
...
Only your phone or tablet can play two tunes at once, so Crossfade is grayed out while the output is set to **C64** or **Both**.
...
Your choices stay on your phone or tablet. They belong to the music itself, not to a file name, so they survive an update to the collection even if the tune moves.
...
You find it just below the Quick Actions on **Home**, as a card that starts closed; tap its header to open it. The card appears only where the machine can stream, so an Ultimate-II cartridge does not offer it. Inside are two switches:
...
If the stream stops reaching the device running the app, because the network drops out or the machine is switched off, Live View tells you so. You never sit in front of a frozen picture wondering.
...
On a phone or tablet with a physical keypad, the same keys can either drive the C64 or move the picture. A colored border around the picture, with a matching label in the corner, tells you which.
...
Three switches in **Settings → Play and Disk** control this, and all three start on. The two native ones take effect on Android only. **Low-latency audio (native)** is that fast path. **Fast video (native assembly)** builds the picture the same way, and reaches the full 50 frames a second of a PAL machine.
...
**Local** changes only the picture in Live View on your phone or tablet. **Remote** changes what the connected Ultimate-family device itself draws, so the television in the room changes too. **Both** does both.

The list begins with **Follow the C64**, the starting choice: Live View paints whatever palette the machine uses, so your phone or tablet and your television match.

Below it are nine bundled palettes (warmer, cooler, monochrome and more), each showing all sixteen colors before you choose. Palettes already installed on the connected Ultimate-family device are listed under **Already on this C64**.
...
Remote Input turns your phone or tablet into a joystick and keyboard for the C64. It is handy when you are across the room from the machine, when no joystick is plugged in, or when you just want to type a command.
...
C64 Commander gives your C64 two disk drives and a Soft IEC drive. Each has its own card on the **Disks** page, a small control panel all of its own.
...
A disk that already lives on the connected Ultimate-family device mounts where it is. A **Local** disk is copied across first. Whatever a program writes to it comes back to your own file when you eject, so your high scores and saved games are safe.
...
A RAM snapshot is a copy of what is in your C64's memory right now, saved onto your phone or tablet so you can put it back later. Think of it as a save button for programs that have none of their own.
...
Snapshots live on your phone or tablet, not on the C64. Each is named after its type, date and time; if something is playing, its title becomes the label. You can add or change a **Comment** later. The app keeps a hundred and drops the oldest when it runs out of room.
...
Beside **Save** on the **Config** card are **Load** from flash, **Reset** to the factory settings, and **Revert**, which puts back the settings the device had when the app first connected to it. The app also keeps its own named **configuration snapshots** on the phone or tablet, apart from the device's flash. Save a setup you like, and load the whole thing back whenever you want it.
...
**Share all** produces a small ZIP file holding the app's logs, traces, errors and recent actions, a health snapshot, and details of your app version, your phone or tablet, and the active C64: its name, host address and firmware. Your network password is never in it. Its hostname or IP address may be, so send it only to people you trust, or to support.

Afterward, use **Clear all** for a clean slate. It asks you to confirm, then shows **Diagnostics cleared**.

## Safe Device Use

C64 Commander talks to the connected Ultimate-family device with ordinary REST, FTP and Telnet requests. Even so, the firmware can stop answering under some network conditions. The app lowers that risk by pacing its requests and telling you when something fails.
...
Enter the network password set on the connected Ultimate-family device. If the saved password stops working, the app asks again.
...
Open Diagnostics if you can, and look at the recent REST, FTP and Telnet activity. If HTTP, FTP and Telnet all refuse to connect while ping still works, switch the connected Ultimate-family device off and on again.
