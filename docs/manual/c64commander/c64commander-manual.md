...
The lower cards cover drives, the printer, the SID mixer, streams, and the **Config** card. That last one holds **Save to flash**, which writes the current settings into flash on the connected Ultimate-family device so they survive a power cycle, along with Load, Reset, Revert, and the app's own named configuration snapshots.
