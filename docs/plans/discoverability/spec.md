...
- A **deterministic unit gate** asserts the *work*, not the time: scoring a synthetic 2,000-entry
...
*SID Radio*, "dark col" finds the Appearance rows.
...
"radio" offers *Start SID Radio* before a tune with "radio" in its title), plus a bonus for entries
...
- Any test or automation that addresses a section *header* by testid keeps working. Any that addresses
  a *control inside* one must first open the section — which the video walk already does
...
```ts
// c64u_tour_state:v1
{ completedAt: number | null; skippedAt: number | null; lastStepId: string | null; deviceStepsPending: boolean }
```
...
There is no wildcard and no placeholder that would later become the right value. Binding a *guessed*
...
`useAppStyle` reads the raw id (`useAppStyle.ts:19`) and *removes* an unknown one after falling back
...
| Ranking | Table-driven: a fixed index, a list of queries, the expected top three. Includes "rad" → *Start SID Radio* above any tune containing "radio". |
