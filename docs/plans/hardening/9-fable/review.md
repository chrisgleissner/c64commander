...
| HARD9-013 | HVSC ingest promise never settles after cancel (reject in cancelled coroutine) | native | P1 | correctness, ux, robustness | high [verified] | S | FIXED (74eab854) |
| HARD9-014 | Stale "update applied" records survive HVSC reset, block updates forever | hvsc | P1 | correctness, data-loss, ux | high [verified] | S | FIXED (e4ad9eb2) |
| HARD9-015 | Poisoned empty browse snapshot → recursive HVSC add returns zero songs | hvsc | P1 | correctness, ux, robustness | medium | M | FIXED (7008d654) |
| HARD9-016 | Latest-intent write lane drops committed writes to a different config item | config | P1 | data-loss, correctness, ux | high | M | FIXED (ed0d8d3f) |
| HARD9-017 | Profile load/revert/flash-load never invalidate `c64-config-items` | config | P1 | correctness, ux | high [verified] | S | FIXED (d6fe352b) |
| HARD9-018 | Save-to-App / revert baseline / verification read a stale persistent cache | config | P1 | correctness, data-loss | high | M | FIXED (f579fc56) |
...
| HARD9-040 | Failed HVSC baseline promotion can delete the only library copy | native | P2 | data-loss, correctness, robustness | high | S | FIXED (d8408eda) |
...
| HARD9-053 | Profile load/revert sends entire config as one giant POST /v1/configs | config | P2 | robustness, perf | medium | M | FIXED (a666ff76) |
| HARD9-054 | Audio Mixer solo routing bypasses mutation layer; stale snapshot restore | config | P2 | correctness, robustness | medium | M | FIXED (83b68c58) |
...
- **Area:** shell · **Severity:** P1 · **Dimensions:** ux-responsiveness, a11y, correctness · **Confidence:** high **[verified][HIL]** · **Effort:** M · **Status:** FIXED (f5570328)
...
- **Resolution (f5570328):** `normalizeAndroidInsets` now converts and passes through the real bottom inset the same way it already does for top/right/left, instead of hardcoding `bottom: 0`. `SafeAreaPlugin.kt` already reported the correct `systemBars() or displayCutout()` bottom inset; only the TS-side zeroing was the defect. The existing `--native-safe-area-inset-bottom` → `--safe-area-inset-bottom` → `--app-tab-bar-safe-area-bottom` CSS chain (already consumed by the tab bar frame and every bottom-padded sheet/dialog/toast) required no changes. Two `safeArea.test.ts` assertions that pinned `bottom` to always be 0 encoded the bug and were updated to assert the converted value flows through.
...
- **Area:** playback · **Severity:** P1 · **Dimensions:** correctness, data-loss · **Confidence:** high **[verified]** · **Effort:** M · **Status:** FIXED (76cb69da)
...
- **Area:** playback · **Severity:** P1 · **Dimensions:** correctness, data-loss, ux-responsiveness · **Confidence:** high · **Effort:** M · **Status:** FIXED (9fdc7ca9)
...
- **Resolution (9fdc7ca9):** Shuffle is now a non-destructive playback-order layer. `usePlaylistManager` holds a `shuffleSeed` (lazily generated on first enable; Reshuffle draws a new seed and no longer touches the playlist array). New pure `resolveNextPlaylistIndex`/`resolvePreviousPlaylistIndex` helpers compute a deterministic seeded shuffle order over the current item ids and walk it forward/backward, falling back to the exact prior linear-traversal contract when shuffle is off. `handleNext`, `handlePrevious`, and `flushPendingUserSkip`'s mid-race recompute all go through these helpers. The seed is persisted into the already-existing (previously always-null) `PlaylistSessionRecord.randomSeed` field, giving it its intended purpose without the heavier async `RandomPlaySession`/`createSession`/`next` repository CRUD machinery (which would add IndexedDB round-trip latency to every skip and isn't required to fix either observable bug). `reshufflePlaylist`/`handleReshufflePlaylist` (the destructive array-reorder helper) were removed along with their now-obsolete tests, which encoded the destructive-reorder behavior.
...
- **Area:** disks · **Severity:** P1 · **Dimensions:** data-loss, ux-responsiveness, correctness · **Confidence:** high · **Effort:** M · **Status:** PARTIAL FIX (63347046)
...
- **Resolution (63347046) — PARTIAL, not the full fix:** Implemented only the "at minimum" half of the fix sketch. Added `DiskEntry.sourceKind: "commoserve" | null`, set at the CommoServe import site, and checked in `resolveLocalDiskBlob`'s final fallback to throw "This disk's data is no longer available. Re-import it from CommoServe to mount it again." instead of the generic local-folder message. The underlying data loss (bytes are still never persisted, so the disk genuinely cannot be mounted until manually re-imported) is **not fixed** — that requires either persisting archive-imported bytes (IndexedDB/Filesystem) or re-downloading on demand via the stored `sourceId`+selection, both a materially larger change (new persistence/network-retry infrastructure) than a same-session hardening fix should take on without explicit product sign-off on the UX (e.g. should a stale re-download silently replace the entry, prompt the user, or require an explicit re-add action?). Left the byte-persistence half of this finding intentionally OPEN; a real fix requires a dedicated design/implementation pass, not a rushed addition here.
...
- **Area:** native · **Severity:** P1 · **Dimensions:** correctness, ux-responsiveness, robustness · **Confidence:** high **[verified]** · **Effort:** S · **Status:** FIXED (74eab854)
...
- **Resolution (74eab854):** Implemented the fix sketch's primary option exactly - both the `CancellationException` catch and the generic `Exception` catch now wrap `call.reject(...)` in `withContext(NonCancellable + Dispatchers.Main)`. `handleOnDestroy`'s `activeJob?.cancel(...)` doesn't itself call `call.reject` (it just cancels the job; the running coroutine's own catch block does the rejecting), so it needed no separate change - it now benefits from the same fixed catch blocks. Validated via `./gradlew :app:testDebugUnitTest` plus two new isolated coroutine-semantics tests proving the exact mechanism (a cancelled Job skips a bare `withContext(Dispatchers.Main)` block but still runs one wrapped in `NonCancellable`); the real `ingestHvsc()` cancellation race itself isn't reproduced deterministically, since that would need either a `kotlinx-coroutines-test` virtual-time harness or a production test seam - both larger than this fix warrants.

### HARD9-014 — Stale "update applied" records survive HVSC library reset and permanently block updates
- **Area:** hvsc · **Severity:** P1 · **Dimensions:** correctness, data-loss, ux-responsiveness · **Confidence:** high **[verified]** · **Effort:** S · **Status:** FIXED (e4ad9eb2)
...
- **Resolution (e4ad9eb2):** Implemented the fix sketch exactly - `updates: {}` added to `resetHvscLibraryData`'s patch, and to `applyIngestionSuccess`'s patch when `plan.type === "baseline"` (covering a direct baseline reinstall too, not just the explicit-reset path). Discovered `resetHvscLibraryData` had zero prior test coverage, and the test file's `hvscFilesystem`/`hvscStatusStore` mocks were missing `resetHvscCache`/`getDefaultHvscStatusSummary` entirely (would have thrown `TypeError: ... is not a function` the moment any test called it) - completed those mocks to make the new regression tests possible.

### HARD9-015 — Poisoned empty browse snapshot makes recursive HVSC "add folder" silently return zero songs
- **Area:** hvsc · **Severity:** P1 · **Dimensions:** correctness, ux-responsiveness, robustness · **Confidence:** medium · **Effort:** M · **Status:** FIXED (7008d654)
...
- **Resolution (7008d654):** `listSongsRecursiveFromBrowseIndex` and `streamSongsRecursiveFromBrowseIndex` (the latter also used by `addFileSelections.ts`'s "add all recursively" playlist flow, same bug class) now return `null` when `Object.keys(snapshot.songs).length === 0`, in addition to the pre-existing "folder path missing" check — closing the loophole where root always exists even in a wholly-empty snapshot. `HvscMediaIndexAdapter.queryFolderPage` no longer caches its fallback rebuild into `this.browseSnapshot` when the rebuild saw zero entries, so a snapshot that has not loaded yet is retried on the next call instead of being poisoned permanently. `hvscSourceAdapter.ts`'s `if (bulkSongs)` truthiness check needed no change — it already correctly falls back to BFS on `null` and now only sees `[]` for a genuinely-empty query within a populated library. New tests (`tests/unit/hvsc/hvscBrowseIndexStore.test.ts`, `tests/unit/hvsc/hvscMediaIndex.test.ts`) confirmed failing against pre-fix code via `git stash`, passing with the fix; `npx vitest run tests/unit/hvsc` (500 tests) passes.

### HARD9-016 — Latest-intent write lane silently drops committed writes to a different config item
- **Area:** config · **Severity:** P1 · **Dimensions:** data-loss, correctness, ux-responsiveness · **Confidence:** high · **Effort:** M · **Status:** FIXED (ed0d8d3f)
...
- **Resolution (ed0d8d3f):** Added an optional `merge` function to `createLatestIntentWriteLane` (default: replace, preserving the lane's plain string/number/`PlaybackMixerWrite` use elsewhere) and wired `useInteractiveConfigWrite` to merge per-item updates via object spread. Merging only where `schedule()` sets a still-queued `latestJob` was not sufficient by itself: `process()` pulls the very first job out of `latestJob` synchronously (before its own first `await`), so a second `schedule()` call in the same tick never sees a job to merge into - the actual drop happens later, when a newer job supersedes one already waiting in `beforeRun()` (or one whose `run()` just failed). Added the same merge at both of those points too. Documented the pre-existing volume/pan preview-mode asymmetry (volume `commitOnly`, pan defaults `throttled`) as intentional via a comment at the `AudioMixer.tsx` call site, since both now safely share the merging lane regardless of write frequency. New tests proven failing against pre-fix code via `git stash`; targeted suites (1100 tests) and full `npm test -- run` (658 files / 7812 tests) pass.

### HARD9-017 — Profile load / revert / load-from-flash never invalidate `c64-config-items` — Home shows pre-load values
- **Area:** config · **Severity:** P1 · **Dimensions:** correctness, ux-responsiveness · **Confidence:** high **[verified]** · **Effort:** S · **Status:** FIXED (d6fe352b)
...
- **Resolution (d6fe352b):** Added `c64-config-items`/`c64-config-item` invalidation to all three gaps: `applyConfigData` (blanket, since a profile load/revert spans every category in the snapshot) and `useC64SetConfig`/`useC64UpdateConfigBatch`'s `onSuccess` (category-scoped, matching their existing `c64-category` invalidation). Deliberately left `useC64UpdateConfigBatch`'s `skipInvalidation` branch (the interactive slider write lane) untouched - it already has its own debounced reconciliation invalidation, and adding this there would defeat its purpose of avoiding a refetch storm during drags. New tests proven failing against pre-fix code via `git stash`; targeted suites (962 tests) and full `npm test -- run` (658 files / 7815 tests) pass.
...
- **Area:** config · **Severity:** P1 · **Dimensions:** correctness, data-loss · **Confidence:** high · **Effort:** M · **Status:** FIXED (f579fc56)
...
- **Resolution (f579fc56):** All four of `fetchAllConfig`'s call sites (idle initial snapshot, manual initial snapshot, Save-to-App, revert verification) need a ground-truth read, none want cache-serving behavior - so instead of threading a `fresh` flag through every call site, removed the `getCachedCategory` shortcut from `readCategorySnapshot` entirely and deleted the now-fully-unused `getCachedCategory` method from `C64API` (it had no other callers). `getCachedConfigItem` (a distinct, still-used cache for static option *metadata* only - its own docstring already states the device-fresh value comes from the category read) is untouched, matching "keep the cache for background enrichment only." Tests that exercised the removed method or encoded the old stale-cache-reuse as expected behavior were updated/removed; a new test proves every category is read fresh regardless of what is cached. New/updated tests proven failing against pre-fix code via `git stash`; targeted suites (1042 tests) and full `npm test -- run` (658 files / 7814 tests) pass.
...
- **Area:** playback · **Severity:** P2 · **Dimensions:** data-loss, robustness · **Confidence:** medium · **Effort:** M · **Status:** FIXED (9c58f95c)
...
- **Resolution (9c58f95c):** `commitPlaylistSnapshot` now chains each commit's actual write onto a per-playlistId tail promise, so writes for the same playlistId always execute in strict request order (a prior failure doesn't break the chain). The "keep only the latest pending intent" coalescing optimization was intentionally not implemented - strict ordering alone already eliminates the stale-overwrite data-loss bug, and coalescing is a performance nicety, not a correctness requirement; adding it would have meant more complex cancellation/multi-resolver semantics for no correctness gain. Separately, `replacePlaylistSnapshot`/`replacePlaylistItems` in the IndexedDB repository now read the previous playlist-position order before writing and delete `playlist-item:*` keys for ids no longer present. `track:*` records are deliberately left alone since a trackId can legitimately be shared across playlistIds (e.g. the same file added from more than one device) - deleting them on a single playlist's replace would risk breaking hydration for another playlist still referencing the same track; that would need cross-playlist reference counting, out of scope for this fix. `localStorageRepository` already replaces its array wholesale and has no equivalent orphan-key issue.
...
- **Area:** snapshot · **Severity:** P2 · **Dimensions:** correctness, robustness, ux-responsiveness · **Confidence:** high · **Effort:** S · **Status:** FIXED (60775949)
...
- **Area:** native · **Severity:** P2 · **Dimensions:** data-loss, correctness, robustness · **Confidence:** high · **Effort:** S · **Status:** FIXED (d8408eda)
...
- **Resolution (d8408eda):** Extracted the promotion sequence into `promoteBaselineLibrary`: directory swap now runs first, and the DB rewrite only commits once the swapped files are in place; if the DB write itself then fails, the swap is rolled back best-effort (staging content moved back to the staging path, old library restored) so the DB - never committed in that case - keeps matching what is actually on disk. Extracted the "only delete `oldRoot` once `libraryRoot` is confirmed present" guard into `cleanupHvscOldRootIfLibraryPresent`, reused at all three sites that previously deleted `oldRoot` unconditionally: the startup stale-artifact cleanup (which would otherwise wipe a preserved backup on the very next retry) and both `ingestHvsc` failure/cancellation catch blocks. New reflection-based tests exercise the happy path, first-rename failure (original library untouched), second-rename failure with successful recovery (DB left correctly untouched/consistent), and the guard's core invariant directly (`oldRoot` survives when `libraryRoot` is missing, is cleaned up once it exists). All proven failing against pre-fix code via `git stash`; `./gradlew testDebugUnitTest` (full suite) passes.
...
- **Area:** sources · **Severity:** P2 · **Dimensions:** correctness, ux-responsiveness · **Confidence:** medium · **Effort:** M · **Status:** FIXED (857fd280)
...
- **Resolution (857fd280):** Took the fix sketch's second option (`runtimeFilesBySource.has(source.id)`) but applied it as a new `requireLocalSourceRuntimeFiles` throw at the actual listing entry points (`buildFileList`, `listFilesRecursive`'s local branch) rather than folding it into the shared `requireLocalSourceEntries`/`resolveRootPath` metadata path - `resolveRootPath` runs eagerly at `SourceLocation` construction (e.g. just to read `isAvailable`) and must not throw merely from being unable to browse. Reuses the existing "must be re-added" `LocalSourceListingError` pattern already established for Android SAF handle loss (new `local-runtime-files-missing` code). Also fixed the previously-unconsumed-but-still-wrong `isAvailable` computation for consistency. Several existing tests built a `SourceLocation` without ever registering runtime files (unrealistic - production always registers them immediately after building entries) and needed `setLocalSourceRuntimeFiles` added to keep exercising their intended happy path; new tests proven failing against the pre-fix code via `git stash`.
...
- **Area:** config · **Severity:** P2 · **Dimensions:** robustness, performance · **Confidence:** medium · **Effort:** M · **Status:** FIXED (a666ff76)
...
- **Resolution (a666ff76):** Chose the "N-item batches spaced by the existing write throttle" option as the smallest, lowest-risk fix - it directly bounds POST body size (the documented root cause) without needing a diff-against-device pre-read. Entries destined for the merged (non-sequential) payload are chunked into groups of at most 20 items across however many categories, each sent as its own POST through the existing `scheduleConfigWrite` queue, which already serializes and throttles every write - chunking gets safe spacing for free, no new plumbing needed. Small multi-item writes (a handful of items) stay under the cap and are unaffected; the existing Turbo-Control/CPU-speed sequential-write exception is unaffected (it is extracted before chunking, same as before). New test proven failing against pre-fix code via `git stash`; confirms every item still gets written and no chunk exceeds the cap.
...
- **Area:** config · **Severity:** P2 · **Dimensions:** correctness, robustness · **Confidence:** medium · **Effort:** M · **Status:** FIXED (83b68c58)
...
- **Resolution (83b68c58):** Routed solo/unsolo writes through the existing `useC64UpdateConfigBatch` mutation (already used elsewhere in this component) instead of a direct `getC64API()` call, getting the same invalidation/hasChanges as every other config write for free. Added a timestamp to the stored snapshot and a shared `readFreshSoloSnapshot` helper that discards (and logs) a snapshot older than 5 minutes instead of restoring it; both the mount-time restore effect and the close/unmount restore path now go through it. Part (3) was already resolved by HARD9-053's chunking fix. New/modified tests proven failing against pre-fix code via `git stash` (two existing tests also had to be updated - they had encoded the pre-fix direct-API-call as expected behavior).
...
- **Area:** native · **Severity:** P3 · **Dimensions:** performance, ux-responsiveness · **Confidence:** high · **Effort:** S · **Status:** FIXED (3a1f6469)
...
- **Resolution (3a1f6469):** Moved the query onto `scope`'s existing `Dispatchers.IO` (the same coroutine scope `ingestHvsc` already uses), resolving/rejecting via `withContext(NonCancellable + Dispatchers.Main)` (same HARD9-013 reasoning: `handleOnDestroy()` cancels this scope). Scope note: `HvscIngestion.queryAllSongs` currently has no caller anywhere in `src/` - it is exposed, unused native API surface, not a currently-reachable production path. Fixed anyway since it is a live `@PluginMethod` any future/other JS code could invoke, and the fix is fully self-contained (no JS-side changes). Did not add the fix sketch's optional limit/offset paging: with zero current callers there is no real consumer to validate paging semantics against, and speculative unused paging plumbing would be exactly the kind of unnecessary abstraction the mission's constraints ask to avoid; the payload-size half of the finding is left for a follow-up once/if a real caller exists. New test proves the call returns before `resolve()` runs and that it resolves once pumped; fails against the pre-fix synchronous code (`git stash`). `./gradlew testDebugUnitTest` (full suite) passes.
