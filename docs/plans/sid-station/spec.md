...
- **§4–§6** — the principles, UX, and architecture that constrain *how*.
...
| **Engine** | *Where a SID plays*: **C64** (Ultimate, heard via the audio mirror) or **Local** (on-device WASM, §12). Orthogonal to stations. |
...
| `c64u_local_playback_enabled` | `localPlaybackEnabled` | Gates *availability* of the Local engine toggle (§12) | **off → on** |
...
> The flag `localPlaybackEnabled` decides whether the engine *toggle is shown at all*;
> `c64u_playback_engine` records the user's *choice*. Never conflate them.
...
generic "fast-paced" firehose — it is *your* fast-paced, biased toward what you've liked.

| Named entry | = Seed × Style | User intent |
| --- | --- | --- |
| **Song Radio** | Song × (optional style) | "Keep playing tunes like *this one*." |
| **Style Radio** | Broad **or** Likes × one style | "I'm in a *fast-paced* mood." |
| **Taste Radio** | Likes × (optional style) | "Play me more of what *I* like." |
...
apply), or un-like. It is *not* a radio; it is also the natural seed pool for Taste
stations.
...
2. **User rankings (new, tiny, local).** A **Like ♥ / Not-for-me ✕** affordance the user
   can tap while a tune plays. These are *ambient* — no setup, no modal, no required
...
**song station** ("Radio based on *Commando*"), a **taste station** ("Radio from tunes you
...
1. **Reverse index (built once at load, off-thread):** neighbour edges are a DAG
   pointing to *smaller* ordinals, so walking similarity *both* ways needs inversion. The
...
- **Fresh variety by default** — each station start draws a *random* `shuffleSeed`, so the
...
1. **`md5PathIndex` rides the same finalize hook.** Because it is a projection of
   `Songlengths.md5`, the `md5_48 → virtualPath[]` index rebuilds at the *exact* point the
...
**Two version lines, both surfaced.** The Settings bundle-status line shows *installed
HVSC version* (baseline+update) **and** *sidcorr corpus snapshot* so the (rare) skew is
...
4. **User is always in control.** Start/stop from the transport; ✕ skips *now*;
...
5. **Off the main thread.** Bundle parse + BFS run in a **Web Worker**. We have been
   ...
   *service* worker). M0 must therefore prove the vite-worker → Capacitor-WebView path
...
9. **Radio is lean-back; don't overload "Shuffle."** A station is an endless algorithmic
   ...
   *internal* property (pure function of `(seed, rankingSnapshot, shuffleSeed)`) powering
...
2. **The SID Radio launcher** (`sid-radio-launcher`). A compact entry in the **Play page
   ...
   - **A "based on my likes" toggle on the sheet (Q4).** With it on, tapping any style
     tile launches the **composed** station ("Fast-Paced *from your Likes*") — a style
     filter over a Likes seed. Off → a broad style station. Likes still *steer* either way.
...
- ♥ / ✕ steer live; ✕ also skips. (Steer affects *future* refills, not the visible
  lookahead — Q2.)
...
- Tapping the chip expands a one-line **"why this tune"** provenance (Q8): "similar to
  *Commando*" / "matches Fast-Paced" / "a composer you like" — discoverable, never noisy.
...
- The list is **browsable and manageable**: each row can be **un-liked** (removes it from
  `rankingStore`, so it drops out of Liked Tunes *and* stops steering stations); tap to
...
(source `hvsc`) and calls the *existing* `startPlaylist(items, 0)` / appends via
...
`playlistRepository` session record. The full scored queue is *not* stored.
...
etc.). All keys/defaults are the canon in **§0.4** — this section only says *where*
they surface:
...
- Surfaced in `SettingsPage.tsx` under a **"SID Radio"** group, with a **"Clear my
  ...
  schema version / size / sha-pin match **and** *installed HVSC version* vs *sidcorr
  corpus snapshot*, so any skew is visible.
...
**(c) Exit gate** — a *test-shaped* condition, cross-referenced to §0.3. No milestone is
...
- A **golden smoke test** (opt-in, `SIDCORR_REAL=1`) that parses the *real* pinned
...
(emitted tunes carry the style bit *and* skew toward liked neighbourhoods); Taste unlocks
...
Unit tests prove the engine *logic*; only hardware proves the two things that actually
...
Pixel 4 → C64U, and we pin the committed threshold *above* the measured value with
...
| D7 | Shuffle/Repeat for a station (Q10) | **Station exposes no shuffle control** (lean-back, Pandora/Spotify norm); transport Shuffle/Repeat **disabled while a station drives**, normal for finite lists (playlists, Liked Tunes). Variety is intrinsic (fresh `shuffleSeed`/start). Determinism is *internal* (resume + tests), keyed on `(seed, rankingSnapshot, shuffleSeed)`. | a user-facing "Familiar mix" station option is ever wanted → add as a station-scoped control, never as transport "Shuffle". |
| D8 | Steer (♥/✕) timing (Q2) | **Future-refill only**; ✕ also skips *now*, visible lookahead stays put | responsiveness testing shows users expect the queue to re-rank live. |
...
optional *sink upgrade* later (§12.2, D6), not a separate engine.
...
- libsidplayfp is **GPL-2.0-or-later** (headers/README: *"either version 2 … or (at
  your option) any later version."*). GPL-2.0-**or-later** is **compatible with
...
- **iOS/App-Store note (not Android):** GPL × Apple App Store has historical friction
  (true for *any* GPL code on iOS, WASM or native). It does **not** gate the primary
...
- **v1 = chunked pre-render (recommended, D6 default).** The worker renders N-second
  ...
  **No `SharedArrayBuffer`, no AudioWorklet, no COOP/COEP headers** — the single biggest
  portability win; works in any WebView incl. SailfishOS's and the low-end Callback
  8020. Songlength drives duration/auto-advance exactly as today.
...
- **Layered sink = native escape hatch.** For robust background/screen-off audio, swap
  *only the output sink* for a native `AudioTrack` fed by the same WASM worker —
...
- **LE0 — Licence & feasibility spike.** *Test-first:* a licence-audit checklist
...
- **LE1 — Local engine + chunked Web Audio sink.** *Test-first:* `localSidEngine.test.ts`
...
- **LE2 — Engine toggle wired into playback.** *Test-first:* route-selection test
...
- **Device-launch seam (§12):** `src/lib/playback/playbackRouter.ts` (`executePlayPlan`
  — verified; the single point the Local engine routes *around*),
...
- **No Web Worker precedent** exists in `src/` today (only the PWA *service* worker in
