...
- [~] LE2 **engine app-layer DONE + green** — `playbackEngineRouting.ts` (pure route/notice decision: SID+local→local; non-SID/ROM/unsupported→C64 + one-time notice), `localSidPlaybackController.ts` (lifecycle: reads the `LocalPlayFile` bytes → engine, propagates ROM-required for fallback), `c64u_local_engine_enabled` **rollout gate** (default off — playback byte-for-byte unchanged) + `usePlaybackEngine` hook + `PlaybackEngineToggle` (`playback-engine-c64`/`playback-engine-local` testids). 22 new tests. **Remaining (on-device L3):** wire the route into `usePlaybackController.playItem` + `handleStop` + the Play-page/Settings surface + foreground wake lock — the spec's "clean instant switch" is device-validated, so it lands with the L2/L3 hardware pass → **L3**
