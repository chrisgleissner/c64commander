...
| Settings             | `SettingsPage`           | `/settings`                      | Connection, diagnostics, app behavior, archive settings, and device-safety tuning                                |
...
| Enter Game Mode _(flag `remote_input_enabled`)_      | Quick action card **Game Mode** (`home-machine-inline-openGameMode`), first in the grid                                       | Tap                                                            | Starts the remembered Watch/Listen feeds and opens the Remote Input sheet in Game Mode. Disabled while disconnected | `MachineControls` -> `HomePage` -> `startGameMode()` -> `avMirrorSession` + the Game Mode request bus -> `RemoteInputSheet` |
| Run machine actions                                 | Quick action cards, in ring order: Game Mode, Menu, Pause/Resume, Remote Input, Save REU, Save RAM, Load RAM, Reset, Reboot, Reboot (Clr Mem), Power Cycle, Power Off | Tap                                                            | Uses REST for primary reboot and shared busy-state orchestration for the remaining machine actions. All extra Telnet-only actions render inline as quick action cards (no overflow menu). Power Cycle and Reboot (Clr Mem) are Telnet-only, hidden by default, and gated by `home_telnet_power_cycle_enabled` and `home_telnet_clear_ram_reboot_enabled` respectively                             | `MachineControls` -> `HomePage` -> `deviceControl` / `useC64MachineControl` / `useTelnetActions` -> `c64api` + Telnet                                                                                      |
| Save/load RAM and REU images                        | `Save RAM`, `Load RAM`, `Save REU`                                                                                            | Tap                                                            | Uses folder-backed RAM snapshots plus filesystem-backed REU snapshots. `Load RAM` now includes REU entries with `Load into REU` and `Preload on Startup` actions                                                                                                                                                                                                                                  | `MachineControls` -> `ramOperations` / snapshot store / REU workflow -> memory REST endpoints + FTP + Telnet                                                                                               |
...
| Manage home drives                                  | Drive cards                                                                                                                   | Toggle, select bus/type, tap path, reset                       | Updates drive config and mounts via REST. The Telnet shortcut footer (drive A reset, drive B turn on, Soft IEC turn on/reset/set dir) is hidden by default and gated by `home_telnet_drive_actions_enabled`                                                                                                                                                                                       | `DriveManager` -> shared config actions / `getC64API().mountDrive` / `useTelnetActions` -> config + drive endpoints + Telnet                                                                               |
| Manage printer                                      | Printer card                                                                                                                  | Toggle, change bus/config, reset                               | Updates printer emulation settings via REST. The Telnet shortcut footer (turn on, flush/eject, reset) is hidden by default and gated by `home_telnet_printer_actions_enabled`                                                                                                                                                                                                                     | `PrinterManager` -> shared config actions / `useTelnetActions` -> `/v1/configs/...` + Telnet                                                                                                               |
...
| Manage streams                                      | Stream rows                                                                                                                   | Start/Stop/Edit endpoint                                       | Starts or stops configured UDP streams; validates endpoint edits. **Live View precedence:** while the shared A/V mirror session is receiving the VIC (video) or Audio feed, that row goes read-only with a "Live View" badge + note (`home-stream-liveview-badge/note-*`); the user's target resumes control when Live View stops.                                                                | `StreamStatus` -> `useStreamData` + `useAvMirror` -> stream config writes + `/v1/streams/*`                                                                                                                |
| Mirror device audio _(flag `audio_mirror_enabled`)_ | **Live View** card Audio toggle (`av-audio-toggle`) beneath the quick actions                                                 | Tap `Listen` / `Listening`                                     | Receives the device UDP audio stream and plays it in-app via the single app-wide A/V mirror session (a live dot signals it is running). User-visible and non-developer, **off by default** until the phone stream receiver ships; only mounted when `deviceCapabilities.supportsStreaming` is true                                                                                                | `HomePage` -> `LiveViewCard` -> `AvMirrorControls` -> `useAvMirror(avMirrorSession)` -> `AudioMirrorController` (StreamReceiver + AudioMirrorPlayer) -> stream start/stop (`/v1/streams/*`)                |
| Mirror device video _(flag `video_mirror_enabled`)_ | **Live View** card Video toggle (`av-video-toggle`) + collapsible check preview (`live-view-expand`)                          | Tap `Watch` / `Watching`; expand for a larger preview          | Decodes the device VIC UDP stream (shared session) and renders it to a 384×272 canvas (GPU integer-scaled, `image-rendering: pixelated`), showing fps. The full zoom/pan immersive screen lives in Remote Input game mode (see below). User-visible and non-developer, **off by default** until the phone stream receiver ships; only mounted when `deviceCapabilities.supportsStreaming` is true | `HomePage` -> `LiveViewCard` -> `AvMirrorControls`/`AvMirrorPreview` -> `useAvMirror`/`useAvMirrorCanvas` -> `VideoMirrorController` (StreamReceiver + `vicDecode`) -> stream start/stop (`/v1/streams/*`) |
| Manage device/app configs                           | Config action cards                                                                                                           | Save/load/reset flash; save/load/revert/manage app configs     | Flash save/load/reset and app config save/load/revert/manage are always available. The advanced Telnet-backed `Save (file)`, `Load (file)`, and `Clear Flash` cards are hidden by default and gated by `home_telnet_config_actions_enabled`                                                                                                                                                       | `useC64MachineControl` + `useAppConfigState` -> flash endpoints + app config store                                                                                                                         |
...
- Home machine actions now centralize Menu, Reboot, Reboot (Clr Mem), and Power Cycle in `src/lib/deviceControl/deviceControl.ts`; primary reboot stays REST-only, power cycle stays Telnet-only, and concurrent actions are blocked by page task state plus the Telnet in-flight guard for remaining Telnet-only flows.
...
- App config snapshots are local-only. Device flash save/load/reset is separate from app snapshot save/load/manage, and REU snapshots are stored as native files with lightweight metadata in app storage.
...
| RAM dump folder, RAM actions, and REU workflows           | Playwright + Unit | `playwright/homeRamDumpFolder.spec.ts`, `tests/unit/pages/HomePage.ramActions.test.tsx` | Partial  |
...
- RAM save/load moves full-memory images through SAF/native bridges, and REU save/restore adds a staged FTP plus Telnet workflow with long-running progress state.
...
| Select SID tune                 | Tune dialog                    | Open and pick a tune within the SID                                                 | Rebuilds request with selected `songNr` and restarts current item                                        | `PlaybackSettingsPanel` -> `handleSongSelection` -> replay through controller                            |
...
| Game launch         | `usePlaybackController` -> `PlayFilesPage.handleUserLaunchedItem` | A user-initiated `prg`/`crt`/`disk` launch that succeeded, with the sheet closed and the setting on   |
...
| Stick joystick                                                                | Draggable stick zone                                                                                                                     | Pointer down/move/up                                                                           | Computes 8-way direction from drag vector via `resolveDragDirections`; diffs against last-sent held set                                                                                                                                                                            | `VirtualJoystick` -> `resolveDragDirections` -> `session.setHeldJoystickInputs`                                                                                                                                     |
...
| Swipe joystick                                                                | Same zone as Stick, drag-from-touchdown                                                                                                  | Pointer down/move/up                                                                           | Direction computed relative to the touch-DOWN point (not center), unlike Stick                                                                                                                                                                                                     | `SwipePad` -> `session.setHeldJoystickInputs`                                                                                                                                                                       |
...
| Game mode                                                                     | "Game mode" / "Exit" button (Joystick tab only, full tier only)                                                                          | Tap                                                                                            | Edge-anchored, maximized, no-look layout; strips mode toggle, quick-keys bar, size stepper, and Release All/Close footer (HARD13 accessibility - deliberate, tested)                                                                                                               | `RemoteInputSheet` immersive state                                                                                                                                                                                  |
...
| A/V mirror controls _(flags `audio_mirror_enabled` / `video_mirror_enabled`)_ | Audio/Video toggles pinned in the sheet chrome (`remote-input-mirror-controls`, `av-audio-toggle`/`av-video-toggle`)                     | Tap Listen / Watch                                                                             | Hear/see the machine while driving it, sharing the single app-wide A/V mirror session with Home's Live View (no second stream). Shown when a mirror flag is on and the device advertises streaming                                                                                 | `RemoteInputSheet` -> `AvMirrorControls` -> `useAvMirror(avMirrorSession)`                                                                                                                                          |
| Immersive screen mirror _(flag `video_mirror_enabled`)_                       | Maximised zoom/pan surface above the input controls (`av-mirror-immersive`) with zoom/fit/follow/adjust controls and a draggable minimap | Pinch/drag/double-tap on the picture; on-screen ＋/−/⤢/◎ buttons; physical keys in Adjust mode | Full-width, native-aspect mirror for keypad-driven devices (e.g. Callback 8020): GPU-scaled zoom 1×–8×, clamped pan, smart activity-follow, and a colour-coded **view-lock mode** so a physical key is never ambiguously "drive the C64" vs "adjust the view" (06-av-mirror-ux §7) | `RemoteInputSheet` (game mode) -> `AvMirrorImmersive` -> `useMirrorViewport` (`mirrorViewport`/`motionTracker`) + `AvMirrorMinimap`; physical keys route to viewport ops in Adjust mode, else to the existing relay |
...
| Done | Feature                             | UI Element                                | Test Name                                                | Test Steps                                                                         | Test Assertions                                                           |
| ---- | ----------------------------------- | ----------------------------------------- | -------------------------------------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| [x]  | Joystick relay                      | Stick/D-Pad/Swipe/FIRE/autofire           | Real-hardware joystick relay HIL                         | Hold each movement style + FIRE/autofire on real u64                               | Correct press/release payloads, no stuck input after release (2026-07-06) |
...
| Create blank disk _(flag `new_disk_enabled`)_          | `New disk` header button (`new-disk-open`) + New disk dialog        | Tap `New disk`; pick Type/File name/Disk label/Tracks/Storage folder; tap `Create & mount` | Creates a formatted blank D64/D71/D81/DNP image on the device, adds it to the library, and mounts it to drive A. Behind `new_disk_enabled` (experimental, default off). Tracks field appears for D64/DNP only                                                                                         | `HomeDiskManager` -> `NewDiskDialog` -> `buildCreateDiskPlan` (`lib/disks/createDisk`) -> `c64api.createDisk` -> `useDiskLibrary.addDisks` + `handleMountDisk` |
| Explore disk contents _(flag `disk_explorer_enabled`)_ | Per-disk `Open (Disk Explorer)…` menu action + Disk contents dialog | Open action; tap `Run`, `Load`, or `Mount & Load` on an entry                              | Reads the disk directory in-app and launches a single program via DMA (`Run`/`Load`) or mounts the whole image then loads the entry (`Mount & Load`), instead of only mounting the disk. Behind `disk_explorer_enabled` (experimental, default off); action shown only for `.d64/.d71/.d81/.dnp` rows | `HomeDiskManager` -> `useDiskExplorer` -> `listDirectory` (`lib/disks/diskImage`) + `runDiskEntry`/`mountAndLoadEntry` (`lib/playback/diskLaunch`) -> `c64api` |
...
App-level control plane. Configures saved devices, diagnostics, UI theme, preference persistence, feature flags, and device-safety behavior.
...
| Select saved device                                                               | Saved devices list                                                                                                                                                                                                              | Tap device row                   | Switches the selected device immediately from saved metadata, updates the editor fields, and starts `/v1/info` verification                                                                                                                                                                                                                                                 | saved-device store + `useSavedDeviceSwitching`                                                                                                                                                               |
| Add or delete saved device                                                        | Icon-only add / delete buttons in Connection card                                                                                                                                                                               | Tap                              | Creates or removes saved devices while preserving per-device host, ports, and password state, and warns before deleting referenced items                                                                                                                                                                                                                                    | saved-device store + secure storage                                                                                                                                                                          |
| Save selected device                                                              | Device name/host/port/password inputs + `Save & Connect`                                                                                                                                                                        | Edit and tap                     | Persists the selected device definition, restores product-derived automatic names when the name is blank, updates runtime routing, verifies the current target, and shows the result toast                                                                                                                                                                                  | saved-device store + secure storage + `useC64Connection.updateConfig` + `useSavedDeviceSwitching`                                                                                                            |
...
| Open diagnostics                                                                  | `Diagnostics` button or app-bar activity indicator                                                                                                                                                                              | Tap                              | Opens diagnostics dialog on the requested tab for health, current-device details, activity, and secondary actions                                                                                                                                                                                                                                                           | diagnostics overlay store + dialog state                                                                                                                                                                     |
| Quick switch saved device                                                         | App-bar activity indicator                                                                                                                                                                                                      | Long press                       | Opens a compact `Switch device` picker with name-first rows, passive concurrent health checks for every saved device, per-row expandable probe detail, 10s auto-refresh while open, and immediate switching from saved metadata                                                                                                                                             | saved-device store + `useSavedDeviceSwitching` + passive saved-device health polling + badge long-press interaction                                                                                          |
...
| Search inside disk images _(flag `in_image_search_enabled`)_                      | `Search inside disk images` checkbox (`settings-search-inside-disks`) in the Play and Disk card                                                                                                                                 | Toggle                           | Persists whether media search also matches the programs inside `.d64/.d71/.d81` images, not just their filenames. Behind `in_image_search_enabled` (experimental, default off; depends on Disk Explorer)                                                                                                                                                                    | `SettingsPage` -> `saveSearchInsideDisks` (persisted app setting)                                                                                                                                            |
| Tune launch safety / boot menu _(flag `launch_safety_enabled`)_                   | Play and Disk card: `Answer cartridge boot menu after reset` checkbox (`settings-boot-menu-answer`) + `Menu key` select (`settings-boot-menu-key`) + `Boot settle (ms)` input (`settings-boot-settle`, both shown when checked) | Toggle / select / edit           | Persists whether, after a Mount & Load reset, the app presses a key to answer a cartridge boot menu (configurable key + settle delay). The primary Launch Safety behavior — parking the configured cartridge around direct-memory launches so a freezer cartridge can't hijack a Run/Load — is automatic and invisible. Behind `launch_safety_enabled` (stable, default on) | `SettingsPage` -> `saveBootMenuAnswerEnabled` / `saveBootMenuKey` / `saveBootSettleMs`; consumed by `lib/playback/launchSafety.ts` (`withCartridgeParked`, `bootSettle`) via `diskLaunch` / `playbackRouter` |
| Set mirror stream ports _(flags `audio_mirror_enabled` / `video_mirror_enabled`)_ | Play and Disk card: `Video stream port` / `Audio stream port` inputs (`settings-stream-video-port` / `settings-stream-audio-port`)                                                                                              | Edit                             | Persists the UDP ports the device streams Audio/Video Mirror to (defaults 11000 / 11001). Shown when `audio_mirror_enabled` or `video_mirror_enabled`                                                                                                                                                                                                                       | `SettingsPage` -> `saveStreamVideoPort` / `saveStreamAudioPort` (persisted app settings)                                                                                                                     |
...
- Connection settings are now per saved device. Selecting a device projects local metadata into runtime config first, then verifies with `/v1/info`.
...
- Saved-device switching invalidates only the active route's essential queries after verification instead of refetching the full config tree.

#### Existing Test Coverage

| Feature                                         | Test Type                   | Test File                                                                                                                                                                                                                                                                       | Coverage |
| ----------------------------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| Saved-device selection and connection editing   | Unit                        | `tests/unit/pages/SettingsPage.test.tsx`, `tests/unit/components/diagnostics/DiagnosticsDialog.savedDeviceSwitch.test.tsx`                                                                                                                                                      | Partial  |
| Diagnostics dialog/share/clear                  | Playwright + Maestro + Unit | `playwright/settingsDiagnostics.spec.ts`, `playwright/homeDiagnosticsOverlay.spec.ts`, `.maestro/ios-diagnostics-export.yaml`, `tests/unit/components/diagnostics/DiagnosticsDialog.test.tsx`, `tests/unit/components/diagnostics/DiagnosticsDialog.savedDeviceSwitch.test.tsx` | Full     |
...
| [ ]  | Local persistence | Transfer + theme               | Settings persistence controls  | Toggle theme/demo; export/import settings            | Values persist; imports apply expected fields               |
