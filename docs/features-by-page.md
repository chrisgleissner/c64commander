...
| Run overflow machine actions | Quick Actions overflow menu                                                                       | Tap                                                            | Exposes `Reboot (Clr Mem)` with Telnet-first REST fallback and the staged `Save REU` workflow                                                                            | `HomePage` -> `MachineControls` overflow -> `deviceControl` / `useTelnetActions` / REU workflow                              |
