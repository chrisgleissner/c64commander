...
| Open diagnostics                         | `Diagnostics` button or app-bar activity indicator             | Tap                              | Opens diagnostics dialog on the requested tab; from there the saved-device list can switch devices in one additional tap                | diagnostics overlay store + dialog state + `useSavedDeviceSwitching`                                   |
