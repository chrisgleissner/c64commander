...
| Quick switch saved device                | App-bar activity indicator                               | Long press                       | Opens a compact `Switch device` picker with name-first rows, including product-derived auto labels such as `U64` and `U64-2`, then switches immediately from saved metadata and starts `/v1/info` verify | saved-device store + `useSavedDeviceSwitching` + badge long-press interaction                          |
