...
| Add or delete saved device               | `Add` / `Delete` buttons in Connection card                    | Tap                              | Creates or removes saved devices while preserving per-device host, ports, and password state                                            | saved-device store + secure storage                                                                    |
| Save selected device                     | Device name/badge/host/port/password inputs + `Save & Connect` | Edit and tap                     | Persists the selected device definition, updates runtime routing, verifies the current target, and shows the result toast               | saved-device store + secure storage + `useC64Connection.updateConfig` + `useSavedDeviceSwitching`      |
...
| Quick switch saved device                | App-bar activity indicator                                     | Long press                       | Opens a compact `Switch device` picker with name-first rows, then switches immediately from saved metadata and starts `/v1/info` verify | saved-device store + `useSavedDeviceSwitching` + badge long-press interaction                          |
