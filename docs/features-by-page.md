...
| Mirror device audio _(flag `audio_mirror_enabled`)_ | Audio Mirror panel `Listen`/`Stop` toggle (`audio-mirror-toggle`) | Tap `Listen` / `Stop` | Receives the device UDP audio stream and plays it in-app, showing connection state and a dropped-packet count. Behind `audio_mirror_enabled` (experimental, default off) and only mounted when `deviceCapabilities.supportsStreaming` is true | `HomePage` -> `AudioMirrorPanel` -> `useAudioMirror` -> `AudioMirrorController` (StreamReceiver + AudioMirrorPlayer) -> stream start/stop (`/v1/streams/*`) |
| Mirror device video _(flag `video_mirror_enabled`)_ | Video Mirror panel `Watch`/`Stop` toggle (`video-mirror-toggle`) | Tap `Watch` / `Stop` | Decodes the device VIC UDP stream and renders it to a 384×272 canvas (GPU integer-scaled, `image-rendering: pixelated`), showing fps and dropped packets. Behind `video_mirror_enabled` (developer-only, default off) and only mounted when `deviceCapabilities.supportsStreaming` is true; CPU-intensive | `HomePage` -> `VideoMirrorPanel` -> `useVideoMirror` -> `VideoMirrorController` (StreamReceiver + `vicDecode`) -> stream start/stop (`/v1/streams/*`) |
