...
| Run machine actions                                 | Quick action cards: Reset, Reboot, Pause/Resume, Menu, Save RAM, Load RAM, Power Cycle, Reboot (Clr Mem), Save REU, Power Off | Tap                                                            | Uses REST for primary reboot and shared busy-state orchestration for the remaining machine actions. All extra Telnet-only actions render inline as quick action cards (no overflow menu). Power Cycle and Reboot (Clr Mem) are Telnet-only, hidden by default, and gated by `home_telnet_power_cycle_enabled` and `home_telnet_clear_ram_reboot_enabled` respectively                             | `MachineControls` -> `HomePage` -> `deviceControl` / `useC64MachineControl` / `useTelnetActions` -> `c64api` + Telnet                                                                                      |
