...
| Run machine actions          | Quick action cards: Reset, Reboot, Pause/Resume, Menu, Save RAM, Load RAM, Power Cycle, Power Off | Tap                                                            | Uses REST for primary reboot, Telnet for power cycle, and shared busy-state orchestration for the remaining machine actions                                              | `MachineControls` -> `HomePage` -> `deviceControl` / `useC64MachineControl` / `useTelnetActions` -> `c64api` + Telnet        |
| Run overflow machine actions | Quick Actions overflow menu                                                                       | Tap                                                            | Exposes `Reboot (Clear RAM)` with Telnet-first REST fallback and the staged `Save REU` workflow                                                                          | `HomePage` -> `MachineControls` overflow -> `deviceControl` / `useTelnetActions` / REU workflow                              |
...
- Home machine actions now centralize Menu, Reboot, Reboot (Clear RAM), and Power Cycle in `src/lib/deviceControl/deviceControl.ts`; primary reboot stays REST-only, power cycle stays Telnet-only, and concurrent actions are blocked by page task state plus the Telnet in-flight guard for remaining Telnet-only flows.
