...
The local-source fixture set (`tests/fixtures/local-source-assets`) is deterministic and generated by `scripts/generate-local-source-fixtures.mjs`. Regenerate it whenever fixture provenance or format details need to be refreshed.
...
COVERAGE_MIN=94 COVERAGE_MIN_BRANCH=91 node scripts/check-coverage-threshold.mjs
```
