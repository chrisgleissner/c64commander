...
- [x] c64u secondary validation recovered on 2026-05-18: `curl --max-time 5 http://c64u/v1/info` returns `C64 Ultimate` / fw `1.1.0`, and the Pixel 4 60 s Home soak remains `C64U · HEALTHY`. Evidence: `evidence/c64u-probe-connectivity-test.txt`, `evidence/phase1-c64u-home-60s-after-telnet-scheduler.png`, `evidence/phase1-c64u-logcat-after-telnet-scheduler.txt`.
...
- [x] `npm run test:coverage` rerun completed during Phase 6 closeout: 966 tests, 91.75 % branch coverage, 94.20 % line coverage, 90.67 % function coverage (2026-05-18).
...
- [x] c64u cold-launch and warm restart now match the same gate: both launches show non-OFFLINE `C64U · HEALTHY` with Home metadata populated (`Device c64u`, `Firmware 1.1.0`). Evidence: `evidence/phase2-c64u-cold-start-after-info-fallback.txt`, `evidence/phase2-c64u-cold-3s-after-info-fallback.png`, `evidence/phase2-c64u-cold-5s-after-info-fallback.png`, `evidence/phase2-c64u-warm-start-after-info-fallback.txt`, `evidence/phase2-c64u-warm-3s-after-info-fallback.png`, `evidence/phase2-c64u-warm-5s-after-info-fallback.png`.
...
- [x] c64u cold-boot matches the Phase 3 targets with 0 `LED Strip Settings` lines, 0 `Keyboard Lighting` lines, 0 `Msg: undefined`, and 0 `triggerEvent` TypeErrors; the only remaining `CapacitorCookies` line is plugin registration. Evidence: `evidence/phase3-F-HTTP-2-F-LOG-1-c64u-cold-5s-after-config-cache-reuse.png`, `evidence/phase3-F-HTTP-2-F-LOG-1-c64u-cold-logcat-after-config-cache-reuse.txt`.
...
- [x] c64u slider stress also holds the interaction pause: the Home CPU slider moved `40 -> 16` while secondary `u64` probes stayed at 0 after interaction start (`u64AfterStartCount = 0`), and the c64u MimeMap repeat sweep held `maxMimeMs=0` / `maxMimeEvents=0` across three cold launches (`614`, `630`, `606` ms). Evidence: `evidence/phase4-F-RT-1-c64u-home-cpu-slider-30s-request-trace-devtools-final.json`, `evidence/phase4-F-RT-1-c64u-home-cpu-slider-30s-after-polling-pause-devtools-final.png`, `evidence/phase4-F-MIME-1-c64u-cold-repeat-summary-after-mime-prewarm.json`.
...
- [x] `c64u` cross-device sweep completed after the device recovered on 2026-05-18. See the c64u evidence rows below for Phases 1, 2, 3, 4, and 6.
...
| 2026-05-18 14:11 | `android/app/build/outputs/apk/debug/c64commander-0.7.9-rc1-debug.apk` | 653 ms cold | Reused the final installed APK after `c64u` recovered. The saved-device switch completed to `c64u`, cold and warm Home captures showed `Device c64u` / `Firmware 1.1.0`, the 60 s soak stayed `C64U · HEALTHY`, the c64u cold-boot log held Phase 3 targets, the c64u slider stress suppressed secondary `u64` probes, and the Play route rendered on-device. Evidence retained under `evidence/phase1-c64u-*`, `phase2-c64u-*`, `phase3-F-HTTP-2-F-LOG-1-c64u-*`, `phase4-F-RT-1-c64u-*`, `phase4-F-MIME-1-c64u-*`, and `phase6-c64u-*`. |

## u64 + c64u real-device validation log

| Finding | u64 result | c64u result | Evidence |
|---|---|---|---|
| F-DIAG-1/F-DIAG-2/F-DIAG-3 | PASS: `u64` reachable (`Ultimate 64 Elite`, fw `3.14e`), Pixel 4 Home soak remains `U64 · HEALTHY` after 60 s. | PASS: `c64u` reachable (`C64 Ultimate`, fw `1.1.0`), and the Pixel 4 Home soak remains `C64U · HEALTHY` after 60 s. | `evidence/phase1-F-DIAG-1-u64-home-60s-after-telnet-scheduler.png`, `evidence/phase1-F-DIAG-1-u64-logcat-after-telnet-scheduler.txt`, `evidence/c64u-probe-connectivity-test.txt`, `evidence/phase1-c64u-home-60s-after-telnet-scheduler.png`, `evidence/phase1-c64u-logcat-after-telnet-scheduler.txt` |
| F-CONN-1/F-CONN-2/F-CONN-3 | PASS: cold launch and hot restart on Pixel 4 both show non-OFFLINE `U64 · HEALTHY` with Home metadata populated (`Device u64`, `Firmware 3.14e`). | PASS: cold launch and warm restart on Pixel 4 both show non-OFFLINE `C64U · HEALTHY` with Home metadata populated (`Device c64u`, `Firmware 1.1.0`). | `evidence/phase2-F-CONN-1-u64-cold-start-after-info-fallback.txt`, `evidence/phase2-F-CONN-1-u64-cold-3s-after-info-fallback.png`, `evidence/phase2-F-CONN-2-u64-cold-5s-after-info-fallback.png`, `evidence/phase2-F-CONN-2-u64-warm-start-after-info-fallback.txt`, `evidence/phase2-F-CONN-2-u64-warm-3s-after-info-fallback.png`, `evidence/phase2-F-CONN-2-u64-warm-5s-after-info-fallback.png`, `evidence/phase2-c64u-cold-start-after-info-fallback.txt`, `evidence/phase2-c64u-cold-3s-after-info-fallback.png`, `evidence/phase2-c64u-cold-5s-after-info-fallback.png`, `evidence/phase2-c64u-warm-start-after-info-fallback.txt`, `evidence/phase2-c64u-warm-3s-after-info-fallback.png`, `evidence/phase2-c64u-warm-5s-after-info-fallback.png` |
| F-HTTP-1/F-HTTP-2/F-LOG-1/F-LOG-2 | PASS: latest Pixel 4 `u64` cold boot holds Phase 3 targets with 2 `LED Strip Settings` requests, 0 `Keyboard Lighting` requests, 0 `Msg: undefined`, and 0 `triggerEvent` TypeErrors in the 35 s capture; only the expected `CapacitorCookies` plugin-registration line remains. | PASS: latest Pixel 4 `c64u` cold boot holds Phase 3 targets with 0 `LED Strip Settings` lines, 0 `Keyboard Lighting` lines, 0 `Msg: undefined`, and 0 `triggerEvent` TypeErrors; only the expected `CapacitorCookies` plugin-registration line remains. | `evidence/phase3-F-HTTP-2-F-LOG-1-u64-cold-5s-after-config-cache-reuse.png`, `evidence/phase3-F-HTTP-2-F-LOG-1-u64-cold-logcat-after-config-cache-reuse.txt`, `evidence/phase3-F-HTTP-2-F-LOG-1-c64u-cold-5s-after-config-cache-reuse.png`, `evidence/phase3-F-HTTP-2-F-LOG-1-c64u-cold-logcat-after-config-cache-reuse.txt` |
| F-RT-1/F-MIME-1 | PASS: Phase 4 closes on `u64`; a timed 30 s Home CPU-slider drag suppressed all secondary-device background probes after the interaction window started, and four cold-boot samples held `MimeMap` contention at 0 ms / 0 events. | PASS: Phase 4 closes on `c64u`; the timed 30 s Home CPU-slider drag changed the value from `40` to `16` while suppressing all secondary `u64` probes after interaction start (`u64AfterStartCount = 0`), and three c64u cold boots held `MimeMap` contention at 0 ms / 0 events (`614`, `630`, `606` ms). | `evidence/phase4-F-RT-1-u64-home-cpu-slider-30s-request-trace-devtools-final.json`, `evidence/phase4-F-RT-1-u64-home-cpu-slider-30s-after-polling-pause-devtools-final.png`, `evidence/phase4-F-MIME-1-u64-cold-repeat-summary-after-mime-prewarm.json`, `evidence/phase4-F-RT-1-c64u-home-cpu-slider-30s-request-trace-devtools-final.json`, `evidence/phase4-F-RT-1-c64u-home-cpu-slider-30s-after-polling-pause-devtools-final.png`, `evidence/phase4-F-MIME-1-c64u-cold-repeat-summary-after-mime-prewarm.json` |
| H-VOL-1/H-VOL-2/H-PLAY-1/H-RT-2 + Phase 6 closeout | PASS: Phase 5 verification is locked in by targeted regressions, and the final Phase 6 APK install/launch on Pixel 4 foregrounded the app against a reachable `u64` with no fatal launch exceptions in the captured logcat. The post-install route probe also reached `/play` and rendered the Play Files shell on-device. | PASS: after `c64u` recovered, the same final installed APK remained stable on Pixel 4 with `C64U · HEALTHY` on Home, `Device c64u` / `Firmware 1.1.0`, no fatal launch exceptions in the captured logcat, and the Play Files route rendered on-device. | `evidence/phase6-u64-home-after-final-install.png`, `evidence/phase6-u64-home-after-final-install.txt`, `evidence/phase6-u64-play-after-final-install.png`, `evidence/phase6-c64u-home-after-final-install.png`, `evidence/phase6-c64u-home-after-final-install.txt`, `evidence/phase6-c64u-play-after-final-install.png` |
