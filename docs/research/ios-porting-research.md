...
3. Screenshot publication policy (decision):
   ...
   - Implementation meaning: `ios-screenshots` always uploads the curated screenshot pack as a CI artifact; no automatic commit to `docs/img/`.
...
| R9  | Existing Maestro config/flows are Android-specific and not reusable as-is for iOS                         | `.maestro/config.yaml:16`, `docs/testing/maestro.md:20`, `.maestro/smoke-hvsc-mounted.yaml:37`                                                                                              | High       | Medium | High     | Add iOS tags/subflows and selector variants; keep shared flow structure where possible                                     | iOS-tagged Maestro suite runs green without Android selectors/permissions assumptions                     |
