...
| Physical HIL           | Production evidence                                       | `docs/testing/physical-device-matrix.md`, agentic docs | `npm run startup:baseline`, `scope:hil:evidence`, app-first droidmind/c64scope/c64bridge runs                                                          | Manual/HIL                         | Pixel 4 + real C64U                  | c64scope timelines, screenshots, logcat, app diagnostics, A/V assertions    | Long                    | real networking, safety, A/V                                  | CI-only repeatability                        |
...
- Known-fast comparator slider such as SID Socket 1 Pan: Playwright/audio mixer extension candidate; current comparator is covered by audio mixer tests but not a soak budget.
...
| Partial connectivity: health fails but config works | Planned + HIL     | degraded-state UI/diagnostics scenario              | TBD Playwright/Vitest/HIL                                                                                                                                                                        | Planned    | HIL for production                | logs + UI                 | Yes             | Observed on Pixel 4 against `u64`: Config populated while health remained `U64 UNHEALTHY` |
| Android native DNS/LAN failure                      | Maestro/HIL       | startup real-device connection validation           | `./build --test-maestro-ci`; physical HIL runbook                                                                                                                                                | Manual/HIL | Emulator/physical                 | Maestro/logcat/HIL bundle | Yes             | Pixel 4 launched current APK; native health mismatch requires investigation               |
...
| Physical LAN hostname/DNS behavior         | Emulator and web localhost do not prove real Wi-Fi/LAN resolution          | Real device may fail while web passes                                     | Pixel 4 + `u64`/`c64u` preflight and app-first connection evidence                                              | Yes                                                                           |
...
| Native health false negative on Pixel 4    | Current APK can render config from `u64` while the badge remains unhealthy | Users may see stale/degraded connection state despite working REST/config | Investigate native health-check composition, logcat `Host unreachable`, and saved-device/probe target selection | Yes                                                                           |
