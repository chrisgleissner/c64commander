# F012 Playback transport controls and queue progression

## Feature Under Test

Playback transport controls and queue progression
...
- Dependency features in terminal state: F011
...
1. Start a `c64scope` session with caseId `F012` and artifact root under `/home/chris/dev/c64/c64commander/docs/testing/agentic-tests/full-app-coverage/runs/`.
...
- Play starts selected/expected item.
- Pause/resume/stop states match button semantics.
- Next/previous move queue deterministically.
...
- Evidence set is complete and mapped to `F012`.
