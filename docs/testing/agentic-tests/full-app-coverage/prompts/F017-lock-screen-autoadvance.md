# F017 Lock-screen/background auto-advance playback

## Feature Under Test

Lock-screen/background auto-advance playback
...
- Dependency features in terminal state: F012
...
1. Start a `c64scope` session with caseId `F017` and artifact root under `/home/chris/dev/c64/c64commander/docs/testing/agentic-tests/full-app-coverage/runs/`.
2. Use `droidmind` to bring app to `/play`.
...
- Playback is active before locking.
- Auto-advance trigger occurs while locked/backgrounded.
- Track state after unlock matches expected single-step progression.

## Evidence Requirements

- App screenshots at each intermediate assertion point.
- C64 screenshots when device-visible behavior is expected.
- App/C64 video capture: required (app + c64 media evidence).
...
- Evidence set is complete and mapped to `F017`.
