# F009 Drive config and Soft IEC controls

## Feature Under Test

Drive config and Soft IEC controls
...
- Start route target: `/disks`
- Dependency features in terminal state: F007
...
1. Start a `c64scope` session with caseId `F009` and artifact root under `/home/chris/dev/c64/c64commander/docs/testing/agentic-tests/full-app-coverage/runs/`.
2. Use `droidmind` to bring app to `/disks`.
...
- Bus/type/power/reset controls apply expected state.
- Soft IEC enable/bus/default path updates are reflected.
- Refresh does not revert intended values.
...
- Evidence set is complete and mapped to `F009`.
