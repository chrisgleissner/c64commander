# F015 HVSC download/install/ingest/cancel/reset lifecycle

## Feature Under Test

HVSC download/install/ingest/cancel/reset lifecycle
...
- Dependency features in terminal state: F010
...
1. Start a `c64scope` session with caseId `F015` and artifact root under `/home/chris/dev/c64/c64commander/docs/testing/agentic-tests/full-app-coverage/runs/`.
...
- Download progresses through expected phases.
- Ingest state and summary counters are coherent.
- Cancel/reset produce valid terminal state.
...
- Evidence set is complete and mapped to `F015`.
