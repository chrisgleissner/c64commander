# F003 Home machine controls (reset/reboot/menu/power/pause-resume)

## Feature Under Test

Home machine controls (reset/reboot/menu/power/pause-resume)
...
1. Start a `c64scope` session with caseId `F003` and artifact root under `/home/chris/dev/c64/c64commander/docs/testing/agentic-tests/full-app-coverage/runs/`.
...
- Each control triggers expected confirmation/state transition.
- Device returns to stable connected state after action.
- No unintended side effects on unrelated controls.
...
- Evidence set is complete and mapped to `F003`.
