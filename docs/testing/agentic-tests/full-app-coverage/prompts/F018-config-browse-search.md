# F018 Config category browse/search/refresh

## Feature Under Test

Config category browse/search/refresh
...
1. Start a `c64scope` session with caseId `F018` and artifact root under `/home/chris/dev/c64/c64commander/docs/testing/agentic-tests/full-app-coverage/runs/`.
...
- Categories load and are searchable.
- Category expand/collapse is deterministic.
- Refresh updates content without broken state.
...
- Evidence set is complete and mapped to `F018`.
