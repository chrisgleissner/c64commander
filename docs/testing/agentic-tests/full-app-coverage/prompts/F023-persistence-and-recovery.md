# F023 Persistence across restart/lock and reconnect recovery

## Feature Under Test

Persistence across restart/lock and reconnect recovery
...
- Start route target: `/`
- Dependency features in terminal state: F001,F012,F020
...
1. Start a `c64scope` session with caseId `F023` and artifact root under `/home/chris/dev/c64/c64commander/docs/testing/agentic-tests/full-app-coverage/runs/`.
...
- Key state snapshots before/after restart are consistent.
- Lock/unlock does not corrupt active session state.
- Reconnect transitions are deterministic and observable.
...
- Evidence set is complete and mapped to `F023`.
