# F016 HVSC cache reuse and cached-content browsing

## Feature Under Test

HVSC cache reuse and cached-content browsing
...
- Dependency features in terminal state: F015
...
1. Start a `c64scope` session with caseId `F016` and artifact root under `/home/chris/dev/c64/c64commander/docs/testing/agentic-tests/full-app-coverage/runs/`.
...
- Cache presence is detected before ingest.
- Re-ingest can run from cache without full redownload.
- Cached content is browseable/playable through app path.
...
- Evidence set is complete and mapped to `F016`.
