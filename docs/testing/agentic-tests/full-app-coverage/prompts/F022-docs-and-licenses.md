# F022 Docs sections and open-source licenses rendering

## Feature Under Test

Docs sections and open-source licenses rendering
...
1. Start a `c64scope` session with caseId `F022` and artifact root under `/home/chris/dev/c64/c64commander/docs/testing/agentic-tests/full-app-coverage/runs/`.
2. Use `droidmind` to bring app to `/docs`.
...
- Docs accordion sections expand/collapse correctly.
- Licenses page opens and renders bundled notices.
- No render or fetch errors are logged.
...
- Evidence set is complete and mapped to `F022`.
