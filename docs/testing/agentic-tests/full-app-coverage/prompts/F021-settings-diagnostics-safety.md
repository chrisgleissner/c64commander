# F021 Settings diagnostics, settings transfer, and device safety controls

## Feature Under Test

Settings diagnostics, settings transfer, and device safety controls
...
- Start route target: `/settings`
- Dependency features in terminal state: F020
...
1. Start a `c64scope` session with caseId `F021` and artifact root under `/home/chris/dev/c64/c64commander/docs/testing/agentic-tests/full-app-coverage/runs/`.
2. Use `droidmind` to bring app to `/settings`.
...
- Diagnostics tabs/filter/share/clear behave correctly.
- Import/export settings works with expected file artifacts.
- Safety preset and advanced fields apply deterministically.
...
- Evidence set is complete and mapped to `F021`.
