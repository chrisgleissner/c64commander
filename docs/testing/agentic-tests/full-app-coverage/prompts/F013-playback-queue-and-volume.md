# F013 Shuffle/repeat/recurse/reshuffle and volume controls

## Feature Under Test

Shuffle/repeat/recurse/reshuffle and volume controls
...
1. Start a `c64scope` session with caseId `F013` and artifact root under `/home/chris/dev/c64/c64commander/docs/testing/agentic-tests/full-app-coverage/runs/`.
...
- Toggle states persist while active session runs.
- Reshuffle changes queue order when allowed.
- Volume/mute changes match UI labels and behavior.

## Evidence Requirements

- App screenshots at each intermediate assertion point.
- C64 screenshots when device-visible behavior is expected.
- App/C64 video capture: recommended.
...
- Evidence set is complete and mapped to `F013`.
