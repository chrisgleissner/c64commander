# F019 Config edit round-trip, mixer solo/reset, clock sync

## Feature Under Test

Config edit round-trip, mixer solo/reset, clock sync
...
- Start route target: `/config`
- Dependency features in terminal state: F018
...
1. Start a `c64scope` session with caseId `F019` and artifact root under `/home/chris/dev/c64/c64commander/docs/testing/agentic-tests/full-app-coverage/runs/`.
2. Use `droidmind` to bring app to `/config`.
...
- Edited values round-trip after refresh/readback.
- Mixer solo/reset actions apply correctly.
- Clock sync action completes without regression.
...
- Evidence set is complete and mapped to `F019`.
