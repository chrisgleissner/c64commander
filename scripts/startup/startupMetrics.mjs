...
const isUserCommandPath = (path) =>
  path === '/v1/machine:reset' || path === '/v1/machine:reboot';
...
export const parseStartupMetricsFromLines = (lines) => {
  ...
  for (const line of lines) {
    ...
    if (
      lowered.includes('capacitor: app resumed') ||
      lowered.includes('capacitor: app started')
    ) {
    ...
    const looksLikeRequest =
      startupWindowOpen &&
      (lowered.includes('capacitorhttp') ||
        lowered.includes('c64 api request') ||
