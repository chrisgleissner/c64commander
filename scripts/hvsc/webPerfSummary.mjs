...
const collectScenarioRuns = (iterations, scenarioName) =>
    iterations.flatMap((iteration) => (iteration.scenarios ?? []).filter((scenario) => scenario.scenario === scenarioName));
...
export const summarizeScenarioIterations = (iterations) => {
    ...
    const scenarioSummaries = Object.fromEntries(
        scenarioNames.map((scenarioName) => [scenarioName, summarizeScenarioRuns(collectScenarioRuns(iterations, scenarioName))]),
    );
