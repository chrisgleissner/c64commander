...
const buildDiffOverlay = (a, b) => {
  ...
  for (let i = 0; i < total; i++) {
    const base = i * 4;
    const rA = a.data[base],
      gA = a.data[base + 1],
      bA = a.data[base + 2],
      aA = a.data[base + 3];
    const rB = b.data[base],
      gB = b.data[base + 1],
      bB = b.data[base + 2],
      aB = b.data[base + 3];
...
const generateDiffImage = async (filePath, headBlob, outPath) => {
  const [disk, head] = await Promise.all([toRgba(await readFile(filePath)), toRgba(headBlob)]);
...
const run = async () => {
  ...
  const results = await Promise.all(
    files.map(async (filePath) => {
      ...
      try {
        ...
        const comparison = await compareScreenshotBuffers(headBlob, diskBuffer);
        ...
        if (diffCount === -1 || comparison.status === "different-size") {
          return { filePath, status: "SIZE_CHANGED", diffCount: null, totalPixels, note };
        }

        if (comparison.status === "anti-aliased-equivalent" || comparison.status === "within-noise-budget") {
          return {
            filePath,
            status: "FUZZY_IDENTICAL",
            diffCount,
            totalPixels,
            comparisonStatus: comparison.status,
          };
        }
        return { filePath, status: "CHANGED", diffCount, totalPixels, comparisonStatus: comparison.status };
      } catch (err) {
  ...
  console.log("┌──────────────────────────────────────────────────────────────────────────────────────");
  console.log(
    `│  RESULT: ${changed.length} real changes  |  ${fuzzyIdentical.length} fuzzy-identical (rendering noise)  |  ${identical.length} exact-identical  |  ${sizeChg.length} size-changed  |  ${newFiles.length} new  |  ${errors.length} errors`,
  );
  console.log("└──────────────────────────────────────────────────────────────────────────────────────\n");
  ...
  if (fuzzyIdentical.length > 0) {
    ...
    for (const r of fuzzyIdentical) {
      ...
      console.log(
        `   FUZZY  ${r.filePath}  (raw: ${r.diffCount.toLocaleString()} px ${rawPct}%  |  mode=${r.comparisonStatus})`,
      );
    }
    console.log();
  }
  ...
  if (changed.length > 0) {
    ...
    for (const r of changed) {
      ...
      console.log(
        `   CHANGED  ${r.filePath}  (${r.diffCount.toLocaleString()} px, ${pct}%  |  mode=${r.comparisonStatus})`,
      );
    }
    console.log();
  }
