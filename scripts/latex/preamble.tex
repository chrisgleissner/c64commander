...
% =====================================================================
%
%  Fonts are chosen by engine. Under LuaLaTeX the book is set in IBM Plex, the
%  same family the app itself uses, so the printed page and the screen are one
%  design. Under pdfLaTeX, which cannot load a system OpenType face, it falls
%  back to the TeX Gyre revivals: Pagella for the body, Heros for headings.
%  Both are real book faces, so the fallback is a change of dress, not of
%  quality.
\usepackage{iftex}
\ifLuaTeX
  \usepackage{fontspec}
...
% \colorlet, which is otherwise local to the group the chapter head
% is set in: without it the opener takes the new hue and every figure caption,
% subheading and bullet in the chapter that follows stays on the old one.
