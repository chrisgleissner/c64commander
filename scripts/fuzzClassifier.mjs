...
export const CONFIDENCE_LEVELS = /** @type {const} */ ([
  'HIGH',
  'MEDIUM',
  'LOW',
]);
...
export const classifyDomain = (signature, lastInteractions) => {
  ...
  if (includes(rawMsg, FUZZ_INFRASTRUCTURE_PATTERNS))
    return 'FUZZ_INFRASTRUCTURE';
  if (includes(rawExc, FUZZ_INFRASTRUCTURE_PATTERNS))
    return 'FUZZ_INFRASTRUCTURE';
  if (frames.includes('playwright/fuzz/chaosrunner.fuzz.ts'))
    return 'FUZZ_INFRASTRUCTURE';
  ...
  if (
    ...
    (msg.includes('fail') || msg.includes('error') || msg.includes('offline'))
  )
    return 'NETWORK';
  if (exc.includes('app.log') && hasChaosEvidence(lastInteractions))
    return 'NETWORK';
  ...
  if (
    includes(frames, ['TypeError', 'ReferenceError', 'components/', 'pages/'])
  )
    return 'UI';
...
export const classifyIssue = (group) => {
  ...
  const isJsError =
    rawExc.includes('TypeError') ||
    rawExc.includes('ReferenceError') ||
    rawMsg.includes('TypeError') ||
    rawMsg.includes('ReferenceError') ||
    rawMsg.includes('Cannot read propert') ||
    rawMsg.includes('is not a function') ||
    rawMsg.includes('undefined is not') ||
    rawMsg.includes('null is not');
