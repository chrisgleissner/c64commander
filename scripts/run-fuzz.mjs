  ...
  try {
    ...
    for (const sessionJsonPath of sessionJsonFiles) {
      ...
      for (const relativePath of requiredSessionPaths) {
        const mergedRelativePath = resolveMergedArtifactPath(relativePath, sessionJsonPath);
        await ensureFile(path.join(outputRoot, mergedRelativePath)).catch((error) => {
          missingArtifacts.push({
            path: `${path.relative(outputRoot, sessionJsonPath)} -> ${mergedRelativePath}`,
      ...
      const mergedVideoRelativePath = resolveMergedArtifactPath(videoRelativePath, sessionJsonPath);
