...
export const renderIssueEntry = (group, classification) => {
  ...
  const exampleVideos = examples
    .map((e) => e.video)
    .filter(Boolean)
    .slice(0, 3);
  const exampleScreens = examples
    .map((e) => e.screenshot)
    .filter(Boolean)
    .slice(0, 3);
  const shardIndices = [
    ...new Set(
      examples.map((e) => e.shardIndex).filter((s) => typeof s === 'number'),
    ),
  ].sort((a, b) => a - b);
  ...
  lines.push('');
  lines.push(
    `- Message: ${sanitizeMarkdownText(group.signature?.message || 'n/a')}`,
  );
  ...
  lines.push(
    `- Exception: ${sanitizeMarkdownText(group.signature?.exception || 'n/a')}`,
  );
  ...
  lines.push(
    `- Top frames: ${(group.signature?.topFrames || []).map(sanitizeMarkdownText).join(' | ') || 'n/a'}`,
  );
  if (classification.explanation) {
    lines.push(
      `- Explanation: ${sanitizeMarkdownText(classification.explanation)}`,
    );
  }
  ...
  if (exampleScreens.length) {
    lines.push(
      `- Screenshots: ${exampleScreens.map((shot) => `[${shot}](${shot})`).join(', ')}`,
    );
  }
...
export const renderReadme = (meta, sortedGroups, classificationMap) => {
  ...
  const realGroups = sortedGroups.filter(
    (g) => classificationMap.get(g.issue_group_id)?.classification === 'REAL',
  );
  const uncertainGroups = sortedGroups.filter(
    (g) =>
      classificationMap.get(g.issue_group_id)?.classification === 'UNCERTAIN',
  );
  const expectedGroups = sortedGroups.filter(
    (g) =>
      classificationMap.get(g.issue_group_id)?.classification === 'EXPECTED',
  );
  ...
  lines.push(
    'EXPECTED issues are artifacts caused by fuzz-induced instability.',
  );
  lines.push('');
  lines.push(
    'Application log levels are preserved. Classification occurs only during fuzz report analysis.',
  );
...
export const renderSummary = (meta, sortedGroups, classificationMap) => {
  ...
  lines.push(
    `REAL: ${realGroups.length} | UNCERTAIN: ${uncertainGroups.length} | EXPECTED: ${expectedGroups.length}`,
  );
  ...
  if (realGroups.length) {
    ...
    for (const group of realGroups) {
      const cls = classificationMap.get(group.issue_group_id);
      const total = Object.values(group.severityCounts || {}).reduce(
        (s, v) => s + (v || 0),
        0,
      );
      lines.push(
        `- ${group.issue_group_id} [${cls?.domain || 'UNKNOWN'} ${cls?.confidence || 'LOW'}] total=${total}: ${group.signature?.message || 'n/a'}`,
      );
    }
  }
