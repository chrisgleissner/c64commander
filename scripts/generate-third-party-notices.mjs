...
const DATA_NOTICES = [
  ...
  '',
  'The libsidplayfp WASM engine powers on-device SID playback (the optional "This device" playback engine). libsidplayfp and libresidfp (the reSIDfp engine, an external library since libsidplayfp v3.x, vendored into the same binary) are GPL-2.0-or-later, compatible with this application\'s GPL-3.0-or-later licence; the vendored build, its pinned upstream refs and its licence are under `public/wasm/libsidplayfp/` (see `VENDORING.md`).',
