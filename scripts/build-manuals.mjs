...
export const renderManualMarkdown = ({ variant, features }) => {
  ...
  const sections = [
    ...
    "### The Tour",
    "",
    `The first time you open ${appName} it offers a short guided walk through the running app rather than through pictures of it. It visits each page, puts a spotlight on what it is describing, and explains it in a line or two.`,
    "",
    ...
    "",
    `**Diagnostics** opens the diagnostics panel, switches debug logging on, and carries **Settings transfer**, which lives here despite the name. Settings transfer exports your app settings, feature switches and device-safety tuning to a file you can import onto another ${appDeviceName(
      variant,
    ...
    `Turn it on deliberately. It is not hard to save a setting that leaves the machine awkward to use, and once saved it comes back that way every time. The way out is to hold **RESTORE** while you switch ${targetDeviceShortName(
      variant,
    )} on: it then starts with its default settings instead of the saved ones, which is enough to get you back to a working machine. Nothing is erased, and your saved values are still there, so you can put the setting right and save it again.`,
    "",
    "### Docs",
    "",
      ...
      ? [
          ...
          ...(isC64uRemoteVariant(variant)
            ? [
                ...
                "",
                "Where you are lying down, or the sensor cannot tell which way up the phone is, the **Orientation** control in Game Mode's toolbar pins the mapping. Choose **Auto**, **0°**, **90°** or **270°**; the choice holds for as long as the sheet is open. It is deliberately not remembered, because an orientation pinned for one game should not still apply to the next one weeks later.",
                "",
