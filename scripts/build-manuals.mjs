...
export const renderManualMarkdown = ({ variant, features }) => {
  ...
  const sections = [
    ...
    "",
    `**How closely to copy the chip.** Under **Settings → SID Radio → SID emulation** you can choose between two ways of recreating the SID. **Accurate** follows the real chip in the finest detail. **Light** does about a third of the work and still sounds good; most listeners will not hear the difference, which makes it a sensible choice on an older ${appDeviceName(variant)} or when you want a long listen to go easy on the battery. Both play every tune, and you can change your mind at any time.`,
    "",
