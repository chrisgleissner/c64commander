...
export const renderManualMarkdown = ({ variant, features }) => {
  ...
  const sections = [
    ...
    "",
    "You can also start a station from whatever is already playing. Tap **More like this**, and the app goes looking for more music in the same vein. It is there whenever a SID is playing — including while a station is already running, which is when you are most likely to want it. Hearing a tune you like is what prompts the thought, and one tap takes the station off in that direction; you never have to stop what is playing first.",
    "",
    ...
    "",
    `Your choices stay on your ${appDeviceName(variant)} and nowhere else. They are attached to the music itself rather than to a file name, so they survive an update to your copy of HVSC even if the tune has since moved to a different folder.`,
    "",
    ...
    "",
    'You can type any part of a word, in any case, and accents make no difference — "oorni" finds Lasse Öörni, "mando" finds Commando. Add a second word to narrow it: "hubbard commando" finds the one tune that answers to both. Each result shows the composer and the folder it came from, because the same title turns up under several composers.',
    "",
    "Tap a result and it plays straight away. Your station is not disturbed: it keeps its place, and when the tune ends it carries on exactly where it was. If instead you would like more music like the tune you just found, tap the radio icon beside it and a new station starts from there.",
    "",
    "![Finding one tune by name, anywhere in HVSC](../../img/app/play/sid-radio/04-find-a-tune.png)",
    "",
    "When you have not typed anything, the sheet shows what you have recently heard, with the same two actions on each row. A station is endless and only goes forwards, so this is the way back to a tune that has just gone by.",
    "",
    "![What was just played, and the way back to it](../../img/app/play/sid-radio/05-recently-played.png)",
    "",
    ...
    "",
    `**Both** only appears when the sound can actually reach you. For that, your C64 needs a wired **Ethernet** connection to your router, and your ${appDeviceName(variant)} needs to be on the same network. If there is no path for the sound, the app simply does not offer the option.`,
    "",
    ...
    "",
    `#### Playing tunes on your ${appDeviceName(variant)}`,
    "",
    ...
    "",
    `These programs are still under copyright and cannot be included with an app, so ${appName} borrows them from your own machine. Go to **Settings → SID Radio → C64 ROMs for on-device playback** and tap **Read from C64**. It takes a moment, and you only ever have to do it once.`,
    "",
    `The copies stay on your ${appDeviceName(variant)}. They are never uploaded, never shared, and never included in a diagnostics report. **Remove** deletes them again whenever you wish. Please only read them from a machine that is yours, or that you have permission to use.`,
    "",
    ...
    "- **Tap the progress bar** anywhere to jump to that point, or hold and slide along it to travel through the tune. The music picks up wherever you let go.",
    `- Jumping *forward* into a part of the tune your ${appDeviceName(variant)} has not prepared yet is the one move that takes a moment: the music has to be worked out from where it is up to, and it cannot be skipped over. The app says so plainly — the timer holds at the last note you heard, and the bar shows how far along the preparation is and roughly how long is left. When it is ready, playing resumes exactly where you asked. Jumping back, or forward into a part already prepared, is immediate.`,
    "",
    "#### The sound itself",
    "",
    `**Volume and Mute.** While your ${appDeviceName(variant)} is playing a tune, these turn down the music alone. They do not touch your device's own volume, so muting a tune here leaves your ringer and your notifications exactly as they were.`,
    "",
    `**One sound at a time.** Your ${appDeviceName(variant)} can either play a tune itself or play the sound sent from your C64, and it will never do both at once. Whichever you start last takes over and the other stops, so you never have to work out which of two sounds you are chasing.`,
    "",
    `**Crossfading.** Normally one tune ends before the next begins. If you would rather they overlapped, **Settings → SID Radio → Crossfade** offers a short, medium or long fade. Fading one tune into another means playing both for a moment, and only your ${appDeviceName(variant)} can do that — your C64 plays a single tune at a time — so the setting applies when **Listen on** is set to **Local**.`,
    "",
    ...
    "",
    `**How closely to copy the chip.** Under **Settings → SID Radio → SID emulation** you can choose between two ways of recreating the SID. **Accurate** follows the real chip in the finest detail. **Light** asks about a third as much of your ${appDeviceName(variant)} and still sounds good — most listeners will not hear the difference. Choosing it leaves more headroom for everything else, so the app stays quick to answer while the music plays, and it makes a long listen kinder to the battery. Both play every tune, and you can change your mind at any time.`,
    "",
