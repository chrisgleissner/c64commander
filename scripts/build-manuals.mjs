      ...
      ? [
          ...
          ...(includeFeature(features, "audio_mirror_enabled")
            ? [
                ...
                "",
                `Sound is light enough to travel over **Wi‑Fi**. When you **Listen** without the picture, ${appName} can ask the C64 to send its audio over Wi‑Fi instead of the wired network — handy when the C64 and your ${appDeviceName(
                  variant,
                )} share only a wireless router, or when you just want a SID tune sent straight to your ${appDeviceName(
                  variant,
                )} with no cabling between them. The picture is too heavy for Wi‑Fi, so this is for **sound on its own** — Wi‑Fi audio never runs at the same time as **Watch**.`,
                "",
                ...
                "",
                `If your ${appDeviceName(
                  variant,
                )} has no Wi‑Fi to offer, ${appName} simply uses the wired network — there is nothing to set up.`,
                "",
              ]
            : []),
