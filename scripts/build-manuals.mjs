...
export const renderManualMarkdown = ({ variant, features }) => {
  ...
  const sections = [
    ...
    "",
    `Your C64 will play any of it. So, for most tunes, will your ${appDeviceName(variant)} — this section covers where the sound comes from, how to move about inside a piece, and the handful of settings that shape it. The next section, **SID Radio**, is about letting the app choose the music for you.`,
    "",
    ...
    "#### The sound itself",
    "",
    `**Volume and Mute** turn down the music alone, leaving your ringer and notifications as they were. Your ${appDeviceName(variant)} will either play a tune itself or play the sound sent from your C64, never both at once: whichever you start last takes over.`,
    "",
    `The rest lives under **Settings → SID Radio**. **Crossfade** overlaps one tune into the next by a short, medium or long fade — only your ${appDeviceName(variant)} can play two at once, so it applies when **Listen on** is **Local**. The C64 was made with two versions of the SID, the earlier **6581** and the later **8580**, and music written for one sounds a little different on the other; most files record which the composer used, and for those that do not, **Match my Commodore 64** reads the chip from your own machine and remembers it. **SID emulation** chooses how closely the chip is recreated: **Accurate** follows the real thing in the finest detail, while **Light** asks about a third as much of your ${appDeviceName(variant)}, still sounds good to most listeners, and is kinder to the battery on a long listen.`,
    "",
    `SID Radio, the heart and cross, and playing tunes on your ${appDeviceName(variant)} are all switched on and off under **Settings → SID Radio**, which also shows which version of the music map and of HVSC you have installed.`,
    "",
    "#### The SID Audio Mixer",
    "",
    ...
    "### SID Radio",
    "",
    `SID Radio plays that archive like a radio station. You choose a mood, or a tune you already like, and the app keeps finding more music of the same kind for as long as you care to listen. You never have to build a playlist, and it needs no internet connection: once the music is on your ${appDeviceName(variant)} there is nothing further to download.`,
    "",
    ...
    "",
    `Your choices stay on your ${appDeviceName(variant)} and are attached to the music itself rather than to a file name, so they survive an update to HVSC even if the tune has moved. **Liked Tunes** is an ordinary playable list: play it, shuffle it, or take a tune off it again.`,
    "",
