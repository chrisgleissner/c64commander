...
export const renderManualMarkdown = ({ variant, features }) => {
  ...
  const sections = [
    ...
    "### The Tour",
    "",
    `The first time you open ${appName} it offers a short guided walk through the app — the real app, not pictures of it. It goes to each page, puts a spotlight on what it is talking about, and explains it in a line or two.`,
    "",
    "**Step n of 8** at the bottom says where you are, and **Skip**, **Back** and **Next** are always there; on a keypad, Left and Right are Back and Next, OK is Next, and the Back key leaves. You can stop at any point.",
    "",
    "It is not a one-off. Start it again whenever you like from the card at the top of **Docs**, or from **Settings → About → Take the tour**. If you took it before your C64 was connected, the three steps that needed one explained themselves instead of pointing at anything; Home offers those three again, once, the first time a machine answers.",
    "",
    "### Home",
    "",
    ...
    "",
    "Start at the top. The search field comes first, and under it **Listen and play** — four tiles for the things that need no C64 attached at all. **Radio** starts a SID Radio station, thousands of tunes played on this device with no network and no hardware. **Resume** picks up the last tune where it stopped, and names it. **Recent** is the way back to anything you have opened before. **Live View** goes to the card that brings the machine's picture and sound into the app. A tile you cannot use right now is still there, greyed, saying what it is waiting for.",
    "",
    "Below that come the machine's own controls, and below those the system strip, which confirms which app build, device, and firmware you are using. Quick Actions run from the ones you reach for most to the ones you reach for rarely: **Game** and **Input**, which sit together as two ways into the same second screen, then Menu, Pause/Resume, the RAM snapshot actions, and then, in their own band, the ones that interrupt whatever the machine is doing — Reset, Reboot, and Power Off where the device supports it. Nothing that stops your C64 sits next to something that does not.",
    "",
    ...
    "",
    "**With no C64 connected**, Home rearranges. Search and Listen and play stay where they are, because neither needs a machine. In place of the machine controls is a card explaining how to connect one, the system strip drops to the app version alone, and the machine's cards are drawn as titles with nothing inside them, under a line saying so. Nothing is lost: whichever cards you had open are open again the moment your C64 answers. The app waits a few seconds before rearranging, so a brief network hiccup does not shuffle the page under you, and it goes back the instant the machine is there again.",
    "",
