...
export const renderManualMarkdown = ({ variant, features }) => {
  ...
  const sections = [
    ...
    "",
    `Your C64 will play any of it. So, for most tunes, will your ${appDeviceName(variant)}. This section is about that music and how to listen to it; the next one, **SID Radio**, is about letting the app choose it for you.`,
    "",
    ...
    "",
    `The rest lives under **Settings → SID Radio**. **Crossfade** overlaps one tune into the next by a short, medium or long fade; both tunes sound at once while one recedes, which only your ${appDeviceName(variant)} can do, so it applies when **Listen on** is **Local**. The C64 was made with two versions of the SID — the earlier **6581** and the later **8580** — and music written for one sounds a little different on the other. Most files record which the composer used; for those that do not, **Match my Commodore 64** reads the chip from your own machine and remembers it. **SID emulation** decides how closely the chip is recreated: **Accurate** follows the real thing in the finest detail, while **Light** asks about a third as much of your ${appDeviceName(variant)}, still sounds good to most listeners, and is kinder to the battery on a long evening.`,
    "",
    "#### The SID Audio Mixer",
    "",
    ...
    "",
    `Your choices stay on your ${appDeviceName(variant)}. They are attached to the music itself rather than to a file name, so they survive an update to the collection even if the tune has moved. **Liked Tunes** is an ordinary playable list: play it, shuffle it, or take a tune off it again.`,
    "",
