...
## Prod Hardening 1 Convergence Plan (2026-05-22)

### Current-state summary

- Classification: `DOC_PLUS_CODE`; playlist and Play Files surfaces also require `UI_CHANGE` discipline because behavior must remain UX-equivalent, but no documented visible UI change is currently planned.
- Startup triage was run before production-code edits: required guidance docs were read, `git status --short`, `git diff --stat`, `git diff --check`, scoped diffs, and `c64scope/package-lock.json` diff were captured.
...
- PR #262 review follow-up is complete locally: four Copilot review findings were addressed with focused regression tests, the review threads were resolved on GitHub, and CI must be green before merge readiness.

### Task table

| ID    | Finding IDs                            | Status | Touched files                                                                              | Test requirements                                                                                                                                      | Evidence requirements                                                                                                                                         | Residual risk                                                                                                                    |
| ----- | -------------------------------------- | ------ | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| PH0   | Phase 0                                | DONE   | evidence files                                                                              | final validation listed below                                                                                                                          | adb/curl probes; APK install; cold-launch logcat counts; CPU slider; lock/unlock; background/foreground captured; c64u unavailable                            | `c64u` REST reset during final proof, so cross-device live switch proof is deferred.                                             |
| CD1   | Current diff lockfile audit            | DONE   | `c64scope/package-lock.json`                                                               | `git diff --check` after revert                                                                                                                        | WORKLOG entry proving unrelated and reverted                                                                                                                  | None known after targeted file revert.                                                                                           |
| CD2   | Current diff binary audit              | DONE   | `src/pages/playFiles/playlistRepositorySync.ts`                                            | textual diff plus playlist sync unit tests                                                                                                             | NUL/encoding scan result in WORKLOG                                                                                                                           | None known; readable diff restored.                                                                                              |
| CD3   | Existing diff classification           | DONE   | all currently modified files                                                               | targeted tests per touched area                                                                                                                        | PLANS Current Diff Triage table                                                                                                                               | None known after final validation.                                                                                               |
| PH5   | Exception policy                       | DONE   | `src`, `android/app/src/main/java`, `tests`, `android/app/src/test`                        | catch guardrail; saved-device parse; HVSC snapshot parse/read; diagnostics stringify; Android JVM                                                      | `20260522T180545Z-phaseA-scan.txt`                                                                                                                           | Legacy `console.*` sites remain explicit PH6 residual risk, not silent catches.                                                  |
| PH8   | Playlist repository snapshot key       | DONE   | `src/pages/playFiles/playlistRepositorySync.ts`, playlist sync tests                       | serialized-field metadata/order regressions plus unchanged snapshot short-circuit                                                                       | readable textual diff; playlist sync tests                                                                                                                    | None known.                                                                                                                      |
| PH9   | FTP interaction keys                   | DONE   | `src/lib/deviceInteraction/deviceInteractionManager.ts`, `src/lib/ftp/ftpClient.ts`, tests | concurrent cross-host no-coalesce; cooldown/backoff isolation; same-host coalescing                                                                    | unit coverage; c64u live proof deferred                                                                                                                       | Live cross-device FTP proof deferred with c64u outage.                                                                           |
| PH10  | Scheduler queue cancellation           | DONE   | `src/lib/deviceInteraction/deviceInteractionManager.ts`, tests                             | queued REST/FTP/Telnet rejected on saved-device reset; new-device actions proceed; same-device priority/concurrency preserved                          | unit coverage; final u64 interaction evidence                                                                                                                 | Already-running native calls remain non-revocable, but guarded callers reject stale queued work.                                 |
| PH1   | Playlist query filtering               | DONE   | `src/pages/playFiles/hooks/useQueryFilteredPlaylist.ts`, playlist filter tests              | repository-ready query changes avoid full React memory filtering; repository-error path avoids large synchronous fallback                              | targeted/unit coverage; final Pixel Play navigation evidence                                                                                                  | Large-list live dataset proof limited by available device state.                                                                  |
| PH12  | Playlist query index/list construction | DONE   | `src/lib/playlistRepository/queryIndex.ts`, query index tests                              | selective query avoids full ordered scan; bounded item model construction; actions preserved                                                           | query-index regression tests                                                                                                                                  | None known.                                                                                                                      |
| PH13  | Duration edit and commit churn         | DONE   | `src/pages/PlayFilesPage.tsx`, `PlaybackSettingsPanel`, duration contract tests             | rapid duration edits bounded commits; final value persists; precedence preserved                                                                       | duration contract tests                                                                                                                                       | Pixel duration edit was not repeated after c64u degraded; unit contract covers churn.                                            |
| PH2   | Idle config snapshot                   | DONE   | `src/hooks/useAppConfigState.ts`, app config tests                                         | hidden skip; background intent; cancel on visibility; user-initiated path preserved                                                                    | final background/foreground evidence on u64                                                                                                                   | None known.                                                                                                                      |
| PH3   | Background rediscovery                 | DONE   | `src/components/ConnectionController.tsx`, connection controller tests                      | hidden app does not schedule rediscovery; visible re-arms once                                                                                         | final background/foreground evidence on u64                                                                                                                   | None known.                                                                                                                      |
| PH4   | Playback start unmute ordering         | DONE   | `src/pages/playFiles/hooks/usePlaybackController.ts`, playback controller tests             | connection before unmute; failed connection avoids unmute                                                                                              | playback controller tests                                                                                                                                     | Muted live playback proof limited by available playlist/device state.                                                            |
| PH14  | Recursive import/native IO             | DONE   | add/import handlers, disk mount, tests                                                     | cancel prevents late playlist mutation; stale native read result ignored; large IO bounded                                                             | add-file/disk unit tests; final Pixel interaction evidence                                                                                                    | Native bridge reads cannot be force-killed once started, but JS no longer accepts late aborted results.                          |
| PH6   | Production log noise                   | DONE   | HVSC perf logging, `main.tsx`, startup font loading tests                                  | no default HVSC perf `console.info`; native skips Google Fonts; web still loads fonts                                                                  | final launch logcat; `fontLoading.test.ts`                                                                                                                    | Legacy console allowlist remains outside this PH6 fix.                                                                           |
...
| PRR   | PR #262 review comments                | DONE   | Play Files duration, playlist sync, query index, catch guardrail tests                      | focused review-comment Vitest; full coverage/lint/build after final patch set                                                                          | Four Copilot review threads resolved on GitHub after follow-up commit                                                                                          | CI must still complete green on the latest pushed head before merge readiness.                                                   |
...
| File/change                                                                                                                                                                                                                                                                                                                                                  | Classification | Finding  | Status | Action                                                                                                                                      |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------- | -------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `android/app/src/main/java/uk/gleissner/c64commander/HvscIngestionPlugin.kt` cleanup/unregister catch logging                                                                                                                                                                                                                                                | KEEP           | PH5      | DONE   | Logging style/context retained and Android JVM tests passed.                                                                                |
| `c64scope/package-lock.json` peer flags                                                                                                                                                                                                                                                                                                                      | REVERT         | none     | DONE   | Reverted only this file; churn was unrelated peer-flag noise.                                                                               |
| `src/components/itemSelection/ArchiveSelectionView.tsx` query-preview catch logging                                                                                                                                                                                                                                                                          | REWORK         | PH5      | DONE   | Production fallback is low-noise and diagnosable.                                                                                           |
| `src/lib/deviceInteraction/deviceInteractionManager.ts` FTP scope and scheduler cancellation                                                                                                                                                                                                                                                                 | COMPLETE       | PH9/PH10 | DONE   | Concurrent/cooldown/reset tests passed; c64u live switch proof deferred with outage evidence.                                               |
| `src/lib/diagnostics/logger.ts` stringify fallback marker                                                                                                                                                                                                                                                                                                    | KEEP           | PH5      | DONE   | Regression test validates recursion-safe fallback marker.                                                                                   |
| `src/lib/diagnostics/recentTargets.ts` sessionStorage logging                                                                                                                                                                                                                                                                                                | KEEP           | PH5      | DONE   | Expected storage absence remains low-noise; unexpected failures log context.                                                                |
| `src/lib/ftp/ftpClient.ts` host/port metadata                                                                                                                                                                                                                                                                                                                | KEEP           | PH9      | DONE   | FTP list/read/write supply stable host/port metadata; same-host coalescing preserved.                                                       |
| `src/lib/hvsc/hvscBrowseIndexStore.ts` parse/read fallback logging                                                                                                                                                                                                                                                                                           | REWORK         | PH5      | DONE   | First-run absence stays quiet; parse/read failures are diagnosable and tested.                                                              |
| `src/lib/media-index/filesystemMediaIndex.ts` parse/read fallback logging                                                                                                                                                                                                                                                                                    | REWORK         | PH5      | DONE   | First-run absence stays quiet; parse/read failures are diagnosable and tested.                                                              |
| `src/lib/savedDevices/store.ts` parse fallback uses `console.warn`                                                                                                                                                                                                                                                                                           | COMPLETE       | PH5      | DONE   | Replaced with repository diagnostics logging and added saved-device parse regression.                                                       |
| `src/hooks/useHealthState.ts`, `src/lib/deviceInteraction/restRequestIdentity.ts`, `src/lib/telnet/telnetClient.ts`, `src/lib/tracing/payloadPreview.ts`, `src/lib/hvsc/hvscFilesystem.ts`, `src/lib/diagnostics/healthModel.ts`, `src/lib/diagnostics/healthCheckEngine.ts`, `src/lib/startup/serviceWorkerRegistration.ts` silent catch/fallback hardening | COMPLETE       | PH5      | DONE   | Added low-noise diagnostics or expected-missing checks and guarded stale/cancellation fallbacks.                                            |
| `src/pages/playFiles/playlistRepositorySync.ts` binary diff with NUL bytes                                                                                                                                                                                                                                                                                   | COMPLETE       | PH8      | DONE   | Restored UTF-8 text and derived snapshot key from canonical serialized payload.                                                             |
| `tests/unit/diagnostics/logger.test.ts` stringify fallback test                                                                                                                                                                                                                                                                                              | KEEP           | PH5      | DONE   | Kept with marker-based implementation.                                                                                                      |
| `tests/unit/hvsc/hvscIngestionPipeline.test.ts` logging mock update                                                                                                                                                                                                                                                                                          | KEEP           | PH5      | DONE   | Kept for diagnostics logging helper coverage.                                                                                               |
| `tests/unit/quality/catchGuardrail.test.ts` empty/silent catch guardrail                                                                                                                                                                                                                                                                                     | KEEP           | PH5      | DONE   | Added repo scan guardrail with explicit allowlist for documented test-only cancellation cleanup.                                            |
| `tests/unit/lib/deviceInteraction/deviceInteractionManager.test.ts` PH9 tests                                                                                                                                                                                                                                                                                | COMPLETE       | PH9/PH10 | DONE   | Cross-host test is concurrent; cooldown isolation, reset cancellation, priority, and concurrency coverage added.                            |
| `src/pages/playFiles/hooks/useQueryFilteredPlaylist.ts` repository-backed query filtering                                                                                                                                                                                                                                                                    | COMPLETE       | PH1      | DONE   | Avoids memory matcher work when repository filtering is ready and keeps stale results instead of full-filtering large playlists on query errors. |
| `tests/unit/playFiles/useQueryFilteredPlaylist.test.tsx` PH1 regressions                                                                                                                                                                                                                                                                                     | COMPLETE       | PH1      | DONE   | Added repository-ready no-memory-match and large repository-error stale-result tests.                                                        |
| `src/lib/playlistRepository/queryIndex.ts` candidate-rank selective query                                                                                                                                                                                                                                                                                    | COMPLETE       | PH12     | DONE   | Selective text/category candidate sets now sort by persisted rank maps instead of scanning every ordered id.                                |
| `tests/unit/lib/playlistRepository/queryIndex.test.ts` PH12 selective query regressions                                                                                                                                                                                                                                                                      | COMPLETE       | PH12     | DONE   | Added large indexed-text and deterministic sort-order tests.                                                                                |
| `src/pages/PlayFilesPage.tsx`, `src/pages/playFiles/components/PlaybackSettingsPanel.tsx` duration override commit path                                                                                                                                                                                                                                      | COMPLETE       | PH13     | DONE   | Slider/input changes update immediate UI state and debounce/commit accepted full-playlist overrides instead of mapping on every tick.       |
| `tests/unit/pages/playFiles/PlayFilesPage.durationContracts.test.ts`, `PlaybackSettingsPanel.test.tsx` PH13 contracts                                                                                                                                                                                                                                         | COMPLETE       | PH13     | DONE   | Added contracts preventing per-tick playlist rewrites and proving slider commit wiring.                                                     |
| `src/hooks/useAppConfigState.ts` idle config snapshot gating                                                                                                                                                                                                                                                                                                  | COMPLETE       | PH2      | DONE   | Idle snapshot now uses background REST intent, AbortSignal, hidden-state skip, visibility cancellation, and user snapshot foreground mode.  |
| `tests/unit/hooks/useAppConfigState.test.tsx` PH2 regressions                                                                                                                                                                                                                                                                                                | COMPLETE       | PH2      | DONE   | Added hidden skip, background intent, visibility cancellation, and foreground request assertions.                                           |
| `src/components/ConnectionController.tsx` background rediscovery visibility gate                                                                                                                                                                                                                                                                             | COMPLETE       | PH3      | DONE   | Hidden app clears rediscovery timers; visible app re-arms a rate-limited foreground-safe background probe.                                  |
| `tests/unit/components/ConnectionController.test.tsx` PH3 regressions                                                                                                                                                                                                                                                                                        | COMPLETE       | PH3      | DONE   | Added hidden no-schedule, hidden cancel, and visible one-probe tests.                                                                       |
| `src/pages/playFiles/hooks/usePlaybackController.ts` playback connection/unmute ordering                                                                                                                                                                                                                                                                     | COMPLETE       | PH4      | DONE   | Playback now proves connection before unmute refresh/writes; connection failures do not call unmute.                                       |
| `tests/unit/playFiles/usePlaybackController.test.tsx` PH4 regressions                                                                                                                                                                                                                                                                                        | COMPLETE       | PH4      | DONE   | Updated ordering test and added failed-connection no-unmute assertion.                                                                      |
| `tests/unit/playFiles/playlistRepositorySync.test.ts` PH8 tests                                                                                                                                                                                                                                                                                              | COMPLETE       | PH8      | DONE   | Expanded to configOrigin, source/origin metadata, archive/subsong/category/label, order-only changes, and unchanged snapshot short-circuit. |

### Validation Commands

- Startup captured: `git status --short`, `git diff --stat`, `git diff --check`, `git diff -- src android/app/src/main/java tests android/app/src/test`, `git diff -- c64scope/package-lock.json`.
- Targeted Phase A Vitest passed after formatting: `npm run test -- tests/unit/lib/savedDevices/store.test.ts tests/unit/playFiles/playlistRepositorySync.test.ts tests/unit/lib/deviceInteraction/deviceInteractionManager.test.ts tests/unit/diagnostics/logger.test.ts tests/unit/hvsc/hvscIngestionPipeline.test.ts` (108 tests).
- Updated Phase A guardrail/targeted Vitest passed: `npm run test -- tests/unit/quality/catchGuardrail.test.ts tests/unit/lib/savedDevices/store.test.ts tests/unit/playFiles/playlistRepositorySync.test.ts tests/unit/lib/deviceInteraction/deviceInteractionManager.test.ts tests/unit/diagnostics/logger.test.ts tests/unit/hvsc/hvscIngestionPipeline.test.ts` (110 tests).
- PH1 targeted Vitest passed after formatting: `npm run test -- tests/unit/playFiles/useQueryFilteredPlaylist.test.tsx tests/unit/playFiles/useQueryFilteredPlaylist.scale.test.tsx` (16 tests).
- PH12 targeted Vitest passed after formatting: `npm run test -- tests/unit/lib/playlistRepository/queryIndex.test.ts tests/unit/playFiles/useQueryFilteredPlaylist.test.tsx tests/unit/playFiles/useQueryFilteredPlaylist.scale.test.tsx` (18 tests).
- PH13 targeted Vitest passed after formatting: `npm run test -- tests/unit/pages/playFiles/PlayFilesPage.durationContracts.test.ts tests/unit/pages/playFiles/PlaybackSettingsPanel.test.tsx tests/unit/pages/playFiles/playFilesUtils.test.ts` (39 tests).
- PH2 targeted Vitest passed after formatting: `npm run test -- tests/unit/hooks/useAppConfigState.test.tsx tests/unit/hooks/useAppConfigState.deferredCapture.test.tsx` (28 tests).
- PH3 targeted Vitest passed after formatting: `npm run test -- tests/unit/components/ConnectionController.test.tsx` (11 tests).
- PH4 targeted Vitest passed after formatting: `npm run test -- tests/unit/playFiles/usePlaybackController.test.tsx tests/unit/playFiles/usePlaybackController.concurrency.test.tsx tests/unit/playFiles/usePlaybackController.autoAdvance.test.tsx tests/unit/playFiles/usePlaybackController.subsong.test.tsx` (55 tests).
- PH14/PH6/PH11/PH15 targeted Vitest passed: `npm run test -- tests/unit/pages/playFiles/handlers/addFileSelectionsBatching.test.ts tests/unit/lib/disks/diskMount.test.ts tests/unit/smoke/smokeMode.test.ts tests/unit/lib/deviceInteraction/deviceInteractionManager.test.ts tests/unit/playFiles/playlistRepositorySync.test.ts` (117 tests).
- Startup font-loading targeted Vitest passed: `npm run test -- tests/unit/startup/fontLoading.test.ts` (2 tests).
...
- Evidence directory: `docs/research/stabilization/prod-hardening-1-evidence/`.
- Required hardware baseline: Pixel 4 `9B081FFAZ001WX`, package `uk.gleissner.c64commander`, hosts `u64` then `c64u`.
- First evidence refresh commands: `adb devices`, `curl --max-time 5 -sS http://u64/v1/info`, `curl --max-time 5 -sS http://c64u/v1/info`.
- Required APK evidence: exact APK path, app version if available, install output, cold launch output, first 12 seconds logcat counts, Telnet activity counts.
- Required interaction evidence: saved-device switch `u64 -> c64u -> u64`, no per-item enrichment storms, CPU slider, muted playback start, lock/unlock, background/foreground.
...
- PLANS rows PH0 through PH15 plus FINAL are all `DONE` or `DEFERRED` with concrete evidence and residual risk.
- Current partial diff is converged: no unexplained lockfile churn, no binary TypeScript diffs, and PH5/PH8/PH9/PH10 are complete with tests.
- PH5 scan/guardrails show no unlogged silent production catches; legacy `console.*` sites are documented as residual console-policy risk while PH6 removed the confirmed default HVSC perf output and native font-network source.
- PH8 snapshot persistence cannot skip commits for serialized metadata-only changes.
...
- PR #262 CI rerun is required after the iOS Maestro smoke-config opt-in and review-follow-up fixes; local follow-up validation, Pixel 4 deploy evidence, and review-thread resolution are complete.

## Classification

- Classification: `DOC_PLUS_CODE` and `UI_CHANGE`.
- Scope: production-hardening findings PH1-PH15 covering Play Files repository/query behavior, duration edits, background REST/rediscovery scheduling, playback ordering, diagnostics/logging, FTP/scheduler scoping, native startup probes, and recursive/native IO cancellation.
- Validation target: focused unit coverage for each finding, full repository validation, Android JVM tests, APK build/install, and Pixel 4/u64 evidence with c64u outage documented.
