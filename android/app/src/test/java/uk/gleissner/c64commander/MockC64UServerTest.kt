...
class MockC64UServerTest {
  ...
  fun configBatchUpdatesAreReflectedInCategoryResponse() {
    ...
    val updatePayload =
            JSONObject().apply {
              put("Audio Mixer", JSONObject().apply { put("Vol Socket 1", "OFF") })
            }
    ...
    val getConnection =
            URL("${server.baseUrl}/v1/configs/Audio%20Mixer").openConnection() as HttpURLConnection
  ...
  fun memoryReadWriteEndpointsRoundTrip() {
    ...
    val writeConnection =
            URL("${server.baseUrl}/v1/machine:writemem?address=00C6&data=01").openConnection() as
                    HttpURLConnection
    ...
    val readConnection =
            URL("${server.baseUrl}/v1/machine:readmem?address=00C6&length=1").openConnection() as
                    HttpURLConnection
  ...
  fun screenMemoryDefaultsMatchLiveHardwareAssumptions() {
    ...
    val screenConnection =
            URL("${server.baseUrl}/v1/machine:readmem?address=0400&length=16").openConnection() as
                    HttpURLConnection
    ...
    repeat(16) { index -> assertEquals(0x20, screenData.getInt(index)) }

    val cia2Connection =
            URL("${server.baseUrl}/v1/machine:readmem?address=DD00&length=1").openConnection() as
                    HttpURLConnection
    ...
    val vicConnection =
            URL("${server.baseUrl}/v1/machine:readmem?address=D018&length=1").openConnection() as
                    HttpURLConnection
  ...
  fun binaryWriteSupportsFullScreenBufferRoundTrip() {
    ...
    val mutateConnection =
            URL("${server.baseUrl}/v1/machine:writemem?address=0400&data=54455354")
                    .openConnection() as
                    HttpURLConnection
  ...
  fun resetClearsKeyboardBuffer() {
    ...
    val writeConnection =
            URL("${server.baseUrl}/v1/machine:writemem?address=00C6&data=05").openConnection() as
                    HttpURLConnection
    writeConnection.requestMethod = "PUT"
    assertEquals(200, writeConnection.responseCode)
    writeConnection.disconnect()
  ...
  fun mockC64UServerCanBeInstantiated() {
    ...
    config.put("general", JSONObject().apply { put("baseUrl", "http://localhost") })

    val state = MockC64UState.fromPayload(config)
    val timingProfile =
            MockTimingProfile.fromJson(
                    JSONObject(
                            """
        {
          "seed": 19,
  ...
  fun serializesConcurrentRequestsUsingSharedTimingProfile() {
    ...
    val first =
            executor.submit<Int> {
              val connection =
                      URL("${server.baseUrl}/v1/info").openConnection() as HttpURLConnection
              connection.requestMethod = "GET"
    ...
    val second =
            executor.submit<Int> {
              val connection =
                      URL("${server.baseUrl}/v1/version").openConnection() as HttpURLConnection
              connection.requestMethod = "GET"
              connection.responseCode
            }
