...
describe("agentic exploratory wave 1", () => {
  ...
  it("writes discovery and summary artifacts from the c64scope working directory", async () => {
    ...
    try {
      ...
      expect(runCaseMock).toHaveBeenNthCalledWith(
        1,
        expect.objectContaining({ caseId: "AF-PLAY-CONTINUITY-001" }),
        "serial-wave1",
        "c64u",
        path.join(tempRoot, "c64scope", "artifacts"),
      );
      expect(runCaseMock).toHaveBeenNthCalledWith(
        2,
        expect.objectContaining({ caseId: "AF-PLAY-AUTOSKIP-001" }),
        "serial-wave1",
        "c64u",
        path.join(tempRoot, "c64scope", "artifacts"),
      );
      ...
      const summary = await readFile(
        path.join(tempRoot, "c64scope", "artifacts", runDirectories[0]!, "summary.md"),
        "utf-8",
      );
