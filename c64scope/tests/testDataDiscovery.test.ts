...
describe("test-data discovery", () => {
    ...
    it("discovers the local mirror and the resolved HVSC target without following symlink loops", async () => {
        ...
        await symlink(path.relative(path.join(workspaceRoot, "test-data", "sid"), externalSidRoot), path.join(workspaceRoot, "test-data", "sid", "hvsc"));
    ...
    it("discovers the device mirror and derives approximate counts from local parity", async () => {
        execFileMock
            .mockImplementationOnce((_file: string, _args: string[], callback: (error: null, result: { stdout: string }) => void) => {
