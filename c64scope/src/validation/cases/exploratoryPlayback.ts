...
import { launchAppForeground, navigateToRoute, ensureDeviceUnlocked, tapByResourceId, tapByText } from "../appFirstPrimitives.js";
import {
    chooseSource,
    confirmAddItems,
    openAddItemsDialog,
...
async function discoverPlaybackTargets(c64uHost: string): Promise<PlaybackTargets> {
    ...
    if (discovery.sidCandidates.length < 2) {
        throw new Error(`Expected at least 2 SID candidates under ${discovery.sidPath}, found ${discovery.sidCandidates.length}.`);
    }
...
async function capturePlaybackLogcat(serial: string, artifactDir: string, fileName: string): Promise<{ path: string; text: string }> {
...
export const appFirstPlaybackContinuity: ValidationCase = {
    ...
    async run(ctx) {
        ...
        try {
            ...
            const initialTrack = await waitForTrackLabel(ctx.serial, targets.sidCandidates[0], targets.sidCandidates, 12, 800);
            ...
            const continuityLogSeen =
                /BackgroundExecutionService|Scheduled dueAtMs watchdog|Service starting/i.test(logcat.text);
...
export const appFirstPlaylistAutoAdvance: ValidationCase = {
    ...
    async run(ctx) {
        ...
        try {
            ...
            await addSidPlaylist(droidmind, ctx.serial, targets);
            await tapByResourceId(droidmind, ctx.serial, "playlist-play");
            const initialTrack = await waitForTrackLabel(ctx.serial, targets.sidCandidates[0], targets.sidCandidates, 12, 700);
            ...
            const advancedTrack = await waitForTrackLabel(ctx.serial, targets.sidCandidates[1], targets.sidCandidates, 10, 800);
